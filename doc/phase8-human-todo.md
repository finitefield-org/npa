...
- [ ] fast kernel / reference / external checker raw result を `MachineCheckResult` に変換する。
- [ ] `NormalizedCheckResult` を required checker profile order で deterministic に生成する。
- [ ] checked / failed / resource exhausted / missing checker result の comparison status を固定する。
- [ ] checker disagreement を CI failure として扱う gate を追加する。
- [ ] PR mode は required profile `reference`、nightly は `reference, external`、release は `fast-kernel, reference, external` にする。

受け入れ条件:

- [ ] checker result の hash / policy hash / request hash が mismatch すると normalization は pass しない。
- [ ] optional checker の missing は PR pass condition を壊さず、required checker missing は failure になる。
- [ ] AI sidecar は comparison status / result hash を上書きできない。
- [ ] disagreement report は module / declaration / checker profile / certificate hash を含む。
...
- [ ] challenge file の statement_core_hash / allowed axioms / import hashes を schema として固定する。
- [ ] proof certificate の theorem statement hash が challenge と一致することを検査する。
- [ ] audit bundle に proof `.npcert`、imports、policy、checker outputs、hashes、axiom report を materialize する。
- [ ] ReleaseAuditBundleManifest に real MachineCheckResult / NormalizedCheckResult / AuxiliaryResult を含める。
- [ ] bundle validation は source / tactic / AI trace なしで再実行できるようにする。

受け入れ条件:

- [ ] challenge statement mismatch は deterministic failure になる。
- [ ] missing import、wrong certificate hash、forbidden axiom は audit bundle validation で failure になる。
- [ ] AI sidecar は bundle metadata として含められても pass condition を変えない。
- [ ] high-trust audit bundle は source ignored / no network / no plugin の前提で検査できる。
...
- [ ] malformed certificate fuzz cases を reference checker に通し、panic せず reject する。
- [ ] proof mutation fixture を作り、fast kernel / reference / external checker が reject することを比較する。
- [ ] axiom report mutation、import hash mutation、noncanonical table mutation を challenge corpus に追加する。
- [ ] differential test で fast kernel OK / reference NG、reference OK / external NG を failure として保存する。
- [ ] challenge replay result を checker result oracle に接続する。

受け入れ条件:

- [ ] mutation target が invalid な場合は generator failure と checker reject を混同しない。
- [ ] outcome-hint は test helper であり、checker result の代わりにならない。
- [ ] accepted mutation は unexpected checker acceptance として CI failure になる。
- [ ] fuzz / mutation tests は deterministic seed と artifact hash を記録する。
...
- [ ] PR / nightly / release / high-trust の CI command set を repository scripts または documented workflow に固定する。
- [ ] PR mode は changed certs + reverse dependencies + reference checker required にする。
- [ ] external checker / full recursive import check / full audit bundle は nightly / release / high-trust required にする。
- [ ] performance benchmark を fast kernel、Machine API、theorem index build、AI benchmark、reference/external checker に分ける。
- [ ] reference / external checker benchmark を PR の同期必須 job に入れず、別 job または cached audit result として扱う。

受け入れ条件:

- [ ] PR の AI candidate hot path latency は Phase 8 audit 追加で増えない。
- [ ] nightly / release は reference / external checker の詳細 benchmark を記録する。
- [ ] release mode は full independent check と audit bundle generation が通らないと pass しない。
- [ ] high-trust mode は all imports recursively checked と at least two independent checkers required を満たす。
