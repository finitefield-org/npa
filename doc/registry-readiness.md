...
- Activate standalone repositories for `npa`, `npa-std`, and `npa-mathlib`
  ...
  0 baseline is fixed by SRA-00 evidence; the next activation dependency is the
  `npa` toolchain reference and `npa-std` package fixture.
- Add larger theorem layers to `npa-mathlib` only after each layer has a closed
  ...
  evidence.
- Use `doc/npa-mathlib-public-release-plan.md` as the immediate follow-up plan
