...
4. parameter / index telescope type check
5. constructor type check
...
step 0 で payload outer canonical decode / scalar field decode に失敗した場合は、singleton family count、
...
3. Phase9AiGoal universe context shape check
...
7. occurrence path traversal, occurrence.expected_ref import resolution, explicit_level_args arity / scope validation
...
2. universe_params / payload-local LevelExpr universe context shape check
3. quotient_v1 primitive refs and eq ref import resolution and public interface check
...
step 0 で payload outer canonical decode / scalar field decode に失敗した場合は、operations、
...
0. MachineSmtCertificateCandidate payload outer canonical framing / enum tag / scalar field decode,
   and reconstruction_plan.steps length prefix cap precheck
1. payload.goal canonical decode and target.goal_fingerprint binding check
2. Phase9AiGoal universe context shape check
3. Phase9AiGoal local_context / target kernel well-typedness check
4. encoded_problem Inline/Artifact bytes acquisition, file_hash / size_bytes check,
   outer framing / cap precheck, full canonical decode, problem_hash / encoding_hash recomputation and comparison
...
9. reconstruction plan canonical shape / final_step bounds / used-ref exactness / import resolution
...
step 0 で payload outer decode または `reconstruction_plan.steps` cap check に失敗した場合は、
`payload.goal`、encoded problem artifact、proof payload artifact、reconstruction step element は検査しません。
...
したがって同じ request に Phase9AiGoal universe context shape violation または goal ill-typedness と
`limit > 256` が同時にある場合は、step 2 の `EnvelopeMalformed` または step 3 の `KernelRejected` が優先です。
...
step 2 の source document / rejection reason artifact の hash mismatch、invalid UTF-8、span violation、embedded hash mismatch は
...
EnvelopeMalformed:
  ...
  feature_error = Some(SmtCertificate(NonCanonicalPayload))
    ...
    reconstruction plan の steps count cap / step id / final_step / imported_theory_refs
    canonical shape の違反。
