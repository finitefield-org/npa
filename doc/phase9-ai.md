...
`Phase9AiGlobalRef` の `module / export_hash / certificate_hash` が envelope imports に存在しない場合、
...
If an occurrence key does not identify exactly one `Const` occurrence after canonical path traversal, validator returns
`Rejected { error = FeatureRejected, feature_error = Some(UniverseRepair(AmbiguousOccurrence)) }`.
...
`payload.target_decl_hash` が envelope `target.target_decl_hash` と一致しない場合は、payload が別 target を指しているため
`Rejected { error = FeatureRejected, feature_error = Some(QuotientConstruction(TargetRefMismatch)) }`
として拒否します。
一致した hash に対して、validator が quotient declaration を再構成した後の `decl_certificate_hash` が一致しない場合は
`Rejected { error = TargetFingerprintMismatch, feature_error = None }`
として拒否します。
...
```text
MvpQfUf:
  Bool, User sort, uninterpreted FunctionDecl / DatatypeDecl only
...
function symbol table:
  ...
  - App must not use reserved or pretty names for builtin theory operators
...
EnvelopeMalformed + SmtCertificate(NonCanonicalPayload):
  ...
  - symbol ASCII grammar violation or reserved theory name misuse
  ...
  - recursive or mutually-recursive datatype selector sort
  - BitVec width = 0, wrong BitVecLit byte length, or non-zero unused high bits
  - Bool / Eq / Ite / BuiltinApp operand sort mismatch
...
FeatureRejected + SmtCertificate(EncodingMismatch):
  ...
  - deterministic encoder table would generate a different symbol for a SortDecl / FunctionDecl /
    DatatypeDecl / Var than the payload contains
...
したがって MVP で `SmtReconstructionRule::PayloadNode` を含む request は、すべて
`Rejected { error = UnsupportedFeature, feature_error = Some(SmtCertificate(RuleRegistryMismatch)) }`
として拒否します。
...
MVP の検査順序では、payload bytes の canonical decode と table shape validation の後、
`reconstruction_plan.steps` に `PayloadNode` が存在するかを確認します。
存在しない場合は local-bookkeeping-only plan として
`Rejected { error = UnsupportedFeature, feature_error = None }`
を返します。
存在する場合はここで
`Rejected { error = UnsupportedFeature, feature_error = Some(SmtCertificate(RuleRegistryMismatch)) }`
...
または payload node table の canonical decode failure は
...
`payload_bindings` が存在するが node table 内の node に解決できない場合、または binding の
...
`TheoremGraphExtractorVersion` / `TheoremGraphFeatureSchemaVersion` / `TheoremGraphEdgeKind` canonical bytes は
variant tag だけで固定します。
MVP では `TheoremGraphExtractorVersion::MvpCertificateGraphV1` と
`TheoremGraphFeatureSchemaVersion::MvpGoalFeaturesV1` だけを受け付けます。
...
として拒否します。
query feature bytes 内の `env_fingerprint` / `goal_fingerprint` が request query と一致しない場合は
...
`optional_proof_candidate.candidate_statement_hash` の一致検査は、`Rejected` intent に proof candidate が付いていないことを
確認した後にだけ実行します。
`intent_record.status = Rejected` かつ `optional_proof_candidate = Some(...)` の場合は、
proof candidate 内の hash が一致するかどうかに関係なく
`Rejected { error = FeatureRejected, feature_error = Some(Formalization(RejectedIntentHasProofCandidate)) }`
として拒否します。
それ以外の proof candidate では、`optional_proof_candidate.candidate_statement_hash` は、この candidate の `statement` から再計算した
...
`options.formalization.tactic_options` 内の imported ref が envelope imports から一意に解決できない場合は
...
3. intent_record.status = Rejected かつ optional_proof_candidate = Some(...) の場合は RejectedIntentHasProofCandidate で拒否する
4. intent_record.status = Rejected かつ optional_proof_candidate = None の場合は candidate / intent hash 整合性だけを検査し、IntentRecordOnly で終了する
5. それ以外の場合は Phase 3 AI complete mode で canonicalize / elaborate / type check する
6. 必要なら人間が intent を確認する
7. proof candidate がある場合は candidate_statement_hash 一致を確認し、elaborated candidate theorem type に対して単一 Phase 4 tactic bridge として検査する
8. certificate には accepted core declaration と proof だけを入れる
```
...
EnvelopeMalformed:
  ...
  Formalization claim_span の byte range / UTF-8 boundary violation は feature_error = None。
  feature_error = Some(SmtCertificate(NonCanonicalPayload))
    ...
    command phase/order/id uniqueness/symbol table/sort validation、reconstruction plan の step id / imported_theory_refs
    canonical shape の違反。
  ...
  feature_error = Some(TheoremGraphQuery(QueryFeaturesMalformed))
    query feature bytes の nested decode、sort order、重複 key、value kind 違反。
...
ImportClosureMismatch:
  imported ref が envelope imports から一意に解決できない、または import closure が不足する。
  feature_error = None。

PayloadHashMismatch:
  artifact file_hash / size_bytes / embedded hash / referenced payload hash の再計算 mismatch。
...
FeatureRejected:
  ...
  QuotientConstruction:
    TargetRefMismatch, PrimitiveInterfaceMismatch, UniverseLevelMismatch
  SmtCertificate:
    ...
    RuleFingerprintMismatch と PayloadNode-specific の ReconstructionConclusionMismatch /
    ReconstructionPremiseMismatch は、非空 solver-native registry を持つ `SmtRuleRegistryProfile` variant でだけ到達する。
    PayloadBindingMismatch は MVP でも LocalBookkeeping step が payload_bindings を持つ場合に到達する。
    MVP empty registry では PayloadNode request は先に UnsupportedFeature + RuleRegistryMismatch になる。
  TheoremGraphQuery:
...
`QuotientConstruction.decl_certificate_hash` は envelope `target.target_decl_hash` と一致しなければなりません。
