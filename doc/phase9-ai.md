...
```text
0. Inline bytes acquisition, or Artifact path validation / file read / file_hash / size_bytes check
...
step 2 の common options canonical shape checks は schema version、field order、sort order、duplicate、protocol cap、
および scalar/range validation だけであり、MVP profile が semantic に受け付けない well-formed option 値の拒否は step 4 で行います。
...
`goal_fingerprint` が一致した後、kernel well-typedness check の前に `Phase9AiGoal` の universe context shape を検査します。
...
`Rejected { error = EnvelopeMalformed, feature_error = None }`
として拒否します。
同じ request に `goal_fingerprint` mismatch とこの universe context shape violation が同時にある場合は、byte-level binding check である
...
その unused options は `options_hash` と `env_fingerprint` だけに影響します。
...
feature-specific validation の最初に `inductives.len() == 1` を検査し、これを満たさない request は
...
`expected_decl_hash = Some(h)` の場合、validator は下の step 10 で `final_inductive_decl` から Phase 2 の通常規則で
...
```text
0. singleton family count check (`inductives.len() == 1`)
1. name uniqueness
...
7. MVP nested occurrence rejection consistency check
8. generated recursor generator/verifier invariant check
9. generated iota rule generator/verifier invariant check
10. expected_decl_hash binding check
```

上の順序は common envelope / options の canonical decode、sort order、重複検査が終わった後の
feature-specific validation 順序です。
step 0 で拒否された request では payload-local name 重複、constructor type、positivity、expected_decl_hash は検査しません。
step 1 で拒否された request では universe parameter well-formedness、constructor type、positivity、expected_decl_hash は検査しません。
step 2 で拒否された request では constructor type、positivity、expected_decl_hash は検査しません。
step 8 / 9 で artifact generator unavailable として拒否した request、または generated artifact invariant failure として
`Error::InternalValidatorFailure` になった request では、
step 10 の `expected_decl_hash` binding check へ進みません。
...
したがって step 7 は approved set で nested occurrence を許可する pass ではなく、
...
canonical decode と common target shape check の後、`target.target_decl_hash = Some` の request は
`goal` / `target_expr` / `instantiations` / `constraint_hints` の feature-specific validation へ進む前に上記 `UnsupportedFeature` で拒否します。
...
です。
solver が充足可能と判定しても、response に使う `repaired_expr` または `constraint_set_hash` がこの節の canonical rule から
byte-for-byte に再計算できない場合は
`Rejected { error = FeatureRejected, feature_error = Some(UniverseRepair(NonCanonicalSolution)) }`
です。
...
`Rejected { error = EnvelopeMalformed, feature_error = None }`
として拒否します。
この universe context shape check は operations sort order / duplicate name check の後、
...
step 2 の quotient_v1 primitive refs and eq ref public interface check は payload の `carrier` や導出後の `u` / `v` に依存しません。
...
実装が step 2 で payload-specific `carrier`、`relation`、`equivalence_proof`、または operation の `result_type` を先読みして
...
この順序は common envelope / options の canonical decode、sort order、重複検査、task target shape check、
および `options.quotient = Some(...)` の task options shape check が終わった後に適用します。

```text
0. operations sort order / duplicate name check
1. universe_params / payload-local LevelExpr universe context shape check
2. quotient_v1 primitive refs and eq ref import resolution and public interface check
3. params telescope type check
4. carrier type inference and carrier universe level derivation
5. relation type inference and carrier -> carrier -> Prop check
6. equivalence_proof kernel check against rel_equiv_type(carrier, relation)
7. quotient_type type inference and defeq check against quotient_type_app(setoid_expr)
8. quotient DefDecl reconstruction and expected_decl_hash binding check
9. operations validation in canonical name order
   9a. raw_function type inference and unary carrier -> result_type shape check
   9b. result_type Pi-head unsupported check
   9c. result_type universe level derivation
   9d. compatibility_proof kernel check
```

step 0 は import 解決や kernel check より先に実行し、違反した場合はこの節の他の feature-specific validation へ進みません。
step 1 も import 解決や kernel check より先に実行し、違反した場合は step 2 以降へ進みません。
step 8 は operations を検査する前に実行します。
...
step 9 では `operations` を payload の canonical name order で検査し、複数 operation に不整合がある場合は
...
ImportClosureMismatch:
  `options.quotient` の quotient_v1 primitive refs または `eq` ref が envelope imports から一意に解決できない。
...
`Rejected { error = EnvelopeMalformed, feature_error = Some(SmtCertificate(NonCanonicalPayload)) }`
で拒否します。
validator は `steps` の vector length prefix を element decode / allocation より前にこの cap と照合しなければなりません。
...
unused ref、重複 ref、sort order violation は
`Rejected { error = EnvelopeMalformed, feature_error = Some(SmtCertificate(NonCanonicalPayload)) }`
として拒否します。
...
`Rejected { error = ImportClosureMismatch, feature_error = None }`
として拒否します。
この reconstruction plan の step count cap / step id / premises / `final_step` / `imported_theory_refs` canonical shape、used-ref exactness、
import 解決、および LocalBookkeeping step が `payload_bindings` を持たないことの検査は、
...
encoded problem bytes validation は、Inline bytes acquisition または Artifact path/file/hash/size check の後、
...
`SmtCertificate` task-specific validation では、encoded problem の artifact/hash decode、
`goal_fingerprint` binding check、および logic mismatch check が終わった後、
...
非空 registry を持つ profile では、registry entry の `SmtRuleDescriptor` から再計算した `rule_fingerprint` と
payload 内の `rule_fingerprint` が一致しなければ
`Rejected { error = FeatureRejected, feature_error = Some(SmtCertificate(RuleFingerprintMismatch)) }`
として拒否します。
`MvpEmptyRegistryV1` の空 registry では、この mismatch 判定に到達せず `RuleRegistryMismatch` になります。
...
MVP の検査順序では、MachineSmtCertificateCandidate の outer canonical framing / enum tag / scalar field decode
（`reconstruction_plan.steps` の vector length prefix cap precheck を含み、cap 超過時は step element decode / allocation を行わない）、
encoded problem Inline/Artifact bytes acquisition and file_hash/outer framing and command/core-expression/expression/sort count cap/full decode/problem_hash / encoding_hash recomputation and comparison、
encoded problem の `goal_fingerprint` / logic binding check、`options.smt` import / public interface check、
command-level validation、proof payload artifact/file_hash/outer framing and node/nested CoreExpr/SmtExpr/SmtSortExpr count cap/payload_hash/full decode/table shape validation、
reconstruction plan の canonical shape / `final_step` bounds / used-ref exactness / import 解決、
LocalBookkeeping の pre-registry structural checks、の順に終えた後、
`reconstruction_plan.steps` に `PayloadNode` が存在するかを確認します。
...
deterministic encoder 再実行結果との一致、`core_expr` の type check は `SmtConclusionEncoding` semantic validation です。
...
registry に受理された後にだけ到達します。MVP empty registry では到達不能です。
...
`Rejected { error = KernelRejected, feature_error = None }` です。
`core_expr` から deterministic encoder を再実行した結果が `encoded_expr` と一致しない場合は
...
として拒否します。
非空 solver-native registry profile で `payload_bindings` が存在するが node table 内の node に解決できない場合、
または binding の
...
この順序は common envelope / options の canonical decode、sort order、task target shape check、
および `MachineTheoremGraphQuery` payload decode が終わった後に適用します。

```text
0. query target binding check
1. ranking_profile is MvpTupleOrder and limit is 0..=256
2. snapshot / query_features artifact bytes validation, vector length cap precheck, embedded hash check,
   and full canonical decode
3. snapshot source_release_hash / extractor_version metadata check
4. query_features env_fingerprint / goal_fingerprint / feature_schema_version metadata check
5. query feature key grammar / sort / duplicate / value kind check
6. snapshot node / edge sort / duplicate / edge target check
7. snapshot node import resolution, decl_certificate_hash / type_hash check, and eligibility classification
8. MvpTupleOrder result construction
```

step 0 で拒否される request では `limit`、artifact hash、snapshot/query feature bytes は検査しません。
step 1 で `limit > 256` の request は、snapshot / query_features artifact を読まずに
...
`ranking_profile` はこの schema では `MvpTupleOrder` 以外を canonical decode できないため、step 1 の unsupported profile branch はありません。
将来同じ schema version に既知だが未対応の ranking profile を追加する場合は、step 1 で
`Rejected { error = UnsupportedFeature, feature_error = None }`
...
step 2 の deterministic order は、各 ref について次で固定します。
step 2 全体では snapshot ref を先にこの順序で最後まで検査し、snapshot ref が成功した後にだけ
...
`Rejected { error = PayloadHashMismatch, feature_error = None }` です。
path validation を通過した artifact bytes を取得できない場合は `Error::ArtifactUnavailable` です。
step 1 の snapshot outer framing / cap precheck failure は
`Rejected { error = EnvelopeMalformed, feature_error = Some(TheoremGraphQuery(SnapshotMalformed)) }`、
...
`Rejected { error = PayloadHashMismatch, feature_error = None }`
です。
step 2 で embedded hash が一致しても step 3 の full canonical decode が失敗した場合、snapshot bytes では
`Rejected { error = EnvelopeMalformed, feature_error = Some(TheoremGraphQuery(SnapshotMalformed)) }`、
...
`Rejected { error = EnvelopeMalformed, feature_error = Some(TheoremGraphQuery(SnapshotMalformed)) }`
として拒否します。
step 7 では snapshot の全 `nodes` を canonical order で走査します。
...
`Rejected { error = EnvelopeMalformed, feature_error = None }`
として拒否します。
この check の正確な位置は下の Formalization deterministic validation order の step 2 です。
`MachineSurfaceTerm` wrapper shape check と、candidate の source document / claim span、
および current intent status で必要な rejection reason の artifact integrity、
...
したがって `MachineSurfaceTerm` shape violation、source/rejection artifact の hash mismatch、
invalid UTF-8、span violation、embedded hash mismatch は `ReviewerId` regex violation より優先し、
...
この順序は common envelope / options の canonical decode、sort order、重複検査、task target shape check、
および `options.formalization = Some(...)` の task options shape check が終わった後に適用します。

```text
0. candidate.statement MachineSurfaceTerm wrapper shape check
...
1. candidate.source_document / claim_span は intent_record の有無や status に関係なく常に
   artifact integrity、UTF-8 decode、span validation、embedded hash recomputation を検査する。
   rejection reason は intent_record.status = Rejected の場合だけ同じ artifact integrity、
   UTF-8 decode、embedded hash recomputation を検査する
2. ReviewerId regex validation
3. RejectedIntentHasProofCandidate check
4. intent_record source_document_hash / claim_span_hash / candidate_statement_hash / rejection_reason_hash consistency check
5. non-rejected status で必要な candidate statement complete-mode elaboration / type inference
6. optional proof bridge validation and certificate binding check
```

step 0 の `MachineSurfaceTerm` shape violation は、source / rejection artifact hash mismatch や
`ReviewerId` regex violation より優先します。
step 1 の source document / rejection reason artifact の hash mismatch、invalid UTF-8、span violation、embedded hash mismatch は
step 2 の `ReviewerId` regex violation より優先し、step 2 は step 3 以降の formalization feature-specific rejection より優先します。
`intent_record.status = Rejected` では step 5 / 6 を実行しません。

この節の artifact 参照は、まず実ファイル bytes に対して `Artifact.file_hash` / `size_bytes` を検査します。
...
`source_document_hash`、`claim_span_hash`、または `rejection_reason_hash` の再計算値が payload 内の hash と一致しない場合は
`Rejected { error = PayloadHashMismatch, feature_error = None }`
として拒否します。
...
validator は source document / rejection reason の artifact integrity、UTF-8 decode、span validation、
embedded hash 再計算、および `ReviewerId` regex validation をすべて終えた後、
`intent_record.source_document_hash` / `claim_span_hash` / `candidate_statement_hash` の一致検査より先に
`Rejected { error = FeatureRejected, feature_error = Some(Formalization(RejectedIntentHasProofCandidate)) }`
として拒否します。
したがって source document / rejection reason bytes の invalid UTF-8、span violation、
...
`source_document_hash` / `claim_span_hash` / `rejection_reason_hash` が、それぞれの ref または status field に埋め込まれた hash と
一致しない場合です。
intent_record の `source_document_hash` / `claim_span_hash` / `candidate_statement_hash` と candidate から再計算した値の
consistency mismatch は step 4 の `IntentRecordMismatch` 判定であり、この step 3 より前には返しません。
...
これは NaturalLanguageFormalization task の common options shape requirement であり、
...
および import 解決を必要としない scalar/range validation だけです。
...
2. MachineSurfaceTerm wrapper shape と Phase 3 Machine Surface term-source canonical decode を検査する
3. source document bytes と claim span range を hash 固定する
4. source document の artifact integrity、UTF-8、span、embedded hash を常に検査する
   rejection reason は intent_record.status = Rejected の場合だけ artifact integrity、UTF-8、embedded hash を検査する
...
EnvelopeMalformed:
  ...
  UniverseRepair の `instantiations` / `constraint_hints` の sort order violation または duplicate も feature_error = None。
  ...
  source/rejection reason の invalid UTF-8、claim_span の byte range / UTF-8 boundary violation、
  ...
  feature_error = Some(SmtCertificate(NonCanonicalPayload))
    ...
    reconstruction plan の step id / final_step / imported_theory_refs
    canonical shape の違反。
  feature_error = Some(TheoremGraphQuery(SnapshotMalformed))
    theorem graph snapshot bytes の nested decode、nodes / edges count cap、sort order、edge target 整合性の違反。
  feature_error = Some(TheoremGraphQuery(QueryFeaturesMalformed))
    ...
    key ASCII grammar、sort order、重複 key、value kind 違反。
...
PayloadHashMismatch:
  ...
  SMT encoded problem の problem_hash / encoding_hash mismatch と command_id recomputation mismatch もここに含む。
  ...
  intent_record と candidate の hash consistency mismatch は Formalization(IntentRecordMismatch) であり、ここには含めない。
  feature_error = None。
...
KernelRejected:
  ...
  SmtCertificate(ReconstructionProofMismatch) は再構成 proof bytes が step.proof と一致しない場合の FeatureRejected にだけ使う。
...
FeatureRejected:
  ...
  SmtCertificate:
    ...
    ConclusionEncodingMismatch、RuleFingerprintMismatch、ReconstructionProofMismatch、および PayloadNode-specific の
