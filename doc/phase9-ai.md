...
`Phase9AiProfileVersion` と `Phase9AiOptionsVersion` の canonical bytes は variant tag だけで固定します。
MVP ではどちらも `MvpV1` だけを受け付けます。
...
`Inline.options_hash` または `Artifact.options_hash` が再計算値と一致しない request は
`Rejected { error = PayloadHashMismatch, feature_error = None }`
として拒否します。
`Artifact.file_hash` / `size_bytes` が実ファイル bytes と一致しない場合も同じ分類です。
artifact bytes を取得できず hash / size を検査できない場合だけ `Error::ArtifactUnavailable` です。
...
`Phase9QuotientOptions` の各 primitive ref は、`QuotientConstruction` task で
...
`params` / `indices` / `result_sort` に block-local `GlobalRef::Local` が現れた場合は
...
step 8 / 9 で artifact generator unavailable または generated artifact mismatch として拒否した request では、
step 10 の `expected_decl_hash` binding check へ進みません。
したがって generated artifact rejection は `expected_decl_hash` mismatch より優先します。
...
Phase 2 generator と Phase 9 validator が同じ `base_inductive_decl_for_generation` / `profile_version` /
resolved public interfaces から異なる constructor spec、recursor spec、または iota rule spec canonical bytes を生成した場合は
`Rejected { error = FeatureRejected, feature_error = Some(AdvancedInductive(GeneratedArtifactMismatch)) }`
として拒否します。
...
- `max_nodes = 0` は candidate application attempt 禁止を意味し、head が一致する candidate を試す時点で `BudgetExceeded`
...
この時点では `current_depth < max_depth` をまだ検査しません。
`node_count` 条件を満たす場合、resolver は candidate application attempt を開始する直前に `node_count += 1` し、
...
`Rejected { error = EnvelopeMalformed, feature_error = None }`
として拒否済みです。
ここでいう重複 ref は、tuple key としては異なるが `resolve_imported_ref` 後に同じ `GlobalRef::Imported` になる ref だけです。
同じ `GlobalRef::Imported` に解決される重複 ref、または class declaration role として使えない public interface は
...
2. quotient primitive refs import resolution and public interface check
...
MVP では `u32::MAX` を valid step id として使わず、許可する step id は `0..u32::MAX` です。
したがって reconstruction plan の最大 step 数は `u32::MAX` 個であり、最大 valid `step_id` は `u32::MAX - 1` です。
`steps.len() > u32::MAX` の reconstruction plan は
...
`Rejected { error = ImportClosureMismatch, feature_error = None }`
として拒否します。
この reconstruction plan の step id / premises / `final_step` / `imported_theory_refs` canonical shape、used-ref exactness、
...
この proof payload bytes の取得、`Artifact.file_hash` / `size_bytes` 検査、`payload_hash` 再計算、
および `SmtProofNodeTable` canonical decode は、`options.smt` import / public interface check と
...
`payload_hash` mismatch または `Artifact.file_hash` / `size_bytes` mismatch は
...
MVP では `u32::MAX` を valid payload node id として使わず、許可する node id は `0..u32::MAX` です。
したがって payload node table の最大 node 数は `u32::MAX` 個であり、最大 valid `node_id` は `u32::MAX - 1` です。
`nodes.len() > u32::MAX` の payload は
`Rejected { error = EnvelopeMalformed, feature_error = Some(SmtCertificate(NonCanonicalPayload)) }`
として拒否します。
...
MVP の検査順序では、payload bytes の canonical decode、encoded problem artifact/hash/decode、
encoded problem の `goal_fingerprint` / logic binding check、`options.smt` import / public interface check、
command-level validation、proof payload artifact/file_hash/payload_hash/decode/table shape validation、
...
ここでいう proof payload table shape validation は、`SmtProofNodeTable` の canonical decode、certificate_format、node order /
contiguity、premise reference、acyclicity、および `SmtConclusionEncoding` 内の nested `CoreExpr` / `SmtExpr` を
canonical schema として decode できることまでです。
...
2. snapshot / query_features artifact file_hash / size_bytes / canonical decode / embedded hash check
...
7. eligible node import resolution and decl_certificate_hash / type_hash check
...
解決はできたが snapshot node の `decl_certificate_hash` または `type_hash` が export table から再計算した値と一致しない場合は
...
`Rejected { error = EnvelopeMalformed, feature_error = None }`
として拒否します。
この check は、現在の intent status で必要な source document / rejection reason の artifact integrity、
...
したがって source/rejection artifact の hash mismatch、invalid UTF-8、span violation、embedded hash mismatch は
`ReviewerId` regex violation より優先し、`ReviewerId` regex violation は
formalization feature-specific rejection より優先します。
...
`source_document_hash` / `claim_span_hash` / `rejection_reason_hash` mismatch、artifact integrity mismatch、
...
`options.formalization.tactic_budget` は canonical decode と `options_hash` / `env_fingerprint` の入力にだけ使います。
...
3. source document の artifact integrity、UTF-8、span、embedded hash を常に検査する
...
4. intent_record.status = Rejected かつ optional_proof_candidate = Some(...) の場合は RejectedIntentHasProofCandidate で拒否する
5. intent_record.status = Rejected かつ optional_proof_candidate = None の場合は candidate / intent hash 整合性だけを検査し、IntentRecordOnly で終了する
6. それ以外の場合は Phase 3 AI complete mode で canonicalize / elaborate / type check し、失敗した場合は CandidateStatementElaborationFailed で拒否する
7. 必要なら人間が intent を確認する
8. proof candidate がある場合は elaboration 成功後に candidate_statement_hash 一致を確認し、elaborated candidate theorem type に対して単一 Phase 4 tactic bridge として検査する
9. certificate には accepted core declaration と proof だけを入れる
```
...
EnvelopeMalformed:
  ...
  TypeclassResolution の `ordered_candidates` duplicate target も feature_error = None。
  Formalization source/rejection reason の invalid UTF-8、claim_span の byte range / UTF-8 boundary violation、
  ReviewerId regex violation は feature_error = None。
  feature_error = Some(SmtCertificate(NonCanonicalPayload))
    SMT payload table の nested decode、certificate_format、node canonical order、acyclicity、payload table shape、
    proof payload `SmtConclusionEncoding.encoded_expr` の representation-only canonical shape、
    command phase/order/id uniqueness/symbol table/sort validation、reconstruction plan の step id / final_step / imported_theory_refs
    canonical shape の違反。
  feature_error = Some(TheoremGraphQuery(SnapshotMalformed))
    theorem graph snapshot bytes の nested decode、sort order、edge target 整合性の違反。
  feature_error = Some(TheoremGraphQuery(QueryFeaturesMalformed))
    query feature bytes の nested decode、feature_schema_version / target metadata mismatch、
...
PayloadHashMismatch:
  ...
  SMT command_id recomputation mismatch もここに含む。
  Formalization source_document_hash / claim_span_hash / rejection_reason_hash の再計算 mismatch もここに含む。
  feature_error = None。
