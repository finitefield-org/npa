...
QuotientConstruction:
  ...
  `quotient_v1` certificate adoption / success path は将来の quotient-capable independent checker profile が
  `Phase9IndependentCheckerProfile` に追加されるまで non-MVP である。
  ...
  将来の quotient-capable profile では、`quotient_v1` primitive refs / `eq` public interface mismatch は checker support ではなく
...
以下でいう quotient profile の MVP は、将来 `Phase9IndependentCheckerProfile` に quotient-capable profile を追加した後の
`quotient_v1` 内の最小仕様を指し、core v0.1 MVP に quotient を含める意味ではありません。
...
この採用条件は quotient-capable independent checker profile を選べる将来 profile 用です。
...
quotient success path は、quotient-capable independent checker profile を別 schema / profile で定義してから有効化します。
...
schema です。現行 `Phase8MvpReference` profile では到達不能であり、`/machine/phase9/quotient/check` はこの variant を返しません。
...
`QuotientConstruction` は current `Phase8MvpReference` では success path がないため checker 実行に到達しません。
future schema / profile で quotient-capable independent checker による success path を有効化する場合は、
quotient type `DefDecl` certificate package を構成し、success を返す前に Phase 8 independent checker へ渡します。
