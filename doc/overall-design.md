...
```json
{
  ...
  "theorem": "Group.mul_inv_cancel",
  "statement_core_hash": "sha256:...",
  "proof_core_hash": "sha256:...",
  "imports": [
    {
...
`source_script`、`kernel_version`、`checked_by` などの説明・監査用 metadata は canonical payload や certificate hash には含めません。
...
```text
- Prop は proof-irrelevant
- Type 階層は predicative
- universe polymorphism を持つ
- impredicative Prop は採用候補
```
...
| Pi  Name BinderInfo Type Body
| Let Name Type Value Body
| Inductive Name [Level]
| Constructor Name [Level]
| Recursor Name [Level]
| Match Scrutinee Motive Branches
| MetaFreeHoleは禁止
```
...
- recursive definitionsはtermination certificateつきで受理する
...
- kernelには recursor / well-founded recursion / termination certificate として渡す
```
