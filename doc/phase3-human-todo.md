...
- [ ] `HumanDiagnosticKind` を MachineDiagnostic と分ける。
- [ ] parser / resolver / elaborator / kernel handoff の phase を payload に含める。
- [ ] AmbiguousName / AmbiguousNotation / NotationConflict / ForwardReference /
  NamedHoleContextMismatch / UnsolvedMeta を表現する。
- [ ] hole goal display 用の local context / expected type を payload に入れる。
- [ ] human-facing message は deterministic payload から生成する。

AI 速度ガード:

- [ ] MachineDiagnostic の canonicalization と Phase 7 failed candidate error payload を変えない。
- [ ] Human diagnostic message の変更を Machine M9 same-output 対象にしない。
