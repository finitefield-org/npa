...
private dependency name は certificate context 内に保持できますが、後続 endpoint がそれを Machine Surface source として
表示する必要がある場合は `InvalidMachineProofState`、theorem index entry として表示する必要がある場合は
`InvalidTheoremIndex` です。

この文書の Phase 5 canonical bytes block は、別途明記しない限り次の primitive encoding を使います。
Prompt payload、session root、theorem index、query、filters、batch policy、diagnostic は同じ primitive を共有します。
...
`eq_family` / `nat_family` は Phase 4 AI の coherent family validation を通した後の resolved family bytes を
...
SessionRoot canonical bytes:
  ...
  - MachineApiOptions canonical bytes:
      kernel_check_profile canonical hash
      allow_axioms as MachineAxiomRefWire canonical bytes in canonical order
...
ここでいう resolved family option bytes は Phase 4 `MachineTacticEnv resolved family bytes` と
byte-for-byte 同じです。`eq_family` / `nat_family` が `null` の場合も field を省略せず、
canonical bytes へ `none` tag を encode します。
...
hash 風 field、checked field、metadata、score は `candidate` の内側に入れてはいけません。
...
`candidate` object 内の unknown field、invalid variant、hash 風 metadata、`RawMachineTerm` wire object shape violation、
...
schema violation、hash 風 metadata、`RawMachineTerm` object shape violation は pre-canonical failure であり、
...
TheoremIndexFingerprint canonical bytes:
  ...
  - entries in canonical order:
      global_ref.module
      global_ref.name
      global_ref.export_hash
      global_ref.decl_interface_hash
      universe_params in ExportEntry order
      statement core hash
      head symbol option as MachineGlobalRefView canonical bytes, or none tag
      axiom dependencies canonical hash
...
`theorem_index_schema_version` MVP value は `"mvp-export-entry-v3-visible-heads-universe-params"` です。
...
`MachineUniverseParamName` として valid でない universe parameter name を持つ public export entry は
`InvalidTheoremIndex` です。
...
match score を計算します。
match score は `search_score_key_fingerprint` と theorem index entry canonical bytes だけの関数です。
...
unknown `kind`、variant field set mismatch、hash 風 metadata、
...
```text
1. doc split
   phase5-human.md と phase5-ai.md を分ける

2. Machine API types
   MachineProofSession / MachineProofSnapshot / MachineGoalView を定義する

3. Snapshot fingerprint
   state_fingerprint 由来の snapshot_id と deterministic serialization を固定する

4. /machine/sessions
   verified imports と MachineProofSpec から初期 snapshot を作る
...
8. Machine diagnostics
   Phase 3 AI / Phase 4 AI の error を MachineApiErrorKind に写す

9. /machine/search/for_goal
