...
SHA-256 実装は次のどちらかにします。

```text
preferred:
  pinned, audited OCaml SHA-256 library

high-trust hardening:
  small vendored SHA-256 implementation with golden vector tests
```

どちらの場合も dependency version と build hash を checker identity manifest に固定します。
...
feature gate を増やす場合は、fast kernel、reference checker、external checker の3者で golden corpus を追加します。
...
```text
- OCaml project をこの repository に置くか、別 repository に置くか
- SHA-256 を pinned library にするか vendored implementation にするか
- first release で quotient feature profile を拒否するか部分対応するか
- checker identity manifest の署名をいつ導入するか
