...
このタスク分解は `doc/phase6-human.md` を正とし、現在の
...
`doc/phase6-ai.md` の責務であり、Human source layout、notation、pretty statement、Human-facing 属性表を
...
`doc/phase6-human.md` が要求する以下の範囲は、現在のコードには standard-library source package として存在しません。
...
```text
- Std.Logic / Std.Nat / Std.List / Std.Algebra.Basic source が存在する。
