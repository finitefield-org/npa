...
`RepairChainStopReason` の JSON wire name は `repeated_error`、`max_repair_depth` です。
`record_repair_chain_stop(node, goal_id, envelope, failure, reason)` は
...
`initial_snapshot.snapshot_id`、`current_plan.initial_state_fingerprint` を渡します。
