...
`record_training_trace_batch` は training trace artifact を保存する関数であり、`trace_events` には書き込みません。
`trace_events` は `Phase7VerifiedProof` と `Phase7SearchFailure` の両方で返し、
training trace JSON とは別 artifact として保存します。
...
error item の `error_kind` が missing / non-string / Phase 5 `FailedCandidateErrorKind` 外の場合も、
raw に読める wire string があれば `error_kind = Some(raw_string)`、読めない場合は `None` にします。
            ...
            for item in result.results:
                if item.status == "success":
                    ...
                    if repeated_repair_error(envelope, failure):
                        record_repair_chain_stop(node, goal.goal_id, envelope, failure)
                        continue
...
`item.candidate_hash = actual_hash` を返した場合、`expected_hash == actual_hash` でなければなりません。
不一致は candidate failure ではなく Phase 5 integration / controller consistency error です。
...
`envelope.candidate_hash = Some(expected_hash)` かつ `item.candidate_hash = Some(actual_hash)` かつ
`expected_hash != actual_hash` の場合だけ true です。
...
per-candidate error item でも、`candidate_hash` / `diagnostic_hash` のどちらかがないもの、または
