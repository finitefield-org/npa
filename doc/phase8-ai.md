...
normalizer はこの section で `NormalizedCheckResult.results[]` entry へ copy すると定義した
field を、source `MachineCheckResult` artifact から直接生成します。
そのため、source artifact field が存在するのに生成した normalized entry へ写されていない場合、
または値が一致しない場合は normalizer implementation invariant violation であり、
user input 起因の `NormalizeErrorResult.error.reason_code` には割り当てません。
実装は partial / invalid `NormalizedCheckResult` を書かず、生成関数の self-check または
単体テストでこの invariant を検査します。
...
`error.field` は不一致した normalized entry member path、
`expected_hash` / `expected_value` は source `MachineCheckResult` の値、
`actual_hash` / `actual_value` は saved `NormalizedCheckResult` の値にします。
...
つまり `required = true` は failed / non-success CI diagnostic target の説明 artifact 完全性を
gate する設定であり、pass している release audit bundle に追加の required sidecar を要求する設定ではありません。
...
ただし下流 artifact kind の expected set または optional response binding が
別 artifact kind の内容から導出される場合、
...
response-binding failure を作る場面だけに適用します。
...
- CI diagnostic required machine_result sidecar sources resolve through the selected MachineCheckResult artifact run_artifact_hash using checker_profile, result_hash, and request_hash
...
- NormalizedCheckResult source-copy mismatches are normalizer invariants, not NormalizeErrorResult reason codes, and release validate-bundle reports saved copy mismatches as audit_bundle_invalid
