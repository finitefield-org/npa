...
MVP で Phase 2 `Name` として decode しない `mutation.target` は
`"$whole_certificate"` だけです。
...
`mutation_target_invalid` は name grammar が valid な target が対象 certificate に存在しない場合、
または mutation kind と target class が合わない場合に使います。
...
challenge_manifest:
  ...
  challenge_manifest set is prerequisite-clean iff:
    ...
    - every included ChallengeManifest is manifest-local JSON / schema / domain valid
      enough to extract challenge_id, policy_hash, mutation.kind, and imports.manifest_hash,
