...
checker_allowlist.checker_id grammar violation:
  field = "checker_allowlist[].checker_id"
  expected_value = "checker_id"
  actual_value = "invalid_name_format"
...
checker_allowlist.allowed_args が static option rule に違反:
  field = "checker_allowlist[].allowed_args[]"
...
stderr:
  copied only into diagnostics after redaction when the exit-code rule for that case permits
  diagnostics; for exit >= 3 diagnostics is omitted; never used as verdict
```
...
`mutated_certificate.claimed_certificate_hash` は raw claimed-hash extractor が
...
`mutated_certificate.claimed_certificate_hash` を required として書きます。
...
claimed_certificate_hash:
  ...
  mutated_certificate では、raw claimed-hash extractor が
  `ModuleHashes.certificate_hash` field byte range / value byte range を exactly one として識別できる場合だけ required。
...
baseline / repeated result store entry manifest-field mismatch:
  ...
  expected_hash / expected_value = result store entry field value
  actual_hash / actual_value = parsed MachineCheckResult same field value
...
`actual_value` に `missing`、`wrong_type`、または `null_not_allowed` を入れます。
top-level `schema` mismatch では `field = "schema"`、
`expected_value = "npa.phase8.release_bundle_staging_plan.v1"`、
`actual_value` に実際の schema 文字列を入れます。
...
完全一致する entry に限り deduplicate してよいです。
...
plan schema / order / duplicate / path violation:
  ...
  actual_value = missing | wrong_type | null_not_allowed | unknown_field |
                 ...
                 duplicate_path | non_empty | actual invalid value
...
repeatable input occurrence を持たないため、同じ `challenge_replay_result[<i>]` field shape を
...
`kind = challenge_replay_result` entry だけの 0-based occurrence index です。
...
`failure_key_mismatch` のいずれかを入れます。
...
`actual_value` に `missing`、`wrong_type`、`unknown_field`、
`invalid_enum`、`invalid_hash_format`、`invalid_name_format`、
`null_not_allowed`、`order_violation`、`duplicate_field`、`failure_key_mismatch` のいずれかを入れます。
...
- challenge structured mutation canonical byte offsets are measured in canonical re-encoded ModuleCertBytes, not original file layout
...
- ReleaseAuditBundleManifest bundle-local manifest merge deduplicates only entries with identical parsed artifact hash and source file bytes and rejects same-key or same-path conflicts
...
- release validate-bundle uses the materialized ReleaseAuditBundleManifest deterministic kind-specific artifact entry occurrence index for challenge_replay_result[<i>] field shapes
