...
12. AiSidecarDiagnosticResult
   required AI sidecar diagnostic target の対応付け gate 結果。
   保存正本 artifact ではなく result_hash を持たない。

13. AiSidecarDiagnosticEvaluationFailure
   required AI sidecar diagnostic gate の target context / explicit input / saved diagnostic result validation failure。
   保存正本 artifact ではなく result_hash を持たない。

14. NormalizationWriteResult
   normalize-results が `--out` 指定時に返す書き込み summary。
   保存正本 artifact ではなく result_hash を持たない。

15. ChallengeRequestMaterializationResult
   challenge replay request materialization の書き込み summary。
   保存正本 artifact ではなく result_hash を持たない。

16. ChallengeGenerationResult
   challenge generation が ChallengeManifest、mutated certificate、challenge output store を
   書き込んだ summary。
   保存正本 artifact ではなく result_hash を持たない。

17. ReleaseBundleStagingResult
...
`result_hash` 用の projection は、`MachineCheckResult` object に対して
ad hoc に recursive field removal を行ったものではなく、次の固定 projection object です。
object member order は 3.3 の canonical JSON rule に従い、optional field が存在しない場合は
projection にも挿入せず、`null` で補ってはいけません。

```text
result_hash_projection:
  - schema
  - policy            # MachineCheckResult.policy object をそのまま copy
  - runner            # MachineCheckResult.runner object をそのまま copy
  - checker:
      required:
        - profile
      copy if present:
  ...
  - status
  - module
  - error, if present # MachineCheckResult.error object をそのまま copy
  - certificate_hash, if present
  - export_hash, if present
  - axiom_report_hash, if present
```

`policy`、`runner`、`error` はそれぞれの schema で closed-world object として検査済みの値を
そのまま projection に copy します。
`checker` だけは audit / display metadata である `version` を除くため、上の fixed nested projection を使います。
将来 `policy`、`runner`、`error` に `result_hash` から除外すべき metadata field を追加する場合は、
この projection schema と `MachineCheckResult` schema version の両方を更新します。
...
unknown field 名を `schema_path_component` で表現できない場合は raw key を copy せず、
containing object path を使います。
top-level object では standalone loader の `field = "$"`、`AuxiliaryResult.error` へ写す場合は
`field = "axiom_report"`、`axioms[]` entry では `field = "axioms[<i>]"` または
`field = "axiom_report.axioms[<i>]"` です。
...
schema validator が unknown field または unknown field の duplicate key を `field` に含める場合、
raw JSON object key は次の component grammar に一致するときだけ literal component として copy できます。
一致しない raw key を escaping して canonical artifact に入れてはいけません。
一致しない場合は、その validator が定義する containing object path を `field` に使い、
`expected_value` / `actual_value` で unknown / duplicate を表します。
この grammar は raw object key 用であり、`budgets.<profile>` のような domain-specific typed key slot、
固定 hash field name、固定 enum field name には適用しません。

```text
schema_path_component:
  [A-Za-z_][A-Za-z0-9_]{0,127}

schema_path_index:
  "[" non_negative_i64_decimal_without_plus_sign "]"

non_negative_i64_decimal_without_plus_sign:
  "0" | [1-9][0-9]*
  numeric value must be <= 9223372036854775807
```

この文書で schema / manifest validator の `JSON path` と書く場合は、
明示された fixed path、または `schema_path_component` と `schema_path_index` だけで表現できる path を指します。
array index は 0-based decimal で、`0` 以外の leading zero を禁止します。
raw key を表現できないときの containing object path は、各 validator の field shape 節で固定します。
...
unknown field 名を `schema_path_component` で表現できない場合は raw key を copy せず、
containing object path を使います。
`request_import_manifest_invalid` へ写す場合の containing object path は
`imports.manifest`、`imports.manifest.imports[<i>]`、または
`imports.manifest.imports[<i>].certificate` です。
...
full schema / domain validation では `request_id` の human reference id grammar、
`request_hash` の `sha256:<lower-hex>` 形式、
...
`MachineCheckRequest.request_id` は non-empty visible ASCII string で、bytes `0x21`-`0x7E` に限定し、
space、control character、non-ASCII character、empty string を禁止します。
これは `MachineCheckResult.request_id` / `MachineCheckResult.result_id` と同じ grammar です。
`request_id` の missing / null / wrong type / empty string / invalid format は
`request_schema_invalid` とし、`error.field = "request_id"`、
`expected_value = "request_id"`、
`actual_value = "missing" | "null_not_allowed" | "wrong_type" |
"empty_string" | "invalid_string_format"` にします。
...
`request_hash_missing` は full schema / domain validation の前の dedicated pre-check であり、
`request_id` の schema failure と同時に成立する場合も `request_hash_missing` を先に返します。
`request_hash` が present だが malformed な場合は full schema / domain validation に含め、
schema field order により `request_id` failure を `request_hash` failure より先に返します。
...
`non_positive_integer`、`integer_out_of_range`、`duplicate_field`、`empty_string`、
`invalid_string_format` のいずれかを入れます。
unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合は、
raw key を copy せず MachineCheckRequest artifact-local containing object path を使います。
containing object path は root object では `field = "$"`、nested object では
`field = "policy"`、`field = "certificate"`、`field = "imports"`、または
`field = "budget"` です。
known field の duplicate key はその known field の JSON path を使います。
...
request_import_manifest_invalid:
  ...
  schema / domain validation failure:
    field = "imports.manifest" for root-level failure,
            ...
            using the final field fallback rule in section 5 when a raw key cannot be represented
...
checker_identity_mismatch:
  ...
  If pre-launch checker_id and binary_id both mismatch, report checker_id first.
...
ただし unknown field name を section 5 の `delegated_schema_path_component` で表現できない場合は
`error.field = "policy"` にします。
`expected_value` に schema requirement 名、
`actual_value` に `missing`、`wrong_type`、`unknown_field`、`invalid_enum`、
`invalid_path`、`invalid_hash_format`、`null_not_allowed`、`order_violation`、
`duplicate_field` のいずれかを入れます。
複数の member-level failure が同時に存在する場合の優先順位は、
4.1 の `RunnerPolicyReference` schema / domain validation priority に従います。
...
`RunnerPolicyReference` schema / domain validation priority は、
この reference を受け取るすべての caller で次に固定します。
複数の violation が同時に存在する場合は、最初に該当したものだけを返します。

1. reference object 自体の missing / wrong type / explicit null。
   `field = "policy"`、`expected_value = "RunnerPolicyReference"`、
   `actual_value = missing | wrong_type | null_not_allowed`。
2. known member の duplicate。schema order は `kind`、`path`、`hash`。
   `field = "policy.<member>"`、`expected_value = "unique_object_keys"`、
   `actual_value = "duplicate_field"`。
3. `kind` の missing / wrong type / explicit null / invalid enum / order violation。
   `field = "policy.kind"`、`expected_value = "file"`。
4. `path` の missing / wrong type / explicit null / invalid path / order violation。
   `field = "policy.path"`、`expected_value = "workspace_relative_path"`。
5. `hash` の missing / wrong type / explicit null / invalid hash format / order violation。
   `field = "policy.hash"`、`expected_value = "sha256:<lower-hex>"`。
6. unknown member。複数ある場合は raw member name の bytewise order で最小のものを返します。
   unknown field name を caller の `schema_path_component` rule で表現できる場合は
   `field = "policy.<unknown_field_name>"`、表現できない場合は `field = "policy"` にします。
   unknown member の duplicate object key はこの unknown member priority 位置で報告し、
   `expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` にします。
   duplicate ではない unknown member では
   `expected_value = "absent"`、`actual_value = "unknown_field"` にします。

member-level failure の `actual_value` は、該当 violation に応じて
`missing`、`wrong_type`、`null_not_allowed`、`invalid_enum`、`invalid_path`、
`invalid_hash_format`、`order_violation`、`duplicate_field`、`unknown_field`
のいずれかにします。
CLI で `--policy` と `--policy-hash` の両方が存在し、path と hash が同時に malformed な場合は、
上の priority により `policy.path` を先に返します。
...
RunnerPolicy-local schema JSON path に unknown field または unknown field の duplicate key を含める場合は、
3 の `schema_path_component` rule を適用します。
raw key を表現できない場合の local containing object path は、top-level object では `$`、
`checker_allowlist[]` entry では `checker_allowlist[<i>]`、
`checker_identity_manifest`、`import_policy`、`axiom_policy`、
`budgets` object では `budgets`、budget entry object では `budgets.<profile>` です。
budget entry object の `<profile>` は typed key slot なので checker profile name grammar に従います。
invalid budget key 自体を報告する場合は下の `budget_key_path_component` rule を使います。
caller prefix rule では local containing object path も同じように変換します。
...
profile name grammar violation in budgets key:
  field = "budgets.<invalid_profile_key>" when the invalid key matches budget_key_path_component,
          otherwise "budgets"
  expected_value = "checker_profile_name"
  actual_value = "invalid_name_format"
...
invalid key を選び、その key が `budget_key_path_component` に一致する場合だけ
`budgets.<invalid_profile_key>` に入れます。一致しない場合は `field = "budgets"` にします。
`budget_key_path_component` は field path reporting 専用の grammar であり、profile key の
validity は checker profile name grammar だけで判定します。

```text
budget_key_path_component:
  [A-Za-z0-9_-]{1,64}
```
...
unknown field 名を `schema_path_component` で表現できない場合は raw key を copy せず、
上で定義した RunnerPolicy-local containing object path を使います。
...
5. checker identity manifest entry の checker_id / binary_id をこの順で照合する
...
`checker_raw` を使います。
この `CheckerRawResult JSON path` は status reason の copied field としても使うため、
次の grammar に一致する path だけを canonical artifact に書けます。

```text
raw_schema_field_path:
  raw_schema_path_component raw_schema_path_index?
  ("." raw_schema_path_component raw_schema_path_index?)*

raw_schema_path_component:
  [A-Za-z_][A-Za-z0-9_]{0,127}

raw_schema_path_index:
  "[" non_negative_i64_decimal_without_plus_sign "]"
```

したがって `checker_raw.schema`、`checker_raw.error.kind`、
`checker_raw.error.core_path[0]` は valid field path です。
unknown field 名や duplicate field 名がこの component grammar に一致しない場合は、
その raw key を field path に copy してはいけません。
top-level object の場合は `field = "checker_raw"`、`error` object 内の場合は
`field = "checker_raw.error"` のように containing object path を使い、
`expected_value` / `actual_value` で unknown / duplicate を表します。
...
unknown field 名を `raw_schema_path_component` で表現できない場合は、上で定義した
`checker_raw` / `checker_raw.error` containing object path を使います。
...
`checker.id` と `checker.build_hash` は `MachineCheckResult` schema 上は独立 optional copy field です。
runner は、それぞれ対応する raw identity field を syntactically valid な値として読めた場合だけ
`MachineCheckResult.checker.id` / `checker.build_hash` に copy し、読めない field を補完してはいけません。
normalizer は raw checker output を読まず、存在する `MachineCheckResult.checker.id` /
`checker.build_hash` だけを `NormalizedCheckResult.results[].checker_id` /
`checker_build_hash` へ copy します。
保存済み `MachineCheckResult` の schema validator は raw checker output を入力に持たないため、
`checker.id` / `checker.build_hash` の欠落だけで `machine_result_schema_invalid` にしてはいけません。
`checker.version` は optional metadata であり、raw `checker_version` が valid string の場合だけ記録します。
`process.launched = true` で accepted checker verdict を採用するには
`checker.id` と `checker.build_hash` の両方が必要です。
欠落または policy mismatch は runner が生成する `policy_failure`、
または `NormalizedCheckResult.comparison` の comparison-generated `policy_failure` であり、
`MachineCheckResult` 単体の schema failure ではありません。
...
`MachineCheckResult.result_id` は human reference id です。
値は non-empty visible ASCII string で、bytes `0x21`-`0x7E` に限定し、
space、control character、non-ASCII character、empty string を禁止します。
この grammar は `AiAuditSidecar.source.result_id` と同一です。
normalizer input validation で `MachineCheckResult` 単体を検査する場合の artifact-local field は
`result_id` です。
`NormalizeErrorResult` に埋め込む場合は caller-prefix rule により、CLI / API とも
`machine_results[].result_id` を使います。
missing / null / wrong type / empty string / invalid format は
`machine_result_schema_invalid` とし、`expected_value = "result_id"`、
`actual_value = "missing" | "null_not_allowed" | "wrong_type" |
"empty_string" | "invalid_string_format"` にします。
`axioms_used` は optional array of Phase 2 `AxiomName` dotted JSON representation です。
runner / checker が deterministic used axiom summary を得た場合だけ書き、得られない場合は omit します。
known empty set を表す場合は empty array を書いてよいです。
`axioms_used` を書く場合、decoded `AxiomName` の canonical name order で昇順に並べ、
重複を禁止します。
`declarations_checked` は optional non-negative i64 integer です。
runner / checker が deterministic checked declaration count を得た場合だけ書き、得られない場合は omit します。
`axioms_used` の wrong type / null、element wrong type / null / name grammar violation、
order violation、duplicate axiom name、および `declarations_checked` の wrong type / null /
integer out of range は `machine_result_schema_invalid` です。
field はそれぞれ `axioms_used`、`axioms_used[<i>]`、または `declarations_checked` とし、
duplicate axiom name の `actual_value` は `duplicate_axiom_name`、
negative `declarations_checked` と i64 overflow は `integer_out_of_range` にします。
...
step 3 として次で固定します。
step 2 の top-level schema validation で `machine_result_wrong_schema` になる failure はこの table に入りません。
ただし duplicate top-level `schema` は `machine_result_schema_invalid` であり、item 1 の
duplicate top-level known field として扱います。
複数の violation が同時に存在する場合は最初の1件だけを返します。

```text
1. duplicate top-level known field, in top-level known field order
2. top-level direct required member presence / null / type / value shape, in top-level direct field order
3. policy object member schema / domain validation, using RunnerPolicyReference priority
4. runner object member schema / domain validation
5. process object member schema / domain validation
6. checker object member schema / domain validation, using validated process.launched for conditional fields
7. resource_usage object member schema / domain validation
8. status = checked conditional required / forbidden fields, in the order
   certificate_hash, export_hash, axiom_report_hash, error
9. status = failed error member presence / null / type, then nested error schema / domain validation
10. status = failed certificate_hash / export_hash / axiom_report_hash
    required / optional / forbidden / value shape, in that field order and by error.kind rule
11. optional metadata schema / domain validation, in the order
    diagnostics, axioms_used, declarations_checked
12. duplicate unknown top-level field, then unknown top-level field,
    by bytewise field name order within each group
```

top-level known field order は `schema`、`request_id`、`request_hash`、`result_id`、
`result_hash`、`run_artifact_hash`、`policy`、`runner`、`checker`、`attempt`、
`status`、`module`、`process`、`resource_usage`、`error`、`certificate_hash`、
`export_hash`、`axiom_report_hash`、`diagnostics`、`axioms_used`、
`declarations_checked` です。
top-level direct field order は `request_id`、`request_hash`、`result_id`、
`result_hash`、`run_artifact_hash`、`policy`、`runner`、`checker`、`attempt`、
`status`、`module`、`process`、`resource_usage` です。
item 2 の `policy` / `runner` / `checker` / `process` / `resource_usage` は
missing / null / object type だけを検査し、object 内 member validation は item 3-7 で行います。
`MachineCheckResult.request_id` は元の `MachineCheckRequest.request_id` を写す
human reference id で、`MachineCheckResult.result_id` と同じ non-empty visible ASCII string grammar を使います。
top-level direct member schema failure の field shape は次で固定します。

```text
request_id / result_id missing / null / type / empty / grammar:
  field = "request_id" | "result_id"
  expected_value = "request_id" | "result_id"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" |
                 "empty_string" | "invalid_string_format"

request_hash / result_hash / run_artifact_hash missing / null / type / hash format:
  field = "<hash_member>"
  expected_value = "sha256:<lower-hex>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_hash_format"

attempt missing / null / type / non-positive / range:
  field = "attempt"
  expected_value = "positive_i64"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" |
                 "non_positive_integer" | "integer_out_of_range"

status missing / null / type / enum:
  field = "status"
  expected_value = "MachineCheckResult.status"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

module missing / null / type / grammar:
  field = "module"
  expected_value = "module_name"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_name_format"

policy / runner / checker / process / resource_usage missing / null / type:
  field = "<object_member>"
  expected_value = "object"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"
```

`policy` は object type を通過した後の 4.1 `RunnerPolicyReference` priority に従います。
`runner` field order は `id`、`version`、`build_hash` です。
`process` field order は `launched`、`exit_code`、`termination_reason` です。
`checker` field order は `profile`、`binary_id`、`binary_hash`、`id`、`build_hash`、`version` です。
checker の `process.launched` dependent required / forbidden rule は、item 5 で
`process.launched` が schema-valid boolean として確定した後だけ item 6 で評価します。
`resource_usage` field order は `steps`、`memory_peak_mb`、`elapsed_ms` です。
runner / process / checker / resource_usage の object 内 priority は、duplicate known member、
required / conditional member shape、forbidden member、allowed optional member shape、
duplicate unknown member、unknown member の順です。
duplicate known member は上の field order、duplicate unknown member と unknown member は
bytewise field name order で最初の member を報告します。
duplicate unknown member がない場合だけ non-duplicate unknown member を報告します。
duplicate known / unknown member では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。
non-duplicate unknown member では `expected_value = "absent"`、
`actual_value = "unknown_field"` を使います。
top-level duplicate unknown field は item 12 で扱い、status-dependent field validation と
optional metadata validation より後に報告します。

`MachineCheckResult` artifact-local JSON path の root marker は `$` です。
schema validation で unknown field または duplicate unknown field 名を
`schema_path_component` で表現できない場合は、raw key を copy せず
artifact-local containing object path を使います。
`MachineCheckResult` の containing object path は `$`、`policy`、`runner`、`checker`、
`process`、`resource_usage`、または `error` です。
`policy` object 内の raw unknown key は `RunnerPolicyReference` の
`delegated_schema_path_component` rule で表現できる場合だけ `policy.<unknown>` とし、
表現できない場合は `policy` を使います。
caller error body が `MachineCheckResult` artifact に prefix を付けて報告する場合、
root marker `$` は caller root field に置換し、root 以外の artifact-local path は
caller root field に suffix として付けます。
たとえば normalizer input の local `$` は `machine_results[]`、local `runner` は
`machine_results[].runner` です。store entry artifact validation では local `$` は
`result_store.results[]`、local `checker` は `result_store.results[].checker` です。
...
policy_failure:
  ...
  reason_code-specific error fields: see the table below
...
`policy_failure` の reason_code-specific `error` member rule は次で固定します。
ここに明記されていない `error` member は forbidden です。
各 reason_code の allowed `field` は 4 の runner pre-check reason field shape table に従います。
その table にない `field` を同じ reason_code で使ってはいけません。

```text
hash payload:
  required error fields: kind, reason_code, field, expected_hash, actual_hash
  forbidden error fields: expected_value, actual_value
  reason_code:
    - runner_policy_hash_mismatch
    - request_axiom_policy_hash_mismatch
    - request_import_manifest_hash_mismatch
...
value payload:
  required error fields: kind, reason_code, field, expected_value, actual_value
  forbidden error fields: expected_hash, actual_hash
  reason_code:
    - runner_policy_reference_invalid
    - runner_policy_file_unreadable
    - runner_policy_invalid
...
request_policy_hash_mismatch:
  field = "policy.id" or "policy.version":
    use value payload
  field = "policy.hash":
    use hash payload
  other field:
...
`checker_internal_error`、`resource_exhausted`、`timeout` の reason_code-specific
`error` member rule は次で固定します。ここに明記されていない `error` member は forbidden です。

```text
checker_internal_error / checker_reported_internal_error:
  required error fields: kind, reason_code

checker_internal_error / malformed_success_output:
  required error fields: kind, reason_code
  optional error fields: field, expected_value, actual_value

checker_internal_error / success_exit_status_mismatch:
  required error fields: kind, reason_code

checker_internal_error / missing_rejection_error:
  required error fields: kind, reason_code

checker_internal_error / malformed_rejection_output:
  required error fields: kind, reason_code
  optional error fields: field, expected_value, actual_value

checker_internal_error / malformed_internal_error_output:
  required error fields: kind, reason_code
  optional error fields: field, expected_value, actual_value

checker_internal_error / checker_module_mismatch:
  required error fields: kind, reason_code, field, expected_value, actual_value

checker_internal_error / process_exit_failure:
  required error fields: kind, reason_code
  optional error fields: field, actual_value
...
resource_exhausted / launch_resource_exhausted:
  required error fields: kind, reason_code

resource_exhausted / checker_resource_exhausted:
  required error fields: kind, reason_code
```

`malformed_*_output` の optional `field` / `expected_value` / `actual_value` は、
raw JSON parse 後の raw schema failure から deterministic field shape を得られた場合だけ入れます。
invalid JSON の場合は `field = "checker_raw"`、`expected_value = "valid_json"`、
`actual_value = "invalid_json"` を入れます。
入れる場合は3 member を必ず同時に入れ、`expected_hash` / `actual_hash` は forbidden です。
`success_exit_status_mismatch`、`missing_rejection_error`、`checker_reported_internal_error` では
payload member を合成してはいけません。
`process_exit_failure` で `process.termination_reason = killed_without_exit_status` を記録する場合だけ
`field = "process.termination_reason"`、`actual_value = "killed_without_exit_status"` を入れます。
入れる場合は2 member を必ず同時に入れ、`expected_hash` / `actual_hash` / `expected_value` は forbidden です。
exit code `>= 3` などで deterministic field がない場合は `field` / `actual_value` を omit します。

`MachineCheckResult.error.field` の reason_code compatibility は、最終的に canonical
`MachineCheckResult.error.field` に保存される文字列に対する closed set として固定します。
validator はこの table の文字列をそのまま照合し、prefix を追加合成してはいけません。

```text
runner_policy_reference_invalid:
  "policy", "policy.kind", "policy.path", "policy.hash",
  or "policy.<unknown RunnerPolicyReference member name matching delegated_schema_path_component>"
...
runner_policy_invalid:
  "policy.path", "$", "schema",
  or a RunnerPolicy-local field path accepted by 4.1

request_policy_hash_mismatch:
  "policy.id", "policy.version", or "policy.hash"
...
request_axiom_policy_mismatch / request_axiom_policy_file_unreadable:
  "axiom_policy"
...
request_budget_mismatch:
  "budget.max_steps", "budget.max_memory_mb", or "budget.timeout_ms"
...
request_import_manifest_invalid:
  "imports.manifest",
  or "imports.manifest.<ImportLockManifest-local field path without imports.manifest prefix>"
...
checker_identity_manifest_invalid:
  "checker_identity_manifest.path",
  or "checker_identity_manifest.<CheckerIdentityManifest-local field path without checker_identity_manifest prefix>"

checker_build_hash_mismatch:
  "checker.build_hash"

checker_identity_mismatch:
  "checker.id" or "checker.binary_id"

checker_identity_missing:
  "checker_identity_manifest.checkers[]", "checker.id", or "checker.build_hash"

malformed_success_output / malformed_rejection_output / malformed_internal_error_output:
  no payload,
  "checker_raw",
  or "checker_raw.<raw_schema_field_path>"

checker_module_mismatch:
  "module"

process_exit_failure:
  no payload, or "process.termination_reason"

checker_reported_internal_error / success_exit_status_mismatch / missing_rejection_error /
launch_timeout / checker_timeout / launch_resource_exhausted / checker_resource_exhausted:
  field forbidden
```

上に列挙していない reason_code では `field` は forbidden です。
delegated field path の suffix は、委譲先 validator の field shape table が出力できる
concrete JSON path だけです。`ImportLockManifest-local field path` は `imports.manifest.`
prefix を含まない `imports[0].module` などの path、`CheckerIdentityManifest-local field path` は
`checker_identity_manifest.` prefix を含まない `$`、`schema`、
`checkers[0].profile` などの path です。参照先 table がすでに
`imports.manifest.<JSON path>` または `checker_identity_manifest.<JSON path>` と書いている場合、
local suffix はその virtual root prefix を除いた部分です。canonical `error.field` には
上の table どおり virtual root prefix を1回だけ含めます。
`RunnerPolicy-local field path` は 4.1 の local field shape をそのまま使います。
`budgets.<profile>` と `budgets.<profile>.<member>` の `<profile>` は
checker profile name grammar に従う typed key slot であり、`fast-kernel` のような hyphen を含む
valid profile name を許します。
`budgets` key の profile grammar violation だけは 4.1 の `budget_key_path_component`
reporting rule を使い、表現できない invalid key は `budgets` に fallback します。
array index は 0-based decimal の具体値を使い、`[]` はその table が literal として
明記した場合だけ許します。
`MachineCheckResult.error` の static validation は上の prefix / root / path shape だけを検査し、
source artifact の「最初の schema failure」と完全に同じ field かどうかは runner の生成処理または
comparison recomputation が検査します。
delegated schema field path に unknown / duplicate raw object key を literal component として含める場合、
その key は次の component grammar に一致しなければなりません。一致しない unknown key や
duplicate key を canonical field path に copy してはいけません。
この grammar は domain-specific typed key slot には適用しません。

```text
delegated_schema_path_component:
  [A-Za-z_][A-Za-z0-9_]{0,127}
```

raw key を表現できない場合、producer は raw key を copy せず、containing object path を
`error.field` に使います。RunnerPolicyReference では `policy`、
RunnerPolicy では `$`、`checker_allowlist[<i>]`、`checker_identity_manifest`、
`import_policy`、`axiom_policy`、`budgets`、または representable な
`budgets.<invalid_profile_key>`、
ImportLockManifest では `imports.manifest`、`imports.manifest.imports[<i>]`、
または `imports.manifest.imports[<i>].certificate`、
CheckerIdentityManifest では `checker_identity_manifest`、
`checker_identity_manifest.generated_by`、または
`checker_identity_manifest.checkers[<i>]` です。
containing object path を使う場合でも `expected_value` / `actual_value` で
unknown / duplicate の内容を表します。

`MachineCheckResult.error` object の kind-specific schema / domain validation priority は
次で固定します。reason-coded error だけでなく、ordinary failure と decode / schema /
noncanonical failure にも同じ priority を適用します。下の field shape は
`MachineCheckResult` artifact 内の path です。
`NormalizeErrorResult.error.reason_code = machine_result_schema_invalid` として報告する場合は
先頭の `error` を `machine_results[].error` に置換し、`NormalizedCheckResult.results[]` entry 内で
報告する場合は先頭の `error` を `results[<i>].error` に置換します。

```text
1. error object type
2. duplicate known key in error object, in error field order
3. kind member presence / null / type / enum
4. reason_code member presence / null / type / enum, for reason-coded error kinds
5. reason_code / kind compatibility, for reason-coded error kinds
6. field member presence / null / type, for reason/kind variants where field can be payload-bearing
7. reason_code / field compatibility, for reason-coded payload-bearing variants
8. kind-specific or reason-code-specific required payload member presence / null / type /
   value shape, in the order expected_hash, actual_hash, expected_value, actual_value
9. conditional forbidden member presence and allowed optional member value shape, in the order
   reason_code, field, declaration, core_path, section, offset,
   expected_hash, actual_hash, expected_value, actual_value
10. duplicate unknown field, then unknown field, by bytewise field name order within each group
```

error field order for duplicate known key は `kind`、`reason_code`、`field`、`declaration`、
`core_path`、`section`、`offset`、`expected_hash`、`actual_hash`、`expected_value`、
`actual_value` です。
step 9 では、現在の kind / reason_code / payload family で forbidden の member が存在する場合、
その値の shape は検査せず forbidden member として報告します。同じ順序位置の member が
allowed optional で存在し、値だけが malformed な場合は value shape failure として報告します。
forbidden member の `expected_value` は次で固定します。
非 reason-coded error kind で forbidden な member、または `reason_code` を持てない
error kind に存在する `reason_code` では `absent_for_error_kind` を使います。
valid かつ compatible な `reason_code` を持つ reason-coded error kind で forbidden な member では、
payload family 起因か reason_code 固有起因かにかかわらず `absent_for_error_reason_code` を使います。
たとえば `policy_failure` に `declaration` が存在する場合は
`expected_value = "absent_for_error_reason_code"` です。
`request_policy_hash_mismatch` のように `field` によって hash payload と value payload が分かれる
reason_code では、step 7 の field compatibility を通過した後に payload family を確定し、
step 8 / 9 を適用します。
`malformed_*_output` や `process_exit_failure` のように no payload と payload の両方を許す
reason_code では、`field` が存在しない場合は no payload、`field` が存在する場合は payload-bearing
reason として step 7 以降を適用します。
ただし optional payload reason で payload member の一部だけが存在する場合は、payload-bearing として扱います。
`malformed_*_output` では `field` / `expected_value` / `actual_value` のいずれかが存在すれば
value payload とみなし、3 member すべてを required として step 6 / 8 を適用します。
`process_exit_failure` では `field` / `actual_value` のいずれかが存在すれば
actual-value-only payload とみなし、2 member すべてを required として step 6 / 8 を適用します。
したがって `malformed_*_output` に `actual_value` だけが存在する場合は
`error.field` missing を先に報告し、no payload として `actual_value` forbidden にはしません。

`MachineCheckResult.error` schema / domain failure の field shape は次で固定します。
...
duplicate known key in error object:
  field = "error.<duplicated_member>"
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

kind missing / null / type / enum:
  field = "error.kind"
  expected_value = "MachineCheckResult.error.kind"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

reason_code missing / null / type / enum:
  field = "error.reason_code"
  expected_value = "MachineCheckResult.error.reason_code"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

reason_code / kind mismatch:
  field = "error.reason_code"
  expected_value = "reason_code_for_error_kind"
  actual_value = "invalid_enum"

field missing / null / type:
  field = "error.field"
  expected_value = "MachineCheckResult.error.field"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

reason_code / field mismatch:
  field = "error.field"
  expected_value = "field_path_for_error_reason_code"
  actual_value = "unknown_field"

declaration missing / null / type / grammar:
  field = "error.declaration"
  expected_value = "phase2_name"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_name_format"

core_path missing / null / type / grammar:
  field = "error.core_path"
  expected_value = "core_path"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_path"

section missing / null / type / grammar:
  field = "error.section"
  expected_value = "section_token"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_name_format"

offset missing / null / type / range:
  field = "error.offset"
  expected_value = "non_negative_i64"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "integer_out_of_range"

expected_hash / actual_hash missing / null / type / hash format:
  field = "error.<hash_payload_member>"
  expected_value = "sha256:<lower-hex>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_hash_format"

expected_value / actual_value missing / null / scalar shape:
  field = "error.<value_payload_member>"
  expected_value = "deterministic_json_scalar"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

conditional forbidden member by error.kind:
  field = "error.<forbidden_member>"
  expected_value = "absent_for_error_kind"
  actual_value = "forbidden_field"

conditional forbidden member by reason_code / payload family:
  field = "error.<forbidden_member>"
  expected_value = "absent_for_error_reason_code"
  actual_value = "forbidden_field"

unknown field:
  field = "error.<unknown_member>"
  expected_value = "absent"
  actual_value = "unknown_field"
```

`MachineCheckResult.error` object 内の duplicate unknown member または unknown member 名を
`schema_path_component` で表現できない場合は、raw key を copy せず `field = "error"` にします。
known member の duplicate key は上の fixed member field を使います。
item 2 の duplicate known key は known member だけを扱います。
duplicate unknown member は item 10 で扱い、kind / reason_code / payload validation より後です。
item 10 の内部では、まず duplicate unknown member を field name の bytewise order で報告し、
duplicate unknown がない場合だけ non-duplicate unknown member を bytewise order で報告します。
duplicate unknown member では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。
...
identity mismatch の payload family は field 種別で固定します。
`checker.build_hash` / `checker.binary_hash` mismatch では
`error.expected_hash` / `error.actual_hash` を使い、
`checker.id` / `checker.binary_id` mismatch では
`error.expected_value` / `error.actual_value` を使います。
value identity mismatch を hash field に入れてはいけません。
  ...
  "results": [
    {
      ...
      "checker_binary_id": "npa-fast-kernel-macos-aarch64",
    ...
    },
    {
      ...
      "artifact_hash": "sha256:...",
      "checker_id": "npa-checker-ref",
      "checker_binary_id": "npa-checker-ref-macos-aarch64",
      "checker_binary_hash": "sha256:...",
      "checker_build_hash": "sha256:...",
      "checker_profile": "reference",
      "process_launched": true,
    ...
    },
    {
      ...
      "checker_binary_id": "npa-checker-ext-macos-aarch64",
      "checker_binary_hash": "sha256:...",
      "checker_build_hash": "sha256:...",
...
`NormalizedCheckResult` top-level schema / domain validation priority は次で固定します。
複数の violation が同時に存在する場合は最初の1件だけを返します。
`results[]` entry と `comparison` object の詳細 priority は後続の各節に従います。

```text
1. top-level object type
2. duplicate top-level known field, in top-level field order
3. top-level schema missing / null / type / mismatch
4. top-level direct required member presence / null / type / value shape, in the order
   normalized_result_id, normalized_result_hash, artifact, artifact_hash,
   policy, results, comparison
5. artifact object schema / domain validation
6. policy object schema / domain validation
7. results[] entry schema / domain validation, by smaller index and entry-local priority
8. comparison object schema / domain validation, by comparison-local priority
9. duplicate unknown top-level field, then unknown top-level field,
   by bytewise field name order within each group
```

top-level field order は `schema`、`normalized_result_id`、`normalized_result_hash`、
`artifact`、`artifact_hash`、`policy`、`results`、`comparison` です。
top-level object type failure では `field = "$"`、`expected_value = "object"`、
`actual_value = "wrong_type" | "null_not_allowed"` を使います。
duplicate top-level known field では duplicated member の field name を使い、
`expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` とします。
top-level schema missing / null / type では `field = "schema"`、
`expected_value = "npa.phase8.normalized_check_result.v1"`、
`actual_value = "missing" | "null_not_allowed" | "wrong_type"` を使います。
top-level schema mismatch では同じ `field` / `expected_value` で、
`actual_value` に実際の schema string を入れ、`wrong_schema` は使いません。
top-level hash member `normalized_result_hash` / `artifact_hash` の
missing / null / wrong type / invalid hash format では、
`expected_value = "sha256:<lower-hex>"`、
`actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_hash_format"` を使います。
`artifact`、`policy`、`comparison` の missing / null / wrong type では
`expected_value = "object"`、`results` の missing / null / wrong type では
`expected_value = "array"` を使い、`actual_value` は
`"missing" | "null_not_allowed" | "wrong_type"` です。
unknown top-level field は `field = "<unknown_member>"`、
`expected_value = "absent"`、`actual_value = "unknown_field"` とします。
unknown top-level field 名を `schema_path_component` で表現できない場合、
または duplicate unknown top-level field を報告する場合は、raw key を copy せず
`field = "$"` を使います。
duplicate unknown top-level field では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` です。
item 2 の duplicate top-level known field は known field だけを扱います。
duplicate unknown top-level field は item 9 で扱い、top-level direct required member、
`artifact` / `policy` / `results[]` / `comparison` validation より後に報告します。
item 9 の内部では、まず duplicate unknown top-level field を field name の bytewise order で報告し、
duplicate unknown がない場合だけ non-duplicate unknown top-level field を bytewise order で報告します。

`NormalizedCheckResult.artifact` は closed-world object で、required member は
`module`、`input_file_hash`、`expected_certificate_hash`、`import_lock_hash`、
`axiom_policy_hash` の順です。
artifact member validation priority は duplicate known member、required member shape、
duplicate unknown member、unknown member の順です。
`artifact.module` は Phase 2 module name grammar、その他の artifact member は
`sha256:<lower-hex>` hash です。
`NormalizedCheckResult.policy` は copied `RunnerPolicy` metadata の closed-world object で、
required member は `id`、`version`、`hash` の順です。
policy member validation priority は duplicate known member、required member shape、
duplicate unknown member、unknown member の順です。
`policy.id` は `RunnerPolicy.id` grammar、`policy.version` は positive i64、
`policy.hash` は `sha256:<lower-hex>` hash です。
artifact / policy の duplicate known member は `field = "<object>.<member>"`、
`expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` とします。
duplicate known member が複数ある場合は、それぞれの object の required member order で最初の
member を報告します。
artifact / policy の required member shape failure ではその member path を `field` に入れ、
`expected_value` は `module_name`、`sha256:<lower-hex>`、`runner_policy_id`、
または `positive_i64` のいずれかです。
artifact / policy の `actual_value` は `missing`、`null_not_allowed`、`wrong_type`、
`invalid_name_format`、`invalid_hash_format`、`non_positive_integer`、
または `integer_out_of_range` のいずれかです。
artifact / policy の unknown member は `field = "<object>.<unknown_member>"`、
`expected_value = "absent"`、`actual_value = "unknown_field"` とします。
unknown member 名を `schema_path_component` で表現できない場合、
または duplicate unknown member を報告する場合は raw key を copy せず
`field = "artifact"` または `field = "policy"` を使います。
duplicate unknown member では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` です。
artifact / policy の duplicate unknown member が複数ある場合は field name の bytewise order で
最初の member を報告します。
duplicate unknown member がない場合だけ non-duplicate unknown member を bytewise order で報告します。

`NormalizedCheckResult.normalized_result_id` は human reference id です。
値は `AiAuditSidecar.source.normalized_result_id` と同じ non-empty visible ASCII string で、
bytes `0x21`-`0x7E` に限定し、space、control character、non-ASCII character、
empty string を禁止します。
missing / null / wrong type / empty string / invalid format は
`normalized_result_schema_invalid` とし、`field = "normalized_result_id"`、
`expected_value = "normalized_result_id"`、`actual_value = "missing" |
"null_not_allowed" | "wrong_type" | "empty_string" | "invalid_string_format"` にします。
`NormalizedCheckResult.results[].result_id` は、元の `MachineCheckResult.result_id` を写す
human reference id であり、同じ grammar を使います。

MVP の `NormalizedCheckResult.results[]` entry schema：

この文書の `npa.phase8.normalized_check_result.v1` では、`process_launched = true` の entry に
`checker_binary_id` と `checker_binary_hash` の両方が required です。
過去の draft や local fixture が `checker_binary_hash` だけを持つ場合、その artifact はこの current
v1 schema では invalid として扱い、current normalizer で再生成します。
この spec を release した後に required field を追加または削除する場合は、
`normalized_check_result` schema version を上げます。
...
optional when status = failed and present in the input MachineCheckResult artifact:
  - certificate_hash
  - export_hash
  - axiom_report_hash
...
`NormalizedCheckResult.results[]` entry の schema / domain validation priority は次で固定します。

```text
1. entry object type
2. duplicate known key in the entry, in result entry field order
3. always required member presence / null / type / value shape, in the order
   result_id, result_hash, request_hash, policy_hash, artifact_hash,
   checker_profile, process_launched, status
4. process_launched conditional fields that apply to the current process_launched value:
   if process_launched = true, required checker_binary_id, checker_binary_hash, in that order;
   if process_launched = false, forbidden checker_binary_id, checker_binary_hash,
   checker_id, checker_build_hash, in that order
5. status = checked conditional required / forbidden fields, in the order
   certificate_hash, export_hash, axiom_report_hash, error, failure_key
6. status = failed error member presence / null / type, then nested error schema / domain validation
7. status = failed failure_key member presence / null / type, then failure_key object schema / domain validation
8. status = failed derived failure_key equality
9. allowed optional copy field value shape, in the order
   checker_id, checker_build_hash, certificate_hash, export_hash, axiom_report_hash
10. duplicate unknown field, then unknown field, by bytewise field name order within each group
```

result entry field order for duplicate known key は `result_id`、`result_hash`、`request_hash`、
`policy_hash`、`artifact_hash`、`checker_profile`、`process_launched`、`status`、
`checker_binary_id`、`checker_binary_hash`、`checker_id`、`checker_build_hash`、
`certificate_hash`、`export_hash`、`axiom_report_hash`、`error`、`failure_key` です。
`status = failed` では `error` を validation してから `failure_key` を validation します。
したがって `error.kind` invalid と `failure_key` missing が同時に存在する場合は
`results[<i>].error.kind` の failure を先に返します。
conditional forbidden field は値の shape を検査しません。
`process_launched` conditional forbidden では `expected_value = "absent_for_process_launched"`、
`status` conditional forbidden では `expected_value = "absent_for_status"` とし、
どちらも `actual_value = "forbidden_field"` とします。
`process_launched = true` の `checker_id` / `checker_build_hash` は required ではありません。
存在する場合の null / type / grammar / hash format は item 9 の optional copy field value shape で検査します。
したがって `process_launched = true` で `checker_id` が malformed かつ
`status = checked` の `certificate_hash` が missing の場合は、item 5 の
`results[<i>].certificate_hash` failure を先に返します。
`process_launched = false` で `checker_binary_id`、`checker_binary_hash`、`checker_id`、または
`checker_build_hash` が存在する場合は `normalized_result_schema_invalid` であり、
comparison-generated `malformed_process_state` には進みません。
`results[]` entry schema failure の field shape は次で固定します。
...
duplicate known key in the entry:
  field = "results[<i>].<duplicated_member>"
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

result_id missing / null / type / empty / grammar:
  field = "results[<i>].result_id"
  expected_value = "result_id"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" |
                 "empty_string" | "invalid_string_format"

checker_binary_id required by process_launched missing / null / type / grammar invalid:
  field = "results[<i>].checker_binary_id"
  expected_value = "checker_binary_id"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_name_format"

hash member required by the current entry state missing / null / type / hash format:
  field = "results[<i>].<hash_member>"
  expected_value = "sha256:<lower-hex>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_hash_format"

optional hash member present but null / type / hash format invalid:
  field = "results[<i>].<hash_member>"
  expected_value = "sha256:<lower-hex>"
  actual_value = "null_not_allowed" | "wrong_type" | "invalid_hash_format"

checker_id present but null / type / grammar invalid:
  field = "results[<i>].checker_id"
  expected_value = "checker_id"
  actual_value = "null_not_allowed" | "wrong_type" | "invalid_name_format"

checker_profile missing / null / type / grammar:
  field = "results[<i>].checker_profile"
  expected_value = "checker_profile_name"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_name_format"

process_launched missing / null / type:
  field = "results[<i>].process_launched"
  expected_value = "boolean"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

status missing / null / type / enum:
  field = "results[<i>].status"
  expected_value = "NormalizedCheckResult.results[].status"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

error missing / null / type:
  field = "results[<i>].error"
  expected_value = "MachineCheckResult.error"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

failure_key missing / null / type:
  field = "results[<i>].failure_key"
  expected_value = "normalized_failure_key"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

conditional forbidden field:
  field = "results[<i>].<forbidden_member>"
  expected_value = "absent_for_process_launched" | "absent_for_status"
  actual_value = "forbidden_field"

unknown field:
  field = "results[<i>].<unknown_member>"
  expected_value = "absent"
  actual_value = "unknown_field"
```

`results[]` entry 内の duplicate unknown member または unknown member 名を
`schema_path_component` で表現できない場合は、raw key を copy せず
`field = "results[<i>]"` にします。known member の duplicate key は上の fixed member field を使います。
item 2 の duplicate known key は known member だけを扱います。
duplicate unknown member は item 10 で扱い、required / conditional / optional copy field validation より後です。
item 10 の内部では、まず duplicate unknown member を field name の bytewise order で報告し、
duplicate unknown がない場合だけ non-duplicate unknown member を bytewise order で報告します。
duplicate unknown member では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。

`checker_binary_id` と `checker_binary_hash` は raw checker identity を trusted verdict として採用できない
launched failure でも、runner-owned executable identity として
`MachineCheckResult.checker.binary_id` / `MachineCheckResult.checker.binary_hash` から必ず写します。
...
normalizer は `results[].result_id` を immediate input の
`MachineCheckResult.result_id` から copy します。
同じ normalizer invocation 内で paired source `MachineCheckResult.result_id` と
生成された `results[].result_id` が一致しない場合は normalizer implementation
invariant violation であり、下の source-copy set invariant と同じく
reachable CLI/API result、`NormalizeErrorResult`、`NormalizationWriteResult`、
partial `NormalizedCheckResult` のいずれにもしてはいけません。
ただし `result_id` は `normalized_result_hash` から除外される trace metadata であり、
retry-equivalent または challenge replay 共有 source artifact 間で異なり得るため、
cross-artifact source-copy mismatch の対象に含めません。
bundle validator / saved artifact validator は `result_id` の違いを
source-copy mismatch、pairing failure、または `normalized_result_hash` 不一致として扱ってはいけません。
cross-artifact validator で使う `MachineCheckResult` source-copy set は次で固定します。

```text
normalized entry member        source MachineCheckResult field             payload on mismatch
result_hash                    result_hash                                 hash
request_hash                   request_hash                                hash
policy_hash                    policy.hash                                 hash
checker_profile                checker.profile                             value
process_launched               process.launched                            value
status                         status                                      value
checker_binary_id              checker.binary_id, when present             value
checker_binary_hash            checker.binary_hash, when present           hash
checker_id                     checker.id, when present                    value
checker_build_hash             checker.build_hash, when present            hash
certificate_hash               certificate_hash, when present              hash
export_hash                    export_hash, when present                   hash
axiom_report_hash              axiom_report_hash, when present             hash
error                          error, when present                         canonical_json_string
```

`artifact_hash` は resolved `MachineCheckRequest` と top-level `artifact` から導出する値であり、
source `MachineCheckResult` copy mismatch の対象ではありません。
`failure_key` は sibling `error` から導出する値であり、この copy mismatch ではなく
`failure_key_mismatch` rule で検査します。
`results[]` entry の array position と `comparison` object も source-copy set には含めません。
source-copy set に含まれる optional source field は、source に存在する場合だけ normalized entry に
同じ値で存在しなければならず、source に存在しない場合は normalized entry でも omit しなければなりません。
presence mismatch では、source present / normalized missing の場合
`expected_value = "present"`、`actual_value = "missing"` を使い、
source absent / normalized present の場合
`expected_value = "absent_when_source_absent"`、`actual_value = "present"` を使います。
値 mismatch では上の table の payload 種別を使います。
`hash` payload は `expected_hash` に source value、`actual_hash` に normalized entry value を入れます。
`value` payload は `expected_value` に source value、`actual_value` に normalized entry value を入れます。
`canonical_json_string` payload は source object と normalized entry object の RFC 8785 canonical JSON bytes を
UTF-8 string として `expected_value` / `actual_value` に入れます。
normalizer は `result_id` とこの table の entry member を、source `MachineCheckResult` artifact から
直接生成します。
そのため、生成した normalized entry が `result_id` immediate-copy rule またはこの table と
一致しない場合は
normalizer implementation invariant violation であり、user input 起因の
`NormalizeErrorResult.error.reason_code` には割り当てません。
conforming implementation はこの状態を reachable CLI/API result として扱わず、
partial / invalid `NormalizedCheckResult` を書いてはいけません。
defensive runtime check がこの invariant violation を検出した場合は artifact contract 外の
implementation error として command / request を失敗させ、`NormalizeErrorResult`、
`NormalizationWriteResult`、または partial `NormalizedCheckResult` を返してはいけません。
実装は生成関数の self-check、debug assertion、または単体テストでこの invariant を検査します。
retry 時に既存 output file / normalized store entry の bytes が今回生成する
`NormalizedCheckResult` と異なる場合は、この copy mismatch 判定ではなく write-stage failure として
`output_path_conflict` または `normalized_store_entry_conflict` を使います。
source `MachineCheckResult` と保存済み `NormalizedCheckResult` を同時に入力として持つ
cross-artifact validator がこの copy 不一致を検出した場合は、その command 固有の
semantic artifact failure として報告します。
MVP の `release validate-bundle` では `AuxiliaryResult.kind = audit_bundle`、
`status = failed`、`error.reason_code = audit_bundle_invalid` とし、
`error.field = "normalized_check_result[<j>].artifact.results[<i>].<member>"` とします。
`<j>` は manifest schema / order validation 後の `ReleaseAuditBundleManifest.artifacts`
deterministic `(kind, path)` order における `kind = normalized_check_result` だけの
0-based occurrence index、`<i>` は saved `NormalizedCheckResult.results[]` の
0-based index、`<member>` は上の source-copy set の normalized entry member 名です。
`npa-check release bundle` generation が同じ saved normalized result copy mismatch を
materialized manifest 生成前に検出する場合は
`CommandError.reason_code = release_bundle_generation_failed` とし、
`field = "normalized_check_result[<j>].artifact.results[<i>].<member>"` を使います。
この場合の `<j>` は生成予定 `ReleaseAuditBundleManifest.artifacts` の deterministic
`(kind, path)` order における `kind = normalized_check_result` だけの 0-based occurrence index です。
`runner`、`process`、`resource_usage`、`diagnostics` など、
`results[]` entry schema に copy 先を持たない source artifact field はこの mismatch 判定の対象外です。
`status = failed` の `certificate_hash`、`export_hash`、`axiom_report_hash` は
source `MachineCheckResult` artifact に存在する場合だけ写します。
`failure_key` は独立した trusted verdict ではなく、同じ entry の `error` から
本 section の normalized failure key rule で導出した deterministic cache です。
...
`<i>` は不一致を起こした `NormalizedCheckResult.results[]` entry の concrete 0-based index です。
...
`comparison` object の static schema / domain validation は `NormalizedCheckResult` schema validation
step の一部であり、失敗時は `normalized_result_schema_invalid` です。
この static validation を通過した後だけ `npa-check compare` は RunnerPolicy を読み、
comparison object を再計算して、埋め込まれた `comparison` との差分を
`comparison_mismatch` として報告します。
static validation では、top-level `comparison` object の object type、duplicate key、
required member、member type / enum、unknown field、下の status-dependent fixed shape、
`matching_fields[]` / `missing_checker_profiles[]` の element shape、
`disagreements[]` entry schema / domain validation、および
`status_reasons[]` entry schema / domain validation だけを検査します。
`missing_checker_profiles` の具体的な profile 集合、`disagreements` の生成内容、
`status_reasons` の生成内容が再計算結果と一致するかどうかは step 6 の
`comparison_mismatch` で検査します。
複数の `comparison` static schema / domain failure が同時に成立する場合の priority は
次で固定します。

```text
1. comparison object type
2. duplicate known key in comparison, in comparison field order
3. required member presence / null / type / enum, in the order
   status, matching_fields, missing_checker_profiles, disagreements, status_reasons
4. matching_fields[] element schema / domain failure, by smaller index
5. missing_checker_profiles[] element schema / domain failure, by smaller index
6. disagreements[] entry schema / domain failure, by smaller index and entry-local priority
7. status_reasons[] entry schema / domain failure, by smaller index and entry-local priority
8. status-dependent fixed shape, in the order
   matching_fields, missing_checker_profiles, disagreements, status_reasons
9. duplicate unknown field in comparison, then unknown field in comparison,
   by bytewise field name order within each group
```

comparison field order for duplicate known key は `status`、`matching_fields`、
`missing_checker_profiles`、`disagreements`、`status_reasons` です。
この priority により、たとえば `comparison.status = all_agree_checked` で
`status_reasons[0]` が malformed の場合は、`comparison.status_reasons` の
status-dependent shape mismatch ではなく、`comparison.status_reasons[0].*` の
nested schema / domain failure を先に返します。
`comparison` static schema / domain failure の field shape は次で固定します。
...
duplicate known key in comparison:
  field = "comparison.<duplicated_member>"
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

comparison.status missing / null / type / enum:
  field = "comparison.status"
  expected_value = "NormalizedCheckResult.comparison.status"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

comparison matching_fields / missing_checker_profiles / disagreements / status_reasons missing / null / type:
  field = "comparison.<member>"
  expected_value = "array"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

comparison.matching_fields element null / type / enum:
  field = "comparison.matching_fields[<i>]"
  expected_value = "comparison_matching_field"
  actual_value = "null_not_allowed" | "wrong_type" | "invalid_enum"

comparison.missing_checker_profiles element null / type / grammar:
  field = "comparison.missing_checker_profiles[<i>]"
  expected_value = "checker_profile_name"
  actual_value = "null_not_allowed" | "wrong_type" | "invalid_name_format"

status-dependent matching_fields shape:
  field = "comparison.matching_fields"
  expected_value = "matching_fields_for:<comparison.status>"
  actual_value = "comparison_field_mismatch"

status-dependent missing_checker_profiles shape:
  field = "comparison.missing_checker_profiles"
  expected_value = "missing_checker_profiles_shape_for:<comparison.status>"
  actual_value = "comparison_field_mismatch"

status-dependent disagreements shape:
  field = "comparison.disagreements"
  expected_value = "disagreements_shape_for:<comparison.status>"
  actual_value = "comparison_field_mismatch"

status-dependent status_reasons shape:
  field = "comparison.status_reasons"
  expected_value = "status_reasons_shape_for:<comparison.status>"
  actual_value = "comparison_field_mismatch"

unknown field in comparison:
  field = "comparison.<unknown_member>"
  expected_value = "absent"
  actual_value = "unknown_field"
```

`comparison` object 内の duplicate unknown member または unknown member 名を
`schema_path_component` で表現できない場合は、raw key を copy せず
`field = "comparison"` にします。known member の duplicate key は上の fixed member field を使います。
item 2 の duplicate known key は known member だけを扱います。
duplicate unknown member は item 9 で扱い、nested entry validation と status-dependent shape validation より後です。
item 9 の内部では、まず duplicate unknown member を field name の bytewise order で報告し、
duplicate unknown がない場合だけ non-duplicate unknown member を bytewise order で報告します。
duplicate unknown member では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。
...
```text
1. entry object type
2. duplicate key in the entry, known field order then unknown bytewise field name order
...
6. non-duplicate unknown field, bytewise field name order
...
schema failure の field shape は次で固定します。
...
duplicate key in the entry:
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

field member missing / null / type / enum:
  expected_value = "disagreement_field"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

baseline_checker_profile / checker_profile missing / null / type / grammar:
  expected_value = "checker_profile_name"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_name_format"

baseline_hash / actual_hash missing / null / type / hash format:
  expected_value = "sha256:<lower-hex>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_hash_format"

baseline_value / actual_value missing / null / type / enum:
  expected_value = "NormalizedCheckResult.results[].status"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

field-specific forbidden member presence:
  expected_value = "absent_for_disagreement_field"
  actual_value = "forbidden_field"

non-duplicate unknown field:
  expected_value = "absent"
  actual_value = "unknown_field"
```

`disagreements[]` entry 内の duplicate unknown member または unknown member 名を
`schema_path_component` で表現できない場合は、raw key を copy せず
`field = "comparison.disagreements[<i>]"` にします。known member の duplicate key は
`comparison.disagreements[<i>].<member>` を使います。

duplicate key の tie-break は `field`、`baseline_checker_profile`、`baseline_hash`、
`baseline_value`、`checker_profile`、`actual_hash`、`actual_value`、その後 duplicate unknown
field name の bytewise field name order です。
item 6 は duplicate ではない unknown member だけを扱います。
required member と forbidden member が同時に不正な場合は required member failure を先に返します。
forbidden member は値の shape を検査せず、上の fixed field shape を使います。
...
それ以外の status では `[]` にします。
status-dependent fixed shape では、entry-local validation が通った後に
`status = policy_failure` のすべての entry が `kind = policy_failure` であること、
`status = inconclusive` のすべての entry が `kind = inconclusive` であることも検査します。
この条件に反する場合は `field = "comparison.status_reasons"` の
`comparison_field_mismatch` です。
...
`kind` は `reason_code` と `error_kind` から一意に決まります。
`kind = policy_failure` は、comparison-generated policy reason と、
原因 result から copy した `error_kind = policy_failure` の reason に使います。
`kind = inconclusive` は、`malformed_process_state` と、原因 result から copy した
`error_kind = checker_internal_error` / `timeout` / `resource_exhausted` の reason に使います。
したがって `malformed_process_state` は `kind = inconclusive` かつ
`error_kind = checker_internal_error` です。
同じ `reason_code` 文字列が MachineCheckResult と comparison-generated reason の両方に存在する場合でも、
この `kind` mapping は同じ結果になります。
hash / value mismatch がある場合は `field` と expected / actual を写します。
`status_reasons` は次の `status_reason_sort_key` の bytewise lexicographic order で昇順に並べます。
omitted component は empty string として扱います。

```text
status_reason_sort_key:
  - kind
  - checker_profile or ""
  - field or ""
  - reason_code
  ...
  - expected_hash or ""
  - actual_hash or ""
  - expected_value as RFC 8785 canonical JSON scalar bytes, or ""
  - actual_value as RFC 8785 canonical JSON scalar bytes, or ""

payload_family_rank:
  0 = no payload
  1 = hash payload
  2 = value payload
  3 = actual-value-only payload
```

同じ `status_reason_sort_key` を持つ entry は canonical JSON object として同一でなければなりません。
同一 entry が重複して存在しても static schema failure にはせず、comparison recomputation との
content mismatch として `comparison_mismatch` で検出します。
`status_reasons[]` entry の static schema / domain validation は
`NormalizedCheckResult` schema validation step の一部であり、失敗時は
`normalized_result_schema_invalid` です。
再計算した comparison が生成する `status_reasons` との内容差分は、この static validation 通過後に
`comparison_mismatch` として報告します。
`status_reasons[]` entry は closed-world object で、required / conditional / forbidden member は
次で固定します。
...
required for result-local reason_code:
  - checker_profile
  - result_hash

forbidden for global reason_code:
  - checker_profile
  - result_hash

hash payload:
  required: field, expected_hash, actual_hash
  forbidden: expected_value, actual_value

value payload:
  required: field, expected_value, actual_value
  forbidden: expected_hash, actual_hash

actual-value-only payload:
  required: field, actual_value
  forbidden: expected_hash, actual_hash, expected_value

no payload:
  forbidden: field, expected_hash, actual_hash, expected_value, actual_value

unknown field:
  forbidden
```

`policy_hash_mismatch` だけが global reason_code です。
それ以外の `NormalizedComparisonReasonCode` は result-local reason_code であり、
`checker_profile` と `result_hash` が required です。
comparison-generated reason_code の payload family は次で固定します。
...
comparison-generated reason_code の allowed `field` は次で固定します。
...
result_policy_hash_mismatch:
  field = "results[].policy_hash"

checker_profile_not_allowed:
  field = "results[].checker_profile"

checker_binary_id_mismatch:
  field = "results[].checker_binary_id"
...
原因 result の `MachineCheckResult.error.reason_code` を写す status reason では、
原因 result の `error` に存在する payload member family をそのまま使います。
この場合の allowed `field` と payload family は、同じ reason_code に対する
`MachineCheckResult.error` の reason_code-specific rule に従います。
`policy_failure` reason_code では 5 の `MachineCheckResult.error.field` final closed set を使い、
4 の runner pre-check reason field shape table から prefix を再合成してはいけません。
copied `status_reasons[]` entry の `field` は、原因 result の `MachineCheckResult.error.field`
と同じ canonical string です。
`malformed_*_output` reason_code では no payload、または `field = "checker_raw"` /
`field = "checker_raw.<raw_schema_field_path>"` の value payload だけを許します。
`raw_schema_field_path` は 5 の `CheckerRawResult JSON path` grammar に従います。
static validator はこの prefix / grammar だけを検査し、source result の raw schema failure と
完全に同じ field かどうかは comparison recomputation の内容検査で確認します。
`checker_module_mismatch` では `field = "module"` の value payload だけを許します。
`process_exit_failure` では no payload、または `field = "process.termination_reason"` の
actual-value-only payload だけを許します。
`checker_reported_internal_error`、`success_exit_status_mismatch`、`missing_rejection_error`、
`launch_timeout`、`checker_timeout`、`launch_resource_exhausted`、
`checker_resource_exhausted` では no payload だけを許します。
同じ `reason_code` 文字列が MachineCheckResult copied reason と
comparison-generated reason の両方に存在する場合、static validator は両方の
allowed `field` set を受け付けます。どちらの source から生成されるべきかは
comparison recomputation の内容検査で固定し、不一致は `comparison_mismatch` です。
原因 result の `error` に `expected_hash` または `actual_hash` が存在する場合は hash payload、
`expected_value` と `actual_value` が存在する場合は value payload、
`field` と `actual_value` だけが存在する場合は actual-value-only payload、
payload member が存在しない場合は no payload です。
hash payload と value payload を同じ entry で混在させてはいけません。
原因 result に存在しない payload member を status reason が補完してはいけません。
`reason_code` と `error_kind` / `kind` / `field` の対応は
5 の `MachineCheckResult.error.reason_code` table、上の comparison-generated reason table、
および allowed `field` table に従います。
対応しない組み合わせは `normalized_result_schema_invalid` です。
ただし static validator は `result_hash` が現在の `results[]` entry を指すかどうか、
または payload がその result の `error` と完全に一致するかどうかまでは検査しません。
それらは comparison recomputation の生成内容検査であり、不一致は `comparison_mismatch` です。

`status_reasons[]` entry の schema validation priority は次で固定します。

```text
1. entry object type
2. duplicate known key in the entry, in status reason field order
3. always required member presence / null / type / enum, in the order kind, error_kind, reason_code
4. reason_code / error_kind / kind compatibility, in the order error_kind, kind
5. checker_profile / result_hash conditional required or forbidden
6. field member presence / null / type, for payload families that require field
7. reason_code / field compatibility, for payload families that require field
8. payload family required member presence / null / type / value shape, in the order expected_hash, actual_hash, expected_value, actual_value
9. payload family forbidden member presence, in the order field, expected_hash, actual_hash, expected_value, actual_value
10. duplicate unknown field, then unknown field, by bytewise field name order within each group
11. array order violation
```

status reason field order for duplicate known key は `kind`、`error_kind`、`reason_code`、
`checker_profile`、`result_hash`、`field`、`expected_hash`、`actual_hash`、
`expected_value`、`actual_value` です。
`request_policy_hash_mismatch` のように `field` によって hash payload と value payload が分かれる
reason_code では、step 7 の field compatibility を通過した後に payload family を確定し、
step 8 / 9 を適用します。`field` が allowed set に入らない場合は、他の payload member を検査せず
step 7 の failure を返します。
`malformed_*_output` や `process_exit_failure` のように no payload と payload の両方を許す
reason_code では、`field` が存在しない場合は no payload、`field` が存在する場合は payload-bearing
reason として step 7 以降を適用します。
ただし optional payload reason の `status_reasons[]` entry で payload member の一部だけが
存在する場合は、`MachineCheckResult.error` と同じく payload-bearing として扱います。
`malformed_*_output` では `field` / `expected_value` / `actual_value` のいずれかが存在すれば
value payload とみなし、3 member すべてを required として step 6 / 8 を適用します。
`process_exit_failure` では `field` / `actual_value` のいずれかが存在すれば
actual-value-only payload とみなし、2 member すべてを required として step 6 / 8 を適用します。
したがって copied `status_reasons[]` entry に `actual_value` だけが存在する場合は
`comparison.status_reasons[<i>].field` missing を先に報告し、no payload として
`actual_value` forbidden にはしません。

`status_reasons[]` entry schema failure の field shape は次で固定します。

```text
entry object type:
  field = "comparison.status_reasons[<i>]"
  expected_value = "object"
  actual_value = "wrong_type" | "null_not_allowed"

duplicate known key in the entry:
  field = "comparison.status_reasons[<i>].<duplicated_member>"
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

kind missing / null / type / enum:
  field = "comparison.status_reasons[<i>].kind"
  expected_value = "status_reason_kind"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

error_kind missing / null / type / enum:
  field = "comparison.status_reasons[<i>].error_kind"
  expected_value = "status_reason_error_kind"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

reason_code missing / null / type / enum:
  field = "comparison.status_reasons[<i>].reason_code"
  expected_value = "NormalizedComparisonReasonCode"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

checker_profile missing / null / type / grammar:
  field = "comparison.status_reasons[<i>].checker_profile"
  expected_value = "checker_profile_name"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_name_format"

result_hash missing / null / type / hash format:
  field = "comparison.status_reasons[<i>].result_hash"
  expected_value = "sha256:<lower-hex>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_hash_format"

field missing / null / type:
  field = "comparison.status_reasons[<i>].field"
  expected_value = "status_reason_field_path"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

expected_hash / actual_hash missing / null / type / hash format:
  field = "comparison.status_reasons[<i>].<hash_payload_member>"
  expected_value = "sha256:<lower-hex>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_hash_format"

expected_value / actual_value missing / null / scalar shape:
  field = "comparison.status_reasons[<i>].<value_payload_member>"
  expected_value = "deterministic_json_scalar"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

conditional forbidden member:
  field = "comparison.status_reasons[<i>].<forbidden_member>"
  expected_value = "absent_for_status_reason"
  actual_value = "forbidden_field"

reason_code / error_kind mismatch:
  field = "comparison.status_reasons[<i>].error_kind"
  expected_value = "error_kind_for_reason_code"
  actual_value = "invalid_enum"

reason_code / kind mismatch:
  field = "comparison.status_reasons[<i>].kind"
  expected_value = "status_reason_kind_for_reason_code"
  actual_value = "invalid_enum"

reason_code / field mismatch:
  field = "comparison.status_reasons[<i>].field"
  expected_value = "field_path_for_status_reason_reason_code"
  actual_value = "unknown_field"

array order violation:
  field = "comparison.status_reasons[<i>]"
  expected_value = "status_reason_sort_order"
  actual_value = "order_violation"

unknown field:
  field = "comparison.status_reasons[<i>].<unknown_member>"
  expected_value = "absent"
  actual_value = "unknown_field"
```

`status_reasons[]` entry 内の duplicate unknown member または unknown member 名を
`schema_path_component` で表現できない場合は、raw key を copy せず
`field = "comparison.status_reasons[<i>]"` にします。known member の duplicate key は
上の fixed member field を使います。
item 2 の duplicate known key は known member だけを扱います。
duplicate unknown member は item 10 で扱い、reason_code / payload validation より後です。
item 10 の内部では、まず duplicate unknown member を field name の bytewise order で報告し、
duplicate unknown がない場合だけ non-duplicate unknown member を bytewise order で報告します。
duplicate unknown member では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。
...
checker_binary_id_mismatch:
  field = "results[].checker_binary_id"
  expected_value = SelectedCheckerPolicy.binary_id
  actual_value = results[*].checker_binary_id
...
この entry では checker が起動していないため、`checker_id`、`checker_binary_id`、
`checker_binary_hash`、`checker_build_hash`、`certificate_hash` は省略されます。
`checker_internal_error`、`timeout`、`certificate_decode_error` なども、
source `MachineCheckResult` artifact に存在しない field は normalized entry でも省略します。
...
`checker_binary_id` と `checker_binary_hash` を
`MachineCheckResult.checker.binary_id` / `MachineCheckResult.checker.binary_hash` から写します。
...
`results[*]` は comparison に必要な field を input `MachineCheckResult` artifact から写すため、
comparison は source `MachineCheckResult` files を再度開かなくても実行できます。
source `MachineCheckResult` file を開く必要があるのは、
`result_hash` の再検証や監査 bundle 生成のときだけです。
`results[*].policy_hash` は input `MachineCheckResult.policy.hash` から写します。
`results[*].artifact_hash` は、その result の `request_hash` から解決した
`MachineCheckRequest` で再構成した artifact object の canonical hash です。
`results[*].process_launched` は input `MachineCheckResult.process.launched` から写します。
...
0件の場合、normalizer は `selector_ambiguous` の `NormalizeErrorResult` を返します。
2件以上の同一 `checker_profile` 入力は、single-artifact convenience mode の selection まで進まず、
先行する `checker_profile` uniqueness validation で `duplicate_checker_profile_result` として拒否します。
...
`MachineCheckResult` intrinsic validation が複数入力に対して走る場合、
input occurrence order で最初に失敗した入力だけを報告します。
input occurrence order は、CLI では `normalize-results` に渡された result file 引数の出現順、
API では `machine_results` array の 0-based index order です。
各入力に対して下記 `MachineCheckResult` intrinsic validation order の step 1-5 を完了し、
失敗した時点で後続入力と後続 validation step は見ません。
すべての入力が intrinsic validation と `artifact_selector` schema validation を通過した後に、
`checker_profile` uniqueness validation を行います。
`checker_profile` uniqueness validation も input occurrence order で入力を走査し、
すでに見た `checker.profile` が再出現した最初の入力で止めます。
複数の profile が重複している場合は、2回目の出現位置が input occurrence order で最小の
profile を `duplicate_checker_profile_result.actual_value` に入れます。
policy file はまだ解決していないため、この tie-break に required / optional profile order を使ってはいけません。
deterministic normalized order は、intrinsic validation、`artifact_selector` schema validation、
`checker_profile` uniqueness validation、policy validation をすべて通過した後の
cross-artifact validation でだけ使います。
...
standalone validator ではこの場合 `error.field = "schema"`、
normalizer の `NormalizeErrorResult` では caller-prefix rule により
`error.field = "machine_results[].schema"` を使います。
ただし top-level JSON value が object でない場合は `schema` member path が存在しないため、
normalizer では `error.field = "machine_results[]"` を使います。
...
`expected_value` に schema requirement 名、
`actual_value` に `missing`、`wrong_type`、`unknown_field`、
`invalid_enum`、`invalid_hash_format`、`invalid_name_format`、`invalid_path`、`null_not_allowed`、
`non_positive_integer`、`integer_out_of_range`、`order_violation`、
`duplicate_field`、`duplicate_axiom_name`、`forbidden_field`、`empty_string`、
`invalid_string_format` のいずれかを入れます。
...
`artifact_selector.<unknown_field_name>` は unknown field 名が `schema_path_component` に一致する場合だけ使います。
一致しない場合は `field = "artifact_selector"` にします。
`artifact_selector` schema / domain validation priority は次で固定します。
複数の violation が同時に存在する場合は最初の1件だけを返します。

```text
1. artifact_selector object 自体の wrong type / explicit null
2. known member の duplicate, in the order module, request_hash
3. module の missing / wrong type / explicit null / invalid name format / order violation
4. request_hash の missing / wrong type / explicit null / invalid hash format / order violation
5. unknown member, by bytewise field name order
```

unknown member の duplicate object key は item 5 の位置で報告し、
`expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` にします。
duplicate ではない unknown member は
`expected_value = "absent"`、`actual_value = "unknown_field"` にします。
...
`request_schema_invalid` のような wildcard-prefixed path にはせず、
この dedicated reason では `npa-check run` の request load validation と同じ
artifact-local field shape を使います。
...
既知 member では `policy.kind`、`policy.path`、`policy.hash` のいずれか、
unknown field では `policy.<unknown_field_name>` です。
`policy.<unknown_field_name>` は unknown field 名が `schema_path_component` に一致する場合だけ使い、
一致しない場合は `field = "policy"` にします。
`expected_value` に schema requirement 名、
`actual_value` に `missing`、`wrong_type`、`unknown_field`、`invalid_enum`、
...
RunnerPolicy validator が unknown raw key を表現できず local containing object path に fallback した場合も、
4.1 の caller prefix rule で `policy` prefix 付きの containing object path に変換します。
...
`request_store.<unknown_field_name>` は unknown field 名が `schema_path_component` に一致する場合だけ使い、
一致しない場合は `field = "request_store"` にします。
`expected_value` に schema requirement 名、
`actual_value` に `missing`、`wrong_type`、`unknown_field`、`invalid_enum`、
`invalid_hash_format`、`invalid_path`、`null_not_allowed`、`order_violation`、
`duplicate_field` のいずれかを入れます。
複数の member-level failure が同時に存在する場合の優先順位は、
`kind`、`path`、`manifest_hash` の schema order で次に固定します。
known member の duplicate、`kind` violation、`path` violation、
`manifest_hash` violation、unknown member の順に検査します。
unknown member が複数ある場合は raw member name の bytewise order で最小のものを返し、
unknown member の duplicate object key はこの unknown member priority 位置で
`expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` として報告します。
duplicate ではない unknown member では
`expected_value = "absent"`、`actual_value = "unknown_field"` にします。
CLI で `--request-store` と `--request-store-hash` の両方が存在し、
path と manifest hash が同時に malformed な場合は `request_store.path` を先に返します。
...
複数の schema / order / duplicate violation が同時に成立する場合は、
store manifest schema / domain validation priority に従います。
...
MVP では `actual_value = "zero_results"` にします。
同一 `checker_profile` が2件以上ある場合は先行する `duplicate_checker_profile_result` で止まるため、
`selector_ambiguous` では `multiple_results` を使いません。
...
3. process_launched = true で、checker_binary_id、checker_binary_hash、checker_id、checker_build_hash のうち
...
8. 下の inconclusive comparison source closed set に該当する result がある
...
comparison status は上の priority で最初に該当した class だけで決めます。
選ばれた class 内では、該当する reason / disagreement を代表1件に丸めず、deterministic order で全件生成します。
選ばれた class より低い priority の reason / disagreement は生成しません。
step 1 では top-level `policy.hash` mismatch、各 `results[*].policy_hash` mismatch、
各 policy 外 `checker_profile` について、それぞれ status reason を生成します。
step 2 では malformed process state 条件に該当しない `error.kind = policy_failure` result すべてについて、
原因 result の policy failure reason を写します。
step 3 と step 4 は checker identity policy class として一体で扱い、どちらかが成立した時点で
両 step の該当 reason を全 launched result / field について生成して `policy_failure` を返します。
step 5 では malformed process state 条件に該当する result すべてについて
`malformed_process_state` reason を生成します。
step 8 では inconclusive comparison source closed set に該当する result すべてについて、
原因 result の infrastructure reason を写します。
生成した `status_reasons` は `status_reason_sort_key` で昇順に並べます。

checker allowlist 照合は `process_launched = true` の result にだけ適用します。
`checker_binary_id` と `checker_binary_hash` は `process_launched = true` の
schema-valid entry では必ず存在するため、missing は comparison ではなく
`normalized_result_schema_invalid` です。
...
inconclusive comparison source closed set は次に限定します。

```text
- process_launched = false, error.kind = timeout,
  error.reason_code = launch_timeout
- process_launched = false, error.kind = resource_exhausted,
  error.reason_code = launch_resource_exhausted
- process_launched = true, error.kind = timeout,
  error.reason_code = checker_timeout
- process_launched = true, error.kind = resource_exhausted,
  error.reason_code = checker_resource_exhausted
- process_launched = true, error.kind = checker_internal_error,
  error.reason_code in:
    checker_reported_internal_error
    malformed_success_output
    success_exit_status_mismatch
    missing_rejection_error
...
上の closed set に含まれない result を step 8 の `inconclusive` として扱ってはいけません。
...
以下をこの文書内の normalized failure key rule と呼びます。
...
```json
{
  ...
  "ai": {
    "agent": "npa-audit-assistant-v1",
    "model": "example-provider/example-model-2026-05-13",
    "prompt_hash": "sha256:..."
  },
  "status": "triaged",
  "classification": {
...
`summary` は non-empty text string です。
`summary` と `suggested_next_actions[]` element は JSON string でなければならず、
empty string、C0 control character `U+0000`-`U+001F`、および `U+007F` を禁止します。
MVP ではこれらの自然言語 field に最大長は設けません。
`suggested_next_actions` が status により required な場合、empty array は
`sidecar_schema_invalid` として扱います。

```text
summary missing / null / type / empty / control character:
  field = "summary"
  expected_value = "non_empty_text_string"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" |
                 "empty_string" | "invalid_string_format"

suggested_next_actions required by status missing / null / type / empty:
  field = "suggested_next_actions"
  expected_value = "non_empty_array_for_status:<status>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "empty_array"

suggested_next_actions optional but malformed:
  field = "suggested_next_actions"
  expected_value = "array"
  actual_value = "null_not_allowed" | "wrong_type"

suggested_next_actions element null / type / empty / control character:
  field = "suggested_next_actions[<i>]"
  expected_value = "non_empty_text_string"
  actual_value = "null_not_allowed" | "wrong_type" |
                 "empty_string" | "invalid_string_format"
```

step 3 priority item 10 の内部では、`status` enum failure を先に評価します。
`status` が valid な場合だけ、status-dependent required field を評価します。
item 10 内で `classification` required と `suggested_next_actions` required / null /
wrong type / empty array が同時に成立する場合は、`classification` を先に報告します。
`suggested_next_actions[]` element の malformed value、
status により optional な `suggested_next_actions` の null / wrong type、
および `summary` の missing / null / wrong type / empty / control character は
step 3 priority item 15 の `その他の field schema failure` として扱います。
これらが unknown field と同時に成立する場合は、step 3 priority item 14 の
unknown field を先に報告します。
...
MVP の `AiAuditSidecar.source` field shape は次で固定します。

```text
source missing / null / type:
  field = "source"
  expected_value = "object"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

source.kind missing / null / type / enum:
  field = "source.kind"
  expected_value = "AiAuditSidecar.source.kind"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"

required source hash member missing / null / type / hash format:
  field = "source.<hash_member>"
  expected_value = "sha256:<lower-hex>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_hash_format"

optional source hash member present but null / type / hash format invalid:
  field = "source.normalized_result_hash"
  expected_value = "sha256:<lower-hex>"
  actual_value = "null_not_allowed" | "wrong_type" | "invalid_hash_format"

source.result_id optional but malformed:
  field = "source.result_id"
  expected_value = "result_id"
  actual_value = "null_not_allowed" | "wrong_type" |
                 "empty_string" | "invalid_string_format"

source.normalized_result_id optional but malformed:
  field = "source.normalized_result_id"
  expected_value = "normalized_result_id"
  actual_value = "null_not_allowed" | "wrong_type" |
                 "empty_string" | "invalid_string_format"
```

required source hash member は、`source.kind = machine_result` では
`result_hash`、`request_hash`、`run_artifact_hash`、
`source.kind = normalized_comparison` では `normalized_result_hash` です。
`source.kind = machine_result` の `source.normalized_result_hash` は optional source hash member です。
`source.result_id` は `source.kind = machine_result` でだけ optional human reference として許可し、
`source.kind = normalized_comparison` では値の shape を検査せず forbidden source member presence として扱います。
`source.normalized_result_id` は両 source kind で optional human reference として許可します。
source id string は `MachineCheckResult.result_id` / `NormalizedCheckResult.normalized_result_id` と
同じ non-empty visible ASCII string で、space、control character、non-ASCII character を禁止します。
malformed な optional source hash / id member は step 3 の `sidecar_schema_invalid` であり、
cross-artifact validation reference requiredness や response metadata の生成には進みません。
...
`actual_value = "missing"` です。
`source.kind = machine_result` で `input_policy.included_fields` に
`input_file_hash` または `expected_certificate_hash` が含まれる場合も、
`source.normalized_result_hash` は required です。
この2 field は `MachineCheckResult` 単体からは解決せず、semantic membership check 済みの
`NormalizedCheckResult.artifact` からだけ解決します。
この rule は、同じ `AiAuditInputPolicy` が union policy として
`normalized_comparison` diagnostic でも使われる場合でも、`machine_result` sidecar には同じように適用します。
MVP の `included_fields` には source-kind qualifier がないため、
producer は `input_file_hash` や `expected_certificate_hash` を
normalized_comparison 専用 field として marker できません。
この required 判定で読む `input_policy.included_fields` は、
`AiAuditSidecar.input_policy.included_fields` の copied metadata です。
step 3 では policy file を読まず、policy file 側の `included_fields` は step 6 の
copied metadata mismatch 検査でだけ使います。
sidecar copied metadata の `included_fields` schema / domain validation は、
`source.normalized_result_hash` の有無に関係なく step 3 の fixed priority で行います。
sidecar copied metadata の `included_fields` が schema-valid かつ allowed / ordered / unique な array として
読めるにもかかわらず
`source.normalized_result_hash` が missing の場合は step 3 の source shape violation として
`sidecar_schema_invalid` を返します。
この場合は `error.field = "source.normalized_result_hash"`、
`expected_value = "required_for_input_field"`、
`actual_value = "missing"` です。
sidecar copied metadata の `included_fields` が schema / domain failure を持つ場合は、
その `sidecar_schema_invalid` を先に返し、`required_for_input_field` 判定は行いません。
この判定は source shape validation なので、schema-only mode でも適用します。
...
4. source.kind ごとの unconditional required source member missing / null / wrong type / invalid hash format
5. source.kind ごとの optional source member present but malformed
6. source.kind = machine_result の `source.normalized_result_hash`
   required_with_source.normalized_result_id missing
7. sidecar copied metadata `input_policy.included_fields` schema / domain failure
8. source.kind = machine_result の `source.normalized_result_hash` required_for_input_field missing
9. source.kind ごとの forbidden source member presence
10. status enum / status-dependent required field
11. static forbidden sidecar field presence
12. classification enum
13. policy-gated field path violation
14. unknown field
15. その他の field schema failure
```

この order では source を解決せずに判定できる shape / schema failure だけを扱います。
item 5 は `source.kind = machine_result` の optional `source.normalized_result_hash`、
source kind 上 optional な `source.result_id` / `source.normalized_result_id` の
null / wrong type / invalid format を扱います。
item 6 は schema-valid な `source.normalized_result_id` が存在し、
`source.normalized_result_hash` が missing の場合だけ扱います。
`source.normalized_result_id` 自体が malformed な場合は item 5 が先に成立します。
item 6 は `input_policy.included_fields` を読まずに判定できる source shape failure なので、
sidecar copied metadata validation より先に報告します。
`source.kind = normalized_comparison` の `source.result_id` は item 9 の
forbidden source member presence であり、値の shape は検査しません。
item 11 の static forbidden sidecar field presence は、
常時 forbidden field と source-kind-dependent static forbidden field の両方を含みます。
...
`AiAuditSidecar.input_policy` は policy file の copied metadata であり、
sidecar-local schema validation では次の field shape を使います。
`input_policy.included_fields` だけは `required_for_input_field` 判定に必要なため、
上の step 3 priority item 7 で先に検査します。
`id`、`version`、`hash`、`redaction` の shape / domain failure は、
より前の priority item が成立しない場合に step 3 priority item 15 の
...
input_policy.hash missing / null / type / format:
  field = "input_policy.hash"
  expected_value = "sha256:<lower-hex>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" |
                 "invalid_hash_format"

input_policy.redaction missing / null / type / enum:
  field = "input_policy.redaction"
  expected_value = "AiAuditInputPolicy.redaction"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_enum"
```
...
`AiAuditSidecar.ai.agent` と `AiAuditSidecar.ai.model` は non-empty visible ASCII string です。
visible ASCII は bytes `0x21`-`0x7E` に限定し、space、control character、
non-ASCII character、empty string は禁止します。
この grammar は文字列の安定性を保証するための syntactic check だけであり、
`ai.model` が mutable alias かどうかの provider lookup は行いません。

```text
ai.agent missing / null / type / empty / grammar:
  field = "ai.agent"
  expected_value = "non_empty_visible_ascii_string"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" |
                 "empty_string" | "invalid_string_format"

ai.model missing / null / type / empty / grammar:
  field = "ai.model"
  expected_value = "non_empty_visible_ascii_string"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" |
                 "empty_string" | "invalid_string_format"

ai.prompt_hash missing / null / type / format:
  field = "ai.prompt_hash"
  expected_value = "sha256:<lower-hex>"
  actual_value = "missing" | "null_not_allowed" | "wrong_type" |
                 "invalid_hash_format"
```
...
MVP の `included_fields` は、AI input builder が sidecar source から取り出してよい
field path の allowlist です。
1つの `AiAuditInputPolicy` を `source.kind = machine_result` と
`source.kind = normalized_comparison` の両方に使ってよいので、field path は
source-kind local namespace として解釈します。
`AiAuditInputPolicy` file 単体の validator は source kind を入力に取らず、
下の machine_result allowed set と normalized_comparison allowed set の union で検査します。
どちらか一方の allowed set に含まれる field path は、policy file 単体では schema-valid です。
input construction と sidecar copied metadata validation では `AiAuditSidecar.source.kind` を使います。
現在の source kind に適用できない field path は input construction では単に使わず、
それだけで sidecar validation failure にしてはいけません。
同じ field path 文字列が machine_result allowed set と normalized_comparison allowed set の
両方に含まれる場合、その field path は現在の `source.kind` の mapping で解決します。
validator や input builder は、同じ文字列を「もう一方の source kind 専用として意図された」と
推測してはいけません。
MVP には `machine_result:input_file_hash` や `normalized_comparison:input_file_hash` のような
source-kind-qualified field path syntax はありません。
ただし `included_fields` に下の union allowed set 外の field path がある場合は、
source kind に関係なく validation failure です。
unqualified `status` は `source.kind = machine_result` でだけ有効で、
`MachineCheckResult.status` を指します。
`source.kind = normalized_comparison` では `comparison.status` を使い、
unqualified `status` から `NormalizedCheckResult.comparison.status` を推測してはいけません。

AI input builder は cross-artifact validation と同じ source resolution を使い、
次の transient projection object を作ってから prompt を組み立てます。
この object は保存 artifact ではありませんが、`ai.prompt_hash` の hash input です。

```text
ai_audit_prompt_input:
  schema = "npa.phase8.ai_audit_prompt_input.v1"
  agent = AiAuditSidecar.ai.agent
  model = AiAuditSidecar.ai.model
  source_kind = AiAuditSidecar.source.kind
  source_hash =
    source.result_hash, if source_kind = machine_result
    source.normalized_result_hash, if source_kind = normalized_comparison
  source_run_artifact_hash =
    source.run_artifact_hash, if source_kind = machine_result
  source_membership_hash =
    source.normalized_result_hash, if source_kind = machine_result and present
  input_policy:
    id
    version
    hash
    ...
    redaction
  fields:
    object whose keys are resolved included_fields field paths
```

上の `schema`、`agent`、`model`、`source_kind`、`source_hash`、
`source_run_artifact_hash`、`source_membership_hash`、`input_policy`、`fields` は
実際の JSON member です。
`source_run_artifact_hash` は `source.kind = machine_result` のときだけ入れ、
`source_membership_hash` は `source.kind = machine_result` かつ sidecar source に
`source.normalized_result_hash` が存在するときだけ入れます。
非該当の member や欠落 optional field を `null` で入れてはいけません。
次は `prompt_hash` 入力 JSON value の最小例です。
実際の hash bytes はこの JSON value を RFC 8785 canonical JSON に変換した bytes です。
...
```json
{
  ...
  "source_membership_hash": "sha256:...",
  "source_run_artifact_hash": "sha256:..."
}
```

`source.kind = normalized_comparison` の場合：
...
```json
{
  ...
  "fields": {
    "artifact_hash": "sha256:...",
    "comparison.status": "all_agree_checked",
    "module": "Nat.Add"
  },
  "input_policy": {
    "hash": "sha256:...",
  ...
  "schema": "npa.phase8.ai_audit_prompt_input.v1",
  "source_hash": "sha256:...",
  "source_kind": "normalized_comparison"
}
```

`fields` object の key は、現在の source kind に適用でき、かつ source artifact から
解決できた `included_fields` だけです。
optional field が source artifact に存在しない場合は `fields` に key を挿入せず、
`null` placeholder を入れてはいけません。
解決できた field path が0件の場合、`fields` は empty object `{}` として入れます。
`fields` member 自体を omit してはいけません。
`fields` object の member name は、解決された `included_fields` element の文字列そのものです。
たとえば `error.kind` は `fields.error.kind` という nested object path ではなく、
`fields` object 直下の literal key `"error.kind"` です。
AI input builder は field path の `.` を JSON object 階層に展開してはいけません。
`fields` value は source artifact から解決した JSON value をそのまま copy します。
hash / name / enum / scalar field は JSON string または integer として copy し、
`comparison.*` の array field は下で定義する通り array 全体を copy します。
MVP では `redaction` は AI input policy の audit / renderer selection label であり、
`fields` value を変換する redaction algorithm ではありません。
`redaction = default`、`strict`、`release` のいずれでも、許可された structured field の
hash、name、enum、integer、array value は同じ JSON value として copy します。
特定の field を AI input から落としたい場合は、`redaction` で値を mask / truncate せず、
`included_fields` からその field path を外します。
将来 `redaction` 値ごとに `fields` value を変換する必要が出た場合は、
既存 `ai_audit_prompt_input.v1` の意味を変えず、新しい projection schema version と
field-level redaction rule を定義します。
sidecar producer は `input_policy` object を選択した `AiAuditInputPolicy` file から作り、
同じ `id`、`version`、`hash`、`included_fields`、`redaction` を
`AiAuditSidecar.input_policy` copied metadata に保存します。
cross-artifact validator は step 6 で copied metadata 一致を確認した後だけ
`prompt_hash` 検証へ進み、projection 内の `input_policy` object は policy file の値から作ります。
`source_run_artifact_hash` は `machine_result` sidecar が説明する exact raw run を
`prompt_hash` に束縛するために常に入れます。
同じ `result_hash` を持つ retry / repeated run でも、`run_artifact_hash` が違えば
`ai.prompt_hash` は異なります。
`fields` object の member order は RFC 8785 canonical JSON rule に任せます。
`ai.prompt_hash` は、この `ai_audit_prompt_input` object の RFC 8785 canonical JSON bytes の
SHA-256 を `sha256:<lower-hex>` で表した値です。
自然言語の wrapper prompt、temperature、retry 回数、または生成された summary /
next actions は `prompt_hash` に含めません。
`AiAuditSidecar.ai.agent` は、この projection を自然言語 prompt に変換する renderer /
template の識別子として扱います。
`agent` は renderer / template の stable versioned id でなければなりません。
自然言語 wrapper prompt、system instruction、field ordering outside canonical JSON、または
projection から実際の model input への変換規則を変える場合は、同じ model を使う場合でも
`agent` 文字列を変更します。
同じ `agent` 文字列で renderer / template の意味を差し替えてはいけません。
`AiAuditSidecar.ai.model` は model identity の stable id です。
human-facing alias や mutable channel name ではなく、provider、model family、snapshot /
release date など、同じ model runtime を再識別できる情報を含めます。
provider が immutable model snapshot id を提供する場合はその id を使います。
provider が mutable alias しか提供しない場合は、sidecar producer が解決時点の
snapshot date または provider-side deployment id を付けた stable string に正規化して保存します。
同じ `model` 文字列で異なる model runtime を指してはいけません。
この stable identity requirement は sidecar producer の生成責務です。
schema-only validator は `ai.model` が required string であることだけを検査し、
`latest`、`default`、または provider 固有 alias が mutable かどうかを判定しません。
cross-artifact validation も `ai.model` の文字列を `ai_audit_prompt_input` と
`prompt_hash` に束縛するだけで、provider へ問い合わせて runtime identity を解決してはいけません。

MVP の `source.kind = machine_result` では、field path を次の source から解決します。
...
certificate_hash:
  MachineCheckResult.certificate_hash, if present
checker_binary_hash:
  MachineCheckResult.checker.binary_hash, if present
checker_binary_id:
  MachineCheckResult.checker.binary_id, if present
checker_build_hash:
  MachineCheckResult.checker.build_hash, if present
checker_id:
  MachineCheckResult.checker.id, if present
checker_profile:
  MachineCheckResult.checker.profile
checker_version:
  MachineCheckResult.checker.version, if present
error.*:
  MachineCheckResult.error.<member>, if present
policy.hash:
  MachineCheckResult.policy.hash
policy.id:
  MachineCheckResult.policy.id
policy.version:
  MachineCheckResult.policy.version
input_file_hash:
  resolved NormalizedCheckResult.artifact.input_file_hash; requires source.normalized_result_hash
expected_certificate_hash:
  resolved NormalizedCheckResult.artifact.expected_certificate_hash; requires source.normalized_result_hash
```

`checker_binary_id` と `checker_binary_hash` は checker process を起動した
`MachineCheckResult` にだけ存在します。
pre-launch failure など `process.launched = false` の source では、これらの field path は
source artifact から解決できない optional field として `fields` から omit し、
AI input construction failure にはしません。
`source.kind = machine_result` で `source.normalized_result_hash` がない sidecar では、
`input_file_hash` と `expected_certificate_hash` を AI input に入れてはいけません。
この2 field が必要な diagnostic では、sidecar source に `source.normalized_result_hash` を入れ、
validator が `NormalizedCheckResult.results[*].result_hash` による semantic membership を
確認できるようにします。

MVP の `source.kind = normalized_comparison` では、field path を次の source から解決します。

```text
module:
  NormalizedCheckResult.artifact.module
input_file_hash:
  NormalizedCheckResult.artifact.input_file_hash
expected_certificate_hash:
  NormalizedCheckResult.artifact.expected_certificate_hash
artifact_hash:
  NormalizedCheckResult.artifact_hash
policy.hash:
  NormalizedCheckResult.policy.hash
policy.id:
  NormalizedCheckResult.policy.id
policy.version:
  NormalizedCheckResult.policy.version
comparison.status:
  NormalizedCheckResult.comparison.status
comparison.disagreements:
  NormalizedCheckResult.comparison.disagreements
comparison.matching_fields:
  NormalizedCheckResult.comparison.matching_fields
comparison.missing_checker_profiles:
  NormalizedCheckResult.comparison.missing_checker_profiles
comparison.status_reasons:
  NormalizedCheckResult.comparison.status_reasons
```

`comparison.disagreements`、`comparison.matching_fields`、
`comparison.missing_checker_profiles`、`comparison.status_reasons` は、検証済み
`NormalizedCheckResult` に保存されている JSON array 全体をそのまま `fields` value に copy します。
AI input builder はこれらの array を並べ替えたり、切り詰めたり、payload member を落としたりしてはいけません。
将来、配列の一部だけを prompt に渡したい場合は、既存 field path の意味を変えず、
新しい field path と schema version を追加します。

MVP の `source.kind = machine_result` で使える field path は次に限定します。
...
MVP の `source.kind = normalized_comparison` で使える field path は次に限定します。
...
`comparison.status_reasons` は `checker_binary_id_mismatch` などの
comparison-generated reason とその fixed payload を AI diagnostic input に含めるための field です。
`checker_binary_id` / `checker_binary_hash` は machine result source では
runner-owned executable identity として許可しますが、`NormalizedCheckResult.results[*]` の
raw result list 全体を AI input として許可する shortcut ではありません。

sidecar copied metadata 側の `input_policy.included_fields` validation でも、
`AiAuditInputPolicy` file と同じ source-kind local namespace の union allowlist を使います。
現在の `AiAuditSidecar.source.kind` に適用できないが、別 source kind では allowed set 内にある
field path は schema-valid です。
その field path は input construction で単に解決されず、`fields` に入りません。
sidecar copied metadata validator は、現在の source kind だけに allowed set を狭めて
`unknown_field` を返してはいけません。

`AiAuditInputPolicy` file 側の `included_fields` validation では、
これらの list にない field path を `input_policy_schema_invalid` として扱います。
これは `sidecar_schema_invalid` ではありません。
この場合の `error.field` は `input_policy.included_fields[<i>]`、
`expected_value = "allowed_input_policy_field"`、
`actual_value = "unknown_field"` です。

sidecar copied metadata 側の `included_fields` validation では、
これらの list にない field path は `sidecar_schema_invalid` として扱います。
...
後続 index です。
sidecar copied metadata 側の `included_fields` field 自体が欠落、explicit null、
または array 以外の場合も `sidecar_schema_invalid` にします。
この場合は `error.field = "input_policy.included_fields"`、
`expected_value = "array"`、
`actual_value = "missing" | "null_not_allowed" | "wrong_type"` です。
`included_fields[]` element が explicit null または string 以外の場合は
`error.field = "input_policy.included_fields[<i>]"`、
`expected_value = "field_path_string"`、
`actual_value = "null_not_allowed" | "wrong_type"` です。
...
`AiAuditInputPolicy JSON path` に unknown field または unknown field の duplicate key を含める場合は、
3 の `schema_path_component` rule を適用します。
raw key を表現できない場合は `error.field = "input_policy"` に fallback します。
...
または `classification` です。
`AuditSidecarValidationResult.error.field` へ写す場合も sidecar-local path をそのまま使い、
root marker `$` は `$` のまま報告します。
known field の duplicate key はその known field の sidecar-local JSON path を使います。
...
それ以外は `sidecar_schema_invalid` です。
top-level 以外の path にある policy-gated field name は step 3 priority item 13 の
policy-gated field path violation として扱います。
top-level path は valid だが value shape が invalid な policy-gated field は
step 3 priority item 15 の `その他の field schema failure` として扱います。

```text
policy-gated top-level field null / type:
  field = "<field>"
  expected_value = "string_or_string_array"
  actual_value = "null_not_allowed" | "wrong_type"
```

string array の element が string 以外または explicit null の場合も
`sidecar_schema_invalid` です。

```text
policy-gated array element null / type:
  field = "<field>[<i>]"
  expected_value = "string"
  actual_value = "null_not_allowed" | "wrong_type"
```

MVP では policy-gated field value の empty string、改行、その他 text content は
hard validation しません。
これらの policy-gated top-level field は sidecar attachment / output field であり、
`ai_audit_prompt_input.v1` の source field ではありません。
`AiAuditInputPolicy.included_fields` に `source_text`、`source_excerpt`、
`theorem_statement`、`proof_script`、`tactic_trace`、`tactic_script`、
`elaboration_trace`、`ai_search_trace` を入れてはいけません。
AI input builder は `allow_source_text = true` または `allow_tactic_trace = true` でも、
これらの sidecar field を model input に送ってはいけません。
したがって policy-gated field value は `ai.prompt_hash` の hash input にも含めません。
将来 source text / tactic trace を AI model input に含める必要がある場合は、
既存 `ai_audit_prompt_input.v1` の意味を変えず、新しい projection schema version、
explicit field path、source mapping、redaction rule を定義します。
...
この例外は `source_text`、`source_excerpt`、`theorem_statement`、`proof_script`、
`tactic_trace`、`tactic_script`、`elaboration_trace`、`ai_search_trace` の8 field name
だけに適用し、それ以外の未定義 field は常に generic unknown field です。
...
exact selected raw result との一致は release audit bundle validator の closed-set rule、
または CI diagnostic required-sidecar binding step だけが検査します。
...
- source.kind = normalized_comparison の場合、source.result_hash / source.request_hash / source.run_artifact_hash / source.result_id は存在しない
...
- cross-artifact validation では `ai_audit_prompt_input` を再計算し、ai.prompt_hash と一致する
...
- sidecar に secret token などの常時 forbidden field が含まれない
- cross-artifact validation では、policy で禁止された top-level policy-gated field が含まれない
- schema-only validation では、top-level policy-gated field は値の shape だけを検査し、policy flag による禁止判定は行わない
```
...
schema-only mode は JSON schema、sidecar-local source shape、常時 forbidden field、
source-kind-dependent static forbidden field、policy-gated field path violation、status enum、
status ごとの required field、classification enum、hash 文字列の構文、
および sidecar copied metadata だけで判定できる `input_policy.included_fields` の
local schema / domain と `required_for_input_field` を検査します。
source hash、input_policy hash、store manifest、または policy file の実在確認は行いません。
...
- ai.prompt_hash が再計算した `ai_audit_prompt_input` projection hash と一致するか
- input_policy.hash が policy file と一致するか
- policy で禁止された top-level policy-gated field が sidecar に含まれるか
```
...
sidecar status 許可違反の `expected_value` に使う許可 status set 名は次で固定します。

```text
sidecar_status:summarized_only
  allowed values: summarized
  used for:
    - source.kind = machine_result and referenced MachineCheckResult.status = checked
    - source.kind = normalized_comparison and referenced comparison.status =
      all_agree_checked or all_agree_failed

sidecar_status:triage_or_summary
  allowed values: summarized, triaged, suggested_fix, suggested_challenge, inconclusive
  used for:
    - source.kind = machine_result and referenced MachineCheckResult.status = failed
    - source.kind = normalized_comparison and referenced comparison.status =
      disagreement, missing_checker_result, policy_failure, or inconclusive
```
...
`ChallengeManifest` manifest-local JSON path の root marker は `$` です。
manifest-local containing object path は `$`、`imports`、`base_certificate`、
`mutated_certificate`、`mutation`、`outcome_hint`、`replay`、または `generated_by` です。
caller field に prefix する場合、root marker `$` は suffix から除きます。
たとえば manifest-local `$` は caller root field の `challenge_manifest` または
`challenge_output_store.entries[].manifest` に写し、manifest-local `mutation` は
`challenge_manifest.mutation` または `challenge_output_store.entries[].manifest.mutation` に写します。
...
`ChallengeOutputStoreManifest` schema / domain validation priority は次で固定します。
複数の violation が同時に存在する場合は、最初に該当したものだけを返します。

```text
1. top-level object type
2. duplicate top-level known field, in the order schema, entries
3. top-level schema missing / null / type / mismatch
4. entries array missing / null / type
5. entry object type, by smaller entry index
6. duplicate entry known field, by smaller entry index and entry field order
7. entry required member / null / type / value shape, by smaller entry index and entry field order
8. unknown entry field, by smaller entry index and bytewise field name order
9. unknown top-level field, by bytewise field name order
10. entries order violation
11. unique key duplicate
```

schema shape failure の field / payload は次で固定します。

```text
top-level object type:
  field = "challenge_output_store"
  expected_value = "object"
  actual_value = "wrong_type" | "null_not_allowed"

duplicate top-level known field:
  field = "challenge_output_store.schema" or "challenge_output_store.entries"
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

top-level schema missing / null / type:
  field = "challenge_output_store.schema"
  expected_value = "npa.phase8.challenge_output_store_manifest.v1"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

top-level schema mismatch:
  field = "challenge_output_store.schema"
  expected_value = "npa.phase8.challenge_output_store_manifest.v1"
  actual_value = actual schema string

entries array missing / null / type:
  field = "challenge_output_store.entries"
  expected_value = "array"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

entry object type:
  field = "challenge_output_store.entries[<i>]"
  expected_value = "object"
  actual_value = "wrong_type" | "null_not_allowed"

duplicate entry known field:
  field = "challenge_output_store.entries[<i>].<member>"
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

entry required member / null / type:
  field = "challenge_output_store.entries[<i>].<member>"
  expected_value = member expected_value from the table below
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

entry value shape:
  field = "challenge_output_store.entries[<i>].<member>"
  expected_value = member expected_value from the table below
  actual_value = member actual_value from the table below
```

entry field order と value shape は次で固定します。
...
unknown field の duplicate object key は unknown field priority 位置で報告し、
`expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` にします。
duplicate ではない unknown field では
`expected_value = "absent"`、`actual_value = "unknown_field"` にします。
entries order violation の `<i>` は、最初に `entries[<i>].challenge_id` が
直前 entry より小さくなる後続 entry index です。
隣接 `challenge_id` が等しい場合は `order_violation` ではなく
`duplicate_challenge_id` として扱います。
entries order violation では `field = "challenge_output_store.entries[<i>]"`、
`expected_value = "challenge_id_bytewise_ascending"`、
`actual_value = "order_violation"` にします。
unique key duplicate の `<i>` は、同じ key がすでに出現している最小の後続 entry index です。
同じ `<i>` で複数の unique key duplicate が成立する場合は
`challenge_id`、`manifest_path` の順で最初の key を返します。
`duplicate_challenge_id` では `field = "challenge_output_store.entries[<i>].challenge_id"`、
`expected_value = "unique_challenge_ids"`、`actual_value = "duplicate_challenge_id"` にします。
`duplicate_manifest_path` では `field = "challenge_output_store.entries[<i>].manifest_path"`、
`expected_value = "unique_manifest_paths"`、`actual_value = "duplicate_manifest_path"` にします。
...
`duplicate_field` のいずれかを入れます。
unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合は、
raw key を copy せず containing object path を使います。
`ChallengeGenerationRequest` の containing object path は `$`、`imports`、`base_certificate`、
`mutation`、`output`、または `generated_by` です。
...
`actual_value` に `missing`、`wrong_type`、`unknown_field`、
`invalid_hash_format`、`invalid_path`、`null_not_allowed`、`order_violation`、
`duplicate_challenge_id`、`duplicate_manifest_path`、`duplicate_field` のいずれか、
または top-level schema mismatch の実 schema string を入れます。
unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合は、
raw key を copy せず `field = "challenge_output_store"` または
`field = "challenge_output_store.entries[<i>]"` にします。
...
manifest-local unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合は、
raw key を copy せず `field = "challenge_output_store.entries[].manifest"` または
`challenge_output_store.entries[].manifest.<manifest-local containing object path without root marker>` を使います。
manifest-local containing object path が root marker `$` の場合は
`field = "challenge_output_store.entries[].manifest"` にします。
...
`invalid_hash_format`、`invalid_name_format`、`invalid_path`、`null_not_allowed`、`order_violation`、
`duplicate_field` のいずれかを入れます。
この field は caller-prefixed ChallengeManifest path とします。
ChallengeManifest manifest-local `$` は `challenge_manifest` に置換し、
manifest-local root 以外の path は `challenge_manifest.<manifest-local path>` とします。
manifest-local unknown field または duplicate unknown field 名を `schema_path_component` で
表現できない場合は、raw key を copy せず `field = "challenge_manifest"` または
`challenge_manifest.<manifest-local containing object path without root marker>` を使います。
manifest-local containing object path が `$` の場合は `field = "challenge_manifest"` にします。
`policy_reference_invalid` では challenge 系 command 共通の policy reference field shape に従います。
`policy_file_unreadable` では `field = "policy.path"`、`actual_value = "unreadable"` にします。
`policy_hash_mismatch` では `field = "policy.hash"`、
`expected_hash` に caller 指定 hash、`actual_hash` に読み込んだ `RunnerPolicy` の canonical hash を入れます。
...
replay store manifest の local `$` は `replay_store` に置換します。
replay store manifest の unknown field または duplicate unknown field 名を
`schema_path_component` で表現できない場合は、raw key を copy せず
`field = "replay_store"` または `field = "replay_store.results[<i>]"` を使います。
...
その他の unique key duplicate reason の `expected_value` / `actual_value` は次で固定します。

```text
duplicate_request_hash:
  expected_value = "unique_request_hashes"
  actual_value = "duplicate_request_hash"

duplicate_run_artifact_hash:
  expected_value = "unique_run_artifact_hashes"
  actual_value = "duplicate_run_artifact_hash"

duplicate_normalized_result_hash:
  expected_value = "unique_normalized_result_hashes"
  actual_value = "duplicate_normalized_result_hash"

duplicate_result_hash:
  expected_value = "unique_result_hashes"
  actual_value = "duplicate_result_hash"

duplicate_path:
  expected_value = "unique_paths"
  actual_value = "duplicate_path"
```
...
`non_positive_integer`、`integer_out_of_range`、`order_violation`、`duplicate_field`、
`duplicate_axiom_name`、`forbidden_field` のいずれかを入れます。
...
parsed `MachineCheckRequest.request_hash` self-hash mismatch では
`expected_hash` に parsed artifact から再計算した request hash、
`actual_hash` に parsed `MachineCheckRequest.request_hash` を入れます。
manifest entry comparison では `expected_hash` に manifest entry の `request_hash`、
`actual_hash` に parsed `MachineCheckRequest.request_hash` を入れます。
...
parsed `MachineCheckResult.result_hash` / `run_artifact_hash` self-hash mismatch では
`expected_hash` に parsed artifact から再計算した hash、
`actual_hash` に parsed `MachineCheckResult` の同じ self-hash field を入れます。
manifest entry comparison では `expected_hash` に manifest entry の hash、
`actual_hash` に parsed `MachineCheckResult` の同じ field を入れます。
...
parsed `NormalizedCheckResult.artifact_hash` / `normalized_result_hash` self-hash mismatch では
`expected_hash` に parsed artifact から再計算した hash、
`actual_hash` に parsed `NormalizedCheckResult` の同じ self-hash field を入れます。
manifest entry comparison では `expected_hash` に manifest entry の hash、
`actual_hash` に parsed `NormalizedCheckResult` の同じ field を入れます。
...
unknown field 名を `schema_path_component` で表現できない場合は raw key を copy せず、
top-level object では `field = "$"`、`ai_triage` object では `field = "ai_triage"` にします。
...
MVP の `ReleasePolicy` は `ai_triage.input_policy_hash` を1つだけ持ちます。
同じ ReleasePolicy の下で生成・検証する required / optional AI sidecar は、
`source.kind = machine_result` と `source.kind = normalized_comparison` のどちらであっても
同じ `AiAuditInputPolicy` file / hash を使わなければなりません。
machine_result と normalized_comparison の両方を診断する運用では、
両 source kind に必要な field path を含む単一の union policy を
`ai_triage.input_policy_hash` として指定します。
union policy に `input_file_hash` または `expected_certificate_hash` を含める場合、
その field path は `machine_result` sidecar にも適用されます。
したがって同じ ReleasePolicy 配下の `machine_result` sidecar は、
7 の rule により `source.normalized_result_hash` を持たなければ valid になりません。
release audit bundle の reproducibility repeated raw result sidecar は
`source.normalized_result_hash` が forbidden なので、この2 field を含む union policy では
valid な optional repeated-result sidecar として bundle に含められません。
その optional sidecar を含めたい運用では、ReleasePolicy の single policy から
`input_file_hash` / `expected_certificate_hash` を外すか、その sidecar を bundle に含めません。
MVP では source kind ごとに別々の input policy hash を `ReleasePolicy` へ指定する field はありません。
`ai_triage.enabled = false` の場合、`ai_triage.required` は false でなければなりません。
`ai_triage.enabled = true` かつ `ai_triage.required = true` の場合、
該当 mode の copied status = failed `machine_result` target、または
comparison.status が `disagreement` / `missing_checker_result` / `policy_failure` /
`inconclusive` の `normalized_comparison` target には required AI sidecar diagnostic と
...
`AiSidecarDiagnosticResult.status = failed` とし、その target の checker / comparison status は変えません。
materialized `ReleaseAuditBundleManifest` では required AI sidecar target set は空でなければなりません。
つまり `required = true` は copied status = failed `machine_result` target と、
`disagreement` / `missing_checker_result` / `policy_failure` / `inconclusive`
`normalized_comparison` target の説明 artifact 完全性を gate する設定であり、
pass している release audit bundle に追加の required sidecar を要求する設定ではありません。
release pass pipeline で required diagnostic target set が空なら、AI sidecar の欠落だけで
release audit bundle generation を失敗させてはいけません。
...
machine_result target:
  CI diagnostic target NormalizedCheckResult.results[] entry のうち、
  copied status = failed の entry 自体。
  selected raw MachineCheckResult resolution は target derivation ではなく、
  target satisfaction 内の後続 validation step で行う。
...
CI diagnostic gate は `ci_diagnostic_targets[]` に複数の distinct
`NormalizedCheckResult` target context を受け取ってよく、pre-candidate validation が通れば
各 distinct entry につき `AiSidecarDiagnosticResult` を1個返します。
ただし同じ `artifact_hash` または同じ `normalized_result_hash` を持つ target context が
複数件ある場合、CI diagnostic gate は
`AiSidecarDiagnosticEvaluationFailure.reason_code = ci_diagnostic_target_duplicate` を返します。
release audit bundle validation 中に同じ状態を検出した場合は bundle invalid です。
...
必ず空です。
release target に copied status = failed の `NormalizedCheckResult.results[]` entry、
comparison.status が `disagreement` / `missing_checker_result` / `policy_failure` /
`inconclusive`、または
...
diagnostic target では `all_agree_failed` を構成する copied-status-failed
`NormalizedCheckResult.results[]` entry に対応する selected raw `MachineCheckResult` も
...
この節の raw result は unnormalized `MachineCheckResult` artifact を指し、
checker process が出力する `CheckerRawResult` ではありません。
release bundle optional `machine_result` source の selected raw result set は、
13章の release target raw result selection rule で導出します。
baseline result entry、つまり `RunnerPolicy.required_checker_profiles[0]` に対応する entry は、
required reproducibility `AuxiliaryResult.selector.baseline_run_artifact_hash` が指す
`MachineCheckResult` だけを selected raw result とします。
baseline result entry を `(checker_profile, request_hash, result_hash, policy_hash)` で
bundle 内の raw result から選び直してはいけません。
non-baseline の release target `results[*]` entry は、
release target raw result selection rule と同じ
`checker_profile`、`request_hash`、`result_hash`、`policy_hash` の組で
bundle 内の raw result に一意に解決できなければなりません。
raw `MachineCheckResult` 側の `policy_hash` component は
top-level field ではなく `MachineCheckResult.policy.hash` から取ります。
解決結果が0件または複数件の場合、release audit bundle generation failure、
または bundle invalid です。
この4要素 tuple は non-baseline release target raw result selection と CI diagnostic target で
selected raw result set を導出するための bundle / CI 内部 key であり、
`AiAuditSidecar.source` の field ではありません。
この tuple の serialized order は
`(checker_profile, request_hash, result_hash, policy_hash)` に固定します。
mismatch member の報告優先順位は、各 rule で別途定義する equality check order に従います。
`source.kind = machine_result` sidecar は `source.policy_hash` を持たず、導出済み selected raw result set
または reproducibility repeated raw result set に `source.run_artifact_hash` で membership check されます。
membership 後に `source.result_hash` と `source.request_hash` を resolved raw result と照合します。
CI diagnostic required `machine_result` sidecar source では、
release target raw result selection rule の baseline / reproducibility selector branch を使いません。
copied status = failed の `NormalizedCheckResult.results[]` entry から
`(checker_profile, request_hash, result_hash, policy_hash)` を取り、
対応する `ci_diagnostic_targets[<n>].normalizer_machine_results[]` から selected raw `MachineCheckResult` を
一意に解決して selected raw result set を導出します。
required diagnostic の sidecar candidate cardinality では `source.run_artifact_hash` を使いません。
candidate は `source.kind = machine_result`、`source.request_hash`、`source.result_hash`、
および存在する場合の `source.normalized_result_hash` で選びます。
`source.run_artifact_hash` は candidate がちょうど1件に絞れた後でだけ、
selected raw `MachineCheckResult.run_artifact_hash` との exact retry binding として検査します。
wrong retry を参照する sidecar が唯一の candidate である場合は、
`required_ai_sidecar_missing` ではなく `required_ai_sidecar_source_mismatch` です。
CI diagnostic required `machine_result` sidecar binding は、
通常の `npa-check audit-sidecar validate` による cross-artifact validation だけでは完了しません。
`AuditSidecarValidationResult.status = valid` は `source.run_artifact_hash` が result store に存在し、
`source.result_hash` / `source.request_hash` と semantic normalized membership が正しいことを示すだけです。
CI diagnostic step は追加で、対象 target から導出した selected raw `MachineCheckResult.run_artifact_hash` と
`AiAuditSidecar.source.run_artifact_hash` が完全一致することを検査します。
同じ `source.result_hash` / `source.request_hash` を持つ別 retry の `run_artifact_hash` を参照する sidecar は、
cross-artifact validation が valid でも、その required target を満たしません。
この binding failure は `AuditSidecarValidationResult` を failed に書き換えず、
AI sidecar diagnostic step の target satisfaction failure として扱います。
wrong retry source の固定 diagnostic shape は、下の
`AiSidecarDiagnosticResult.error.reason_code = required_ai_sidecar_source_mismatch`
で定義します。
...
その値は、CI diagnostic required sidecar では CI diagnostic target
`NormalizedCheckResult.normalized_result_hash`、release bundle optional sidecar では
release target `NormalizedCheckResult.normalized_result_hash` と一致し、
かつその target の `results[*].result_hash` に `source.result_hash` が含まれていなければなりません。
...
CI diagnostic gate の response envelope は pre-candidate validation の結果で分岐します。
target context validation、diagnostic artifact list entry validation、
saved diagnostic result validation がすべて成功した場合だけ、
CI diagnostic step は現在の `ci_diagnostic_targets[]`、`ai_sidecars[]`、
`audit_sidecar_validation_results[]` から deterministic に再計算した、
`ci_diagnostic_targets[]` と同じ order の
`AiSidecarDiagnosticResult` sequence を返します。
sequence length は `ci_diagnostic_targets[]` length と一致し、
entry `<n>` は `ci_diagnostic_targets[<n>].artifact.normalized_result_hash` を
`normalized_result_hash` に写します。
`ci_diagnostic_targets[]` が1件の場合でも、normative return value はこの sequence です。
`ai_sidecar_diagnostic_results[]` に入った保存済み result は、この sequence の再計算を省略する
oracle ではありません。
pre-candidate validation に失敗した場合は、gate 全体の単一
`AiSidecarDiagnosticEvaluationFailure` を返し、`AiSidecarDiagnosticResult` は合成しません。
MVP ではこの step 用の standalone `npa-check` subcommand / HTTP endpoint は定義しないため、
wire-level JSON wrapper 名は仕様外です。
ただし CI driver または library API が required AI sidecar gate を評価する場合、normative result value は
target satisfaction では ordered sequence of `AiSidecarDiagnosticResult`、
pre-candidate validation failure では単一 `AiSidecarDiagnosticEvaluationFailure` であり、
`CommandError` や `AuditSidecarValidationResult` ではありません。
MVP では CI diagnostic store manifest schema を定義しません。
diagnostic step の normative input は、CI driver / library API が渡す explicit ordered target context と
diagnostic artifact lists です。
この gate は raw JSON wrapper を直接受け取る command / HTTP endpoint ではなく、typed library input を
前提にします。
そのため `ci_diagnostic_targets`、`ai_sidecars`、`audit_sidecar_validation_results`、
`ai_sidecar_diagnostic_results` という list container 自体の missing / null / wrong type、
または entry object 自体の wrong type は、この gate の
`AiSidecarDiagnosticEvaluationFailure` reason code に割り当てません。
ここでいう typed library input とは、list container の存在、entry record の存在、
および caller-specified stable order が typed API boundary で保証されるという意味です。
各 entry record 内の `path` / `file_hash` metadata は、typed container に載った
untrusted scalar value としてこの gate が shape validation します。
実装が `path` / `file_hash` を strong type へ正規化してから gate に渡す場合、
下の metadata schema invalid は gate 内では到達不能にしてよいですが、
raw JSON wrapper / 保存済みログ adapter は同じ field shape に map できる境界で
正規化失敗を検査しなければなりません。
diagnostic artifact list entry の normative pre-validation input は `path` と `file_hash` です。
下の list shape に書く `artifact` member は、explicit ordered list entry validation が
`path` の file bytes を読み、`file_hash` と照合し、JSON parse と artifact / response schema validation に
成功した後の post-validation parsed object value を指します。
target satisfaction と pass evaluation はこの post-validation `artifact` だけを使います。
caller が別途渡した parsed object を、`path` / `file_hash` の bytes 検証結果と独立した正本として
使ってはいけません。
実装が entry validation を wrapper / 保存済みログ adapter 側で先に実行し、pure gate core に
pre-resolved `artifact` を渡す場合でも、adapter は同じ file read / hash / JSON / schema validation を
完了していなければなりません。
その contract を満たさずに `artifact` missing / null / wrong type / 未 parse sentinel、
または `path` / `file_hash` の bytes から導出していない parsed object を pure gate core に渡すことは
typed API contract violation であり、gate 起動前に失敗させ、
`AiSidecarDiagnosticEvaluationFailure` を返してはいけません。
raw JSON wrapper / 保存済みログ adapter が file read / JSON parse / artifact schema validation に
失敗した場合は、下の explicit ordered list entry validation の対応する reason code に map します。
raw JSON wrapper や保存済みログ形式からこの gate を呼ぶ実装は、wrapper / container / entry object
shape を gate 起動前に検証し、実装固有の API / command wrapper error として扱わなければなりません。
gate に渡された typed entry record の `path` / `file_hash` metadata shape と referenced artifact
validation だけを、下の explicit ordered list entry validation として扱います。

```text
ci_diagnostic_targets[]:
  prevalidated target context entries, in caller-specified stable order.
  each entry has:
    - artifact: parsed NormalizedCheckResult object
    - normalizer_machine_results[]:
        prevalidated MachineCheckResult objects in the exact order passed to the normalizer
        when this NormalizedCheckResult was produced
        each nested entry has:
          - artifact: parsed MachineCheckResult object

  target entries must be unique by artifact.artifact_hash and artifact.normalized_result_hash.
  If both duplicate conditions hold, artifact.artifact_hash duplicate is reported first.
  Multiple distinct entries are allowed in one gate invocation.

ai_sidecars[]:
  resolved AiAuditSidecar entries, in caller-specified stable order.
  each entry has:
    - path: required workspace-relative file path
    - file_hash: required sha256 of the sidecar file bytes
    - artifact: parsed AiAuditSidecar object after file / JSON / schema validation succeeds

audit_sidecar_validation_results[]:
  resolved AuditSidecarValidationResult entries, in caller-specified stable order.
  each entry has:
    - path: required workspace-relative file path
    - file_hash: required sha256 of the response file bytes
    - artifact: parsed AuditSidecarValidationResult object after file / JSON / response schema validation succeeds

ai_sidecar_diagnostic_results[]:
  resolved AiSidecarDiagnosticResult entries used by pass evaluation,
  in caller-specified stable order.
  each entry has:
    - path: required workspace-relative file path
    - file_hash: required sha256 of the response file bytes
    - artifact: parsed AiSidecarDiagnosticResult object after file / JSON / response schema validation succeeds
```

診断ログとして file に保存してよいですが、その保存形式は MVP 仕様外です。
保存済みログを CI pass evaluation に使う実装は、まず上の diagnostic artifact lists に展開しなければなりません。
`ReleaseAuditBundleManifest` の artifact には含めません。
`ci_diagnostic_targets[]` と nested `normalizer_machine_results[]` は prevalidated target context であり、
この gate はそれらの file / hash / JSON / schema validation failure を
`AiSidecarDiagnosticEvaluationFailure` に割り当てません。
ここでいう prevalidated には、`NormalizedCheckResult` と nested `MachineCheckResult` の
schema / domain validation、`artifact_hash` / `normalized_result_hash` /
`result_hash` / `run_artifact_hash` self-consistency validation、
および normalizer input としての採用順序確定が含まれます。
caller が prevalidated context を用意できない場合は、この gate を起動する前に
既存の artifact validation / normalization error として失敗させなければなりません。
diagnostic artifact list entry の `path` は file read と field reporting のために required ですが、
matching key や pass identity には使いません。
`file_hash` は entry file bytes から計算した値でなければならず、artifact 内に存在しない
`AiAuditSidecar.file_hash` などの field から読んではいけません。
required diagnostic target の matching で使う sidecar file hash は
`ai_sidecars[<i>].file_hash` です。

explicit ordered list entry の file readable / file_hash / JSON / schema validation に失敗した場合、
または matching saved diagnostic result の `target_count` が derived target list length と一致しない場合、
CI diagnostic gate は `AiSidecarDiagnosticResult` を合成せず、次の transient response を返します。
ここでいう explicit ordered list entry validation は、entry metadata の `path` / `file_hash`
shape、file readable、file bytes hash、JSON parse、artifact / response schema validation です。
ここでいう saved diagnostic result validation は、entry metadata validation、file / hash / JSON /
response schema validation と、matching saved diagnostic result の `target_count`
pre-candidate check だけです。
保存済み `AiSidecarDiagnosticResult` と再計算された `AiSidecarDiagnosticResult` の
canonical 一致検査は pre-candidate validation ではなく、下の pass evaluation で行います。

```json
{
  "schema": "npa.phase8.ai_sidecar_diagnostic_evaluation_failure.v1",
  "status": "failed",
  "reason_code": "ai_sidecar_diagnostic_input_schema_invalid",
  "field": "ai_sidecar_diagnostic[0].artifact.status",
  "expected_value": "AiSidecarDiagnosticResult.status",
  "actual_value": "invalid_enum"
}
```

`AiSidecarDiagnosticEvaluationFailure` は saved artifact ではなく、`result_hash` を持ちません。
`schema`、`status`、`reason_code`、`field` は required で、
`status` は `failed` に固定します。
`expected_hash`、`actual_hash`、`expected_value`、`actual_value` は該当する payload shape の場合だけ入れます。
`AiSidecarDiagnosticEvaluationFailure` は closed-world JSON object です。
top-level field は `schema`、`status`、`reason_code`、`field`、
`expected_hash`、`actual_hash`、`expected_value`、`actual_value` だけです。
`schema` は `npa.phase8.ai_sidecar_diagnostic_evaluation_failure.v1` に固定します。
unknown field は forbidden です。
MVP の `reason_code` は次に限定します。

```text
- ai_sidecar_input_unreadable
- ai_sidecar_input_hash_mismatch
- ai_sidecar_input_json_invalid
- ai_sidecar_input_schema_invalid
...
- ai_sidecar_diagnostic_input_unreadable
- ai_sidecar_diagnostic_input_hash_mismatch
- ai_sidecar_diagnostic_input_json_invalid
- ai_sidecar_diagnostic_input_schema_invalid
- ci_diagnostic_target_duplicate
- ai_sidecar_diagnostic_target_count_mismatch
```

`AiSidecarDiagnosticEvaluationFailure` の payload shape は `reason_code` ごとに次で固定します。
表の required payload members 以外の `expected_hash`、`actual_hash`、`expected_value`、
`actual_value` は forbidden です。

```text
file unreadable reason_code:
  - ai_sidecar_input_unreadable
  - audit_sidecar_validation_input_unreadable
  - ai_sidecar_diagnostic_input_unreadable
  required payload: field, expected_value, actual_value
  expected_value = "readable_file"
  actual_value = "unreadable"

file hash mismatch reason_code:
  - ai_sidecar_input_hash_mismatch
  - audit_sidecar_validation_input_hash_mismatch
  - ai_sidecar_diagnostic_input_hash_mismatch
  required payload: field, expected_hash, actual_hash

JSON invalid reason_code:
  - ai_sidecar_input_json_invalid
  - audit_sidecar_validation_input_json_invalid
  - ai_sidecar_diagnostic_input_json_invalid
  required payload: field, expected_value, actual_value
...
explicit list entry metadata schema invalid reason_code:
  - ai_sidecar_input_schema_invalid
  - audit_sidecar_validation_input_schema_invalid
  - ai_sidecar_diagnostic_input_schema_invalid
  required payload: field, expected_value, actual_value
  path metadata:
    field = "ai_sidecar[<i>].path" |
            "audit_sidecar_validation[<j>].path" |
            "ai_sidecar_diagnostic[<k>].path"
    expected_value = "workspace_relative_path"
    actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_path"
  file_hash metadata:
    field = "ai_sidecar[<i>].file_hash" |
            "audit_sidecar_validation[<j>].file_hash" |
            "ai_sidecar_diagnostic[<k>].file_hash"
    expected_value = "sha256:<lower-hex>"
    actual_value = "missing" | "null_not_allowed" | "wrong_type" | "invalid_hash_format"

artifact / response schema / domain invalid reason_code:
  - ai_sidecar_input_schema_invalid
  - audit_sidecar_validation_input_schema_invalid
  - ai_sidecar_diagnostic_input_schema_invalid
  required payload: field plus the delegated artifact validation payload members
  delegated payload family:
    - actual_value-only payload: actual_value
    - value payload: expected_value, actual_value
    - hash payload: expected_hash, actual_hash
  対象 artifact の schema / domain validation が返す payload family をそのまま使い、
  下の prefix rule で `field` だけ置換する。
  delegated artifact validation が `expected_value` を持たない actual_value-only payload を
  返す場合、`AiSidecarDiagnosticEvaluationFailure` でも `expected_value` を合成してはいけない。

target_count mismatch reason_code:
  - ai_sidecar_diagnostic_target_count_mismatch
  required payload: field, expected_value, actual_value
  field = "ai_sidecar_diagnostic[<k>].artifact.target_count"
  expected_value = derived target list length as decimal string
  actual_value = AiSidecarDiagnosticResult.target_count as decimal string

CI diagnostic target duplicate reason_code:
  - ci_diagnostic_target_duplicate
  required payload: field, expected_value, actual_value
  artifact_hash duplicate:
    field = "ci_diagnostic_targets[<n>].artifact.artifact_hash"
    expected_value = "unique_ci_diagnostic_target_artifact_hash"
  normalized_result_hash duplicate:
    field = "ci_diagnostic_targets[<n>].artifact.normalized_result_hash"
    expected_value = "unique_ci_diagnostic_target_normalized_result_hash"
  actual_value = "duplicate"
```

`AiSidecarDiagnosticEvaluationFailure` 自体の schema / domain failure を検査する場合は、
次の順序で最初の1件だけを返します。

```text
1. top-level object type
2. duplicate known field, in top-level field order
3. schema required / null / type / mismatch
4. status required / null / type / fixed-value mismatch
5. reason_code required / null / type / invalid enum
6. field required / null / type
7. reason_code-specific required payload members
8. reason_code-specific forbidden payload members
9. duplicate unknown field, then unknown field, by bytewise field name order within each group
```

top-level field order は `schema`、`status`、`reason_code`、`field`、
`expected_hash`、`actual_hash`、`expected_value`、`actual_value` です。
payload member validation order は `field`、`expected_hash`、`actual_hash`、
`expected_value`、`actual_value` です。
`schema` failure では `expected_value =
"npa.phase8.ai_sidecar_diagnostic_evaluation_failure.v1"`、
`status` failure では `expected_value = "failed"`、
`reason_code` failure では
`expected_value = "AiSidecarDiagnosticEvaluationFailure.reason_code"`、
`field` failure では `expected_value = "field_path"` を使います。
`schema`、`status`、`reason_code`、`field` の missing / null / wrong type では、
`actual_value = "missing"`、`"null_not_allowed"`、または `"wrong_type"` を使います。
`schema`、`status`、`reason_code` が string だが固定値または enum と一致しない場合は
`actual_value = "invalid_enum"` を使います。
`field` が string だが、選択された `reason_code` / payload shape で定義された field path ではない場合は
`field = "field"`、`expected_value = "field_path_for_reason_code"`、
`actual_value = "unknown_field"` を使います。
reason-code-specific required payload member の missing / null / wrong type では、
その payload member path を `field` に入れ、
`actual_value = "missing"`、`"null_not_allowed"`、または `"wrong_type"` を使います。
`expected_hash` / `actual_hash` の schema failure では
`expected_value = "sha256:<lower-hex>"`、
`expected_value` / `actual_value` の schema failure では
`expected_value = "deterministic_json_scalar"` を使います。
hash payload member の invalid hash format では `actual_value = "invalid_hash_format"` を使います。
forbidden payload member では `field = "<member>"`、
`expected_value = "absent_for_reason_code"` または `"absent_for_payload_shape"`、
`actual_value = "forbidden_field"` を使います。
duplicate known field では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。
unknown field では `expected_value = "absent"`、`actual_value = "unknown_field"` を使います。
unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合は、
raw key を copy せず `field = "$"` を使います。

pre-candidate validation は candidate selection より前に実行し、次の順序で最初の1件だけを返します。

```text
1. ai_sidecars[] entry metadata / file / hash / JSON / artifact schema validation, by smaller index
2. audit_sidecar_validation_results[] entry metadata / file / hash / JSON / response schema validation, by smaller index
3. ai_sidecar_diagnostic_results[] entry metadata / file / hash / JSON / response schema validation, by smaller index
4. ci_diagnostic_targets[] duplicate artifact_hash, by smaller later index
5. ci_diagnostic_targets[] duplicate normalized_result_hash, by smaller later index
6. matching ai_sidecar_diagnostic_results[] target_count mismatch, by smaller index
```

`ci_diagnostic_target_duplicate` の `<n>` は、同じ key を持つ2件目以降の
duplicate target のうち最小 index です。
item 6 の matching saved diagnostic result とは、schema / domain valid で、
`policy_hash` が active `ReleasePolicy` の canonical hash と一致し、
`input_policy_hash` が `ReleasePolicy.ai_triage.input_policy_hash` と一致し、
`normalized_result_hash` が一意な `ci_diagnostic_targets[]` entry に一致する
`ai_sidecar_diagnostic_results[]` entry です。
対応する target の derived diagnostic target set が empty の場合、
その saved diagnostic result は pass evaluation 用候補ではないため item 6 では無視します。
上の key が一致しない schema / domain valid な saved diagnostic result も、
target_count mismatch、pass / fail、duplicate 判定の対象外として ignore します。
ただしその entry 自体の file / hash / JSON / schema validation は item 3 で常に評価します。
`policy_hash` / `input_policy_hash` / `normalized_result_hash` が valid hash だが
active context と一致しないことは、`AiSidecarDiagnosticResult` response schema / domain failure
ではなく、上の matching key 不一致です。

entry metadata schema invalid は、file read より前に検査します。
同じ entry で `path` と `file_hash` の両方が invalid な場合は `path` を先に返します。
entry metadata schema invalid では `artifact` を inspect してはいけません。
file unreadable / file hash mismatch / JSON invalid の field は、
`ai_sidecar[<i>].path`、`audit_sidecar_validation[<j>].path`、または
`ai_sidecar_diagnostic[<k>].path` です。
hash mismatch では `expected_hash = <entry>.file_hash`、
`actual_hash = referenced file bytes sha256` を使います。
schema / domain failure では、artifact-local `$` を
`ai_sidecar[<i>].artifact`、`audit_sidecar_validation[<j>].artifact`、または
`ai_sidecar_diagnostic[<k>].artifact` に置換し、root 以外の path は
`<prefix>.<artifact-local path>` として報告します。

MVP の `AiSidecarDiagnosticResult`：

```json
{
  "schema": "npa.phase8.ai_sidecar_diagnostic_result.v1",
  "policy_hash": "sha256:...",
  "input_policy_hash": "sha256:...",
  "normalized_result_hash": "sha256:...",
  "status": "failed",
  "target_count": 1,
  "error": {
    "kind": "ai_sidecar_diagnostic_failure",
    "reason_code": "required_ai_sidecar_source_mismatch",
    "field": "ai_sidecar[0].artifact.source.run_artifact_hash",
    "expected_hash": "sha256:...",
    "actual_hash": "sha256:..."
  }
}
...
`AiSidecarDiagnosticResult` は transient response であり、`result_hash` を持ちません。
`policy_hash` は active `ReleasePolicy` の canonical hash です。
`input_policy_hash` は `ReleasePolicy.ai_triage.input_policy_hash` です。
`normalized_result_hash` は診断対象 `NormalizedCheckResult.normalized_result_hash` です。
`target_count` はその `NormalizedCheckResult` から導出した required AI sidecar target 数です。
`status` は `passed` または `failed` です。
`target_count = 0` の場合は `status = passed` とし、`error` は omit します。
`target_count = 0` の response は direct diagnostic invocation の成功 summary または
informational log としてだけ使い、mode pass evaluation の required diagnostic result set には含めません。
pass evaluation では diagnostic target set が空の `NormalizedCheckResult` に対する
`AiSidecarDiagnosticResult` は要求せず、存在しても pass / fail / duplicate 判定の対象外として ignore します。
diagnostic target set が non-empty の `NormalizedCheckResult` に対しては、
`target_count` が derived target list length と一致しなければなりません。
一致しない response は required diagnostic result 候補にせず、
上の `AiSidecarDiagnosticEvaluationFailure.reason_code =
ai_sidecar_diagnostic_target_count_mismatch` として報告します。
`status = passed` では `error` を omit し、
`status = failed` では `error.kind = ai_sidecar_diagnostic_failure` と
`error.reason_code` を required にします。
`AiSidecarDiagnosticResult` は closed-world JSON object です。
top-level required field は `schema`、`policy_hash`、`input_policy_hash`、
`normalized_result_hash`、`status`、`target_count` です。
`schema` は `npa.phase8.ai_sidecar_diagnostic_result.v1` に固定します。
`target_count` は non-negative integer で、MVP 共通の positive / non-negative integer と同じ
signed 64-bit 上限を超えてはいけません。
`error` object の allowed field は `kind`、`reason_code`、`field`、
`expected_hash`、`actual_hash`、`expected_value`、`actual_value` です。
下の fixed diagnostic shape で指定されない payload field は omit しなければなりません。
response-local JSON path の root marker は `$` です。
top-level object type、known field duplicate、unknown field、missing、null、wrong type、
invalid enum、invalid hash format の payload shape は
`AuditSidecarValidationResult` response schema / domain validation と同じです。
ただし expected schema string は `npa.phase8.ai_sidecar_diagnostic_result.v1`、
`status` の expected_value は `AiSidecarDiagnosticResult.status`、
`error.kind` の expected_value は `ai_sidecar_diagnostic_failure`、
`error.reason_code` の expected_value は `AiSidecarDiagnosticResult.error.reason_code`、
`target_count` の expected_value は `non_negative_i64` とします。
`AiSidecarDiagnosticResult` response schema / domain validation は
`policy_hash`、`input_policy_hash`、`normalized_result_hash` の hash format と型だけを検査し、
active `ReleasePolicy` や `ci_diagnostic_targets[]` との等価性は検査しません。
その context binding は saved diagnostic result matching と pass evaluation で扱います。
`AiSidecarDiagnosticResult` response schema / domain failure が複数成立する場合は、
次の順序で最初の1件だけを返します。
...
3. policy_hash
4. input_policy_hash
5. normalized_result_hash
6. status
7. target_count
8. status = passed error forbidden / status = failed error required
9. error.kind
10. error.reason_code
11. reason_code-specific required payload members
12. reason_code-specific forbidden payload members
13. unknown fields, including duplicate unknown fields
```

known top-level field の duplicate key は、その field の priority 位置で
`actual_value = "duplicate_field"` として報告します。
known `error` field の duplicate key は、対応する `error.kind` / `error.reason_code` /
payload member の priority 位置で同じく `duplicate_field` として報告します。

CI diagnostic target list の順序は次で固定します。

```text
1. machine_result target:
   NormalizedCheckResult.results[] の occurrence order。

2. normalized_comparison target:
   comparison.status が required target 条件を満たす場合だけ、
   すべての machine_result target の後に1件置く。
```

`required_ai_sidecar_targets[<t>]` の `<t>` はこの target list の 0-based index です。
`target_count` と derived target list length はこの target list の長さであり、
`machine_result` target の selected raw resolution が missing / duplicate になっても
その target を除外してはいけません。
再計算された `AiSidecarDiagnosticResult` の `status` はこの target list に対して決めます。
derived target list が empty の場合は `status = passed`、`error` omit です。
derived target list が non-empty で、すべての target satisfaction check が成功した場合も
`status = passed`、`error` omit です。
target satisfaction check が1件でも失敗した場合は `status = failed` とし、
target list order で最初の failing target、かつ同じ target 内の下記優先順位で最初の failure を
`error` に入れます。
各 target は、対応する `AiAuditSidecar` がちょうど1件なければなりません。
`machine_result` target では、sidecar candidate selection の前に、
target の `NormalizedCheckResult.results[]` entry から
`(checker_profile, request_hash, result_hash, policy_hash)` を取り、
対応する `ci_diagnostic_targets[<n>].normalizer_machine_results[]` から selected raw `MachineCheckResult` を
一意に解決します。
この selected raw resolution で候補が0件または複数件の場合は、
その target の diagnostic failure として下の selected raw result missing / duplicate を返し、
sidecar candidate selection へ進みません。
この gate は nested `MachineCheckResult` を再検証せず、prevalidated context の parsed field value だけで
tuple matching と duplicate 判定を行います。
conforming caller では nested `MachineCheckResult` の `result_hash` /
`run_artifact_hash` self-consistency failure はこの gate に到達しません。
defensive implementation が non-prevalidated context を検出した場合は、
この gate の `AiSidecarDiagnosticEvaluationFailure` や target satisfaction failure にせず、
caller-side artifact validation / normalization error として gate 起動前に失敗させます。
non-prevalidated context を silent に candidate set から除外してはいけません。
`machine_result` target の candidate sidecar は、`source.kind = machine_result` で、
`source.request_hash` と `source.result_hash` が target の selected raw `MachineCheckResult` と一致し、
`source.normalized_result_hash` が存在する場合は target `NormalizedCheckResult.normalized_result_hash`
とも一致する sidecar です。
この candidate 判定では `source.run_artifact_hash` をまだ使いません。
candidate が1件に絞れた後で、selected raw `MachineCheckResult.run_artifact_hash` との
完全一致を下の `required_ai_sidecar_source_mismatch` rule で検査します。
`normalized_comparison` target の candidate sidecar は、`source.kind = normalized_comparison` で、
`source.normalized_result_hash` が target `NormalizedCheckResult.normalized_result_hash` と一致する sidecar です。

candidate sidecar に対応する `AuditSidecarValidationResult` は、required diagnostic では
sidecar file hash から先に特定します。
response candidate は `artifact.sidecar_file_hash = ai_sidecars[<i>].file_hash` を満たす
`audit_sidecar_validation_results[]` entry です。
`status = failed` の response でも `sidecar_file_hash` が存在すれば candidate に含めます。
`mode = schema_only` の response でも `sidecar_file_hash` が存在すれば cardinality candidate には含めますが、
required diagnostic を満たす validation response にはなりません。
`sidecar_file_hash` を omit した response はこの candidate set に含めません。
`status = failed` でも `sidecar_file_hash` を omit した response は
`required_ai_sidecar_validation_failed` ではなく candidate 外です。
その target に他の candidate がなければ `required_ai_sidecar_validation_missing` になります。
validation response candidate cardinality は、`mode`、`status`、`input_policy_hash`、
source-kind-specific matching key より前に評価します。
candidate が0件なら validation response missing、2件以上なら validation response duplicate です。
したがって同じ sidecar file hash に対して `mode = schema_only` response と
`mode = cross_artifact` / `status = valid` response が両方存在する場合は、
`required_ai_sidecar_validation_duplicate` を返し、
`required_ai_sidecar_validation_mode_mismatch` は返しません。
candidate が1件に絞れた後は、まず `mode`、次に `status`、次に `input_policy_hash`、
最後に source-kind-specific matching key の順で検査します。
単一 candidate が `mode = schema_only` なら、`status = failed` や source metadata の有無に関係なく
`required_ai_sidecar_validation_mode_mismatch` を返します。
`required_ai_sidecar_validation_failed` は、単一 candidate の `mode = cross_artifact` かつ
`status = failed` の場合だけ返します。
candidate が1件に絞れ、かつ `mode = cross_artifact` かつ `status = valid` の response だけが
full matching candidate になります。
full matching candidate では、13 の release bundle optional sidecar と同じ
source-kind-specific matching key も一致しなければなりません。
required diagnostic での source-kind-specific matching key は、
candidate response と candidate sidecar の parsed artifact だけを比較します。
`sidecar_file_hash` は response candidate cardinality で、`input_policy_hash` は
validation response input policy mismatch で既に扱うため、この key には含めません。
source key component は次の順に比較し、最初に不一致になった component が
`required_ai_sidecar_validation_source_mismatch` の原因です。
外部 payload は下の generic field / expected_value / actual_value に固定し、
component-specific field は報告しません。

```text
common:
  1. AuditSidecarValidationResult.source_kind == AiAuditSidecar.source.kind

source_kind = machine_result:
  2. AuditSidecarValidationResult.source_result_hash == AiAuditSidecar.source.result_hash
  3. if AiAuditSidecar.source.normalized_result_hash is present:
       AuditSidecarValidationResult.source_normalized_result_hash
         == AiAuditSidecar.source.normalized_result_hash
     otherwise:
       AuditSidecarValidationResult.source_normalized_result_hash must be omitted

source_kind = normalized_comparison:
  2. AuditSidecarValidationResult.source_normalized_result_hash
       == AiAuditSidecar.source.normalized_result_hash
  3. AuditSidecarValidationResult.source_result_hash must be omitted
```

各 target に対する diagnostic failure は target list order で最初の failing target を選び、
同じ target 内では次の順序で最初の failure だけを返します。

```text
selected raw result missing:
  reason_code = required_ai_sidecar_selected_raw_missing
  field = "required_ai_sidecar_targets[<t>].selected_raw_result"
  expected_value = "exactly_one_selected_raw_machine_check_result"
  actual_value = "missing"

selected raw result duplicate:
  reason_code = required_ai_sidecar_selected_raw_duplicate
  field = "required_ai_sidecar_targets[<t>].selected_raw_result"
  expected_value = "exactly_one_selected_raw_machine_check_result"
  actual_value = "duplicate"

sidecar missing:
  reason_code = required_ai_sidecar_missing
  field = "required_ai_sidecar_targets[<t>].ai_sidecar"
  expected_value = "exactly_one_ai_audit_sidecar"
  actual_value = "missing"

sidecar duplicate:
  reason_code = required_ai_sidecar_duplicate
  field = "required_ai_sidecar_targets[<t>].ai_sidecar"
  expected_value = "exactly_one_ai_audit_sidecar"
  actual_value = "duplicate"

sidecar input policy mismatch:
  reason_code = required_ai_sidecar_input_policy_mismatch
  field = "ai_sidecar[<i>].artifact.input_policy.hash"
  expected_hash = ReleasePolicy.ai_triage.input_policy_hash
  actual_hash = AiAuditSidecar.input_policy.hash

validation response missing:
  reason_code = required_ai_sidecar_validation_missing
  field = "ai_sidecar[<i>].validation_response"
  expected_value = "exactly_one_audit_sidecar_validation_response"
  actual_value = "missing"

validation response duplicate:
  reason_code = required_ai_sidecar_validation_duplicate
  field = "ai_sidecar[<i>].validation_response"
  expected_value = "exactly_one_audit_sidecar_validation_response"
  actual_value = "duplicate"

validation response mode mismatch:
  reason_code = required_ai_sidecar_validation_mode_mismatch
  field = "audit_sidecar_validation[<j>].artifact.mode"
  expected_value = "cross_artifact"
  actual_value = AuditSidecarValidationResult.mode

validation response failed:
  reason_code = required_ai_sidecar_validation_failed
  field = "audit_sidecar_validation[<j>].artifact.status"
  expected_value = "valid"
  actual_value = "failed"

validation response input policy mismatch:
  reason_code = required_ai_sidecar_validation_input_policy_mismatch
  field = "audit_sidecar_validation[<j>].artifact.input_policy_hash"
  expected_hash = ReleasePolicy.ai_triage.input_policy_hash
  actual_hash = AuditSidecarValidationResult.input_policy_hash

validation response source mismatch:
  reason_code = required_ai_sidecar_validation_source_mismatch
  field = "audit_sidecar_validation[<j>].artifact"
  expected_value = "source_kind_specific_matching_key"
  actual_value = "source_key_mismatch"

wrong retry source:
  reason_code = required_ai_sidecar_source_mismatch
  field = "ai_sidecar[<i>].artifact.source.run_artifact_hash"
  expected_hash = selected raw MachineCheckResult.run_artifact_hash
  actual_hash = AiAuditSidecar.source.run_artifact_hash
```

ここで `<i>` は `ai_sidecars[]` explicit ordered list の 0-based occurrence index、
`<j>` は `audit_sidecar_validation_results[]` explicit ordered list の
0-based occurrence index です。
selected raw result missing / duplicate は `machine_result` target にだけ適用します。
MVP の `AiSidecarDiagnosticResult.error.reason_code` は上の12個に限定します。
これらの `reason_code` は `AuditSidecarValidationResult.error.reason_code` ではありません。
...
AI sidecar conditions:
  ...
  - diagnostic-required only for copied status = failed machine_result targets
    or normalized_comparison targets with comparison.status =
    disagreement / missing_checker_result / policy_failure / inconclusive when
    ReleasePolicy.ai_triage.enabled = true and ai_triage.required = true
  - when a CI diagnostic target NormalizedCheckResult has a non-empty derived
    diagnostic target set, exactly one AiSidecarDiagnosticResult in
    ai_sidecar_diagnostic_results[] for that NormalizedCheckResult must be
    present with status = passed and must match the recomputed result
  - not required for a NormalizedCheckResult whose derived diagnostic target set is empty
  - never sufficient for pass
```

`AiSidecarDiagnosticResult` は `AuxiliaryResult` ではないため、
下の required `AuxiliaryResult` 解決規則には含めません。
required AI sidecar diagnostic result は `ai_sidecar_diagnostic_results[]` explicit ordered list から
`policy_hash`、`input_policy_hash`、`normalized_result_hash` で一意に解決します。
derived diagnostic target set が空の `NormalizedCheckResult` では expected key を作らず、
同じ key の `AiSidecarDiagnosticResult` が list に存在しても pass evaluation では ignore します。
derived diagnostic target set が non-empty の `NormalizedCheckResult` では、
schema / domain valid で、`target_count` が derived target list length と一致する response だけを
required result 候補にします。
`target_count` mismatch は pre-candidate validation item 6 で、この候補集合を作る前に評価します。
同じ key の saved diagnostic result の中に `target_count` mismatch entry と一致 entry が混在する場合も、
最小 index の mismatch entry が `AiSidecarDiagnosticEvaluationFailure` として先に勝ちます。
下の duplicate pass failure は、schema / domain valid かつ key と `target_count` が一致した候補だけを
数えます。
同じ key の `AiSidecarDiagnosticResult` が0件または複数件の場合、
または1件で `status != passed` の場合、その mode の pass failure です。
さらに、required result 候補が1件ある場合でも、同じ key の再計算済み
`AiSidecarDiagnosticResult` sequence entry と RFC 8785 canonical serialization 上で
bytewise 一致しなければ、その mode の pass failure です。
この saved-vs-recomputed mismatch は `AiSidecarDiagnosticEvaluationFailure` ではありません。
`AiSidecarDiagnosticResult` は `result_hash` を持たないため、比較には saved response file bytes の
`file_hash` ではなく parsed object の canonical serialization を使います。

AI sidecar diagnostic pass failure を構造化して報告する実装は、次の failure tuple を使います。
これは transient reporting shape であり、保存 artifact、`CommandError`、
`AiSidecarDiagnosticEvaluationFailure`、`AuxiliaryResult` のいずれでもありません。
複数の expected key で failure が成立する場合は、`ci_diagnostic_targets[]` のうち
derived diagnostic target set が non-empty な entry の order で最初の target を返します。
同じ target 内では次の順で最初の1件だけを返します。

```text
missing saved diagnostic result:
  reason_code = required_ai_sidecar_diagnostic_result_missing
  field = "ci_diagnostic_targets[<n>].ai_sidecar_diagnostic_result"
  expected_value = "exactly_one_matching_ai_sidecar_diagnostic_result"
  actual_value = "missing"

duplicate saved diagnostic result:
  reason_code = required_ai_sidecar_diagnostic_result_duplicate
  field = "ai_sidecar_diagnostic[<k>].artifact"
  expected_value = "unique_matching_ai_sidecar_diagnostic_result"
  actual_value = "duplicate"

saved diagnostic result not passed:
  reason_code = required_ai_sidecar_diagnostic_result_not_passed
  field = "ai_sidecar_diagnostic[<k>].artifact.status"
  expected_value = "passed"
  actual_value = AiSidecarDiagnosticResult.status

saved diagnostic result differs from recomputed result:
  reason_code = required_ai_sidecar_diagnostic_result_mismatch
  field = "ai_sidecar_diagnostic[<k>].artifact"
  expected_hash = sha256(RFC8785 canonical recomputed AiSidecarDiagnosticResult)
  actual_hash = sha256(RFC8785 canonical saved AiSidecarDiagnosticResult)
```

`<n>` は対象 `ci_diagnostic_targets[]` entry の 0-based occurrence index です。
`<k>` は matching saved diagnostic result 候補の 0-based occurrence index です。
duplicate では同じ key を持つ2件目以降の候補のうち最小 index を `<k>` にします。
not-passed / mismatch では一意に解決された候補の index を `<k>` にします。
...
AxiomReportStoreManifest-local schema shape failure の field / payload は次で固定します。
...
duplicate top-level known field:
  field = "schema" or "reports"
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

top-level schema missing / null / type:
  field = "schema"
  expected_value = "npa.phase8.axiom_report_store_manifest.v1"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"
...
reports array missing / null / type:
  field = "reports"
  expected_value = "array"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

reports[] entry object type:
  field = "reports[<i>]"
  expected_value = "object"
  actual_value = "wrong_type" | "null_not_allowed"

duplicate reports[] entry known field:
  field = "reports[<i>].<member>"
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

reports[] entry required member / null / type:
  field = "reports[<i>].<member>"
  expected_value = member expected_value from the table below
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

reports[] entry value shape:
  field = "reports[<i>].<member>"
  expected_value = member expected_value from the table below
  actual_value = member actual_value from the table below
```

reports[] entry member value shape は次で固定します。

```text
axiom_report_hash / file_hash:
  expected_value = "sha256:<lower-hex>"
  actual_value = "invalid_hash_format"
...
caller error body で `axiom_report_store` prefix を使う場合、manifest-local `$` は
`field = "axiom_report_store"`、root 以外の local path は
`field = "axiom_report_store.<manifest-local path>"` に置換します。
known field の duplicate object key は、その field の schema order 位置で
`expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` として報告します。
unknown field の duplicate object key は unknown field の位置で
`expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` を返し、
...
unknown field 名を `schema_path_component` で表現できない場合は raw key を copy せず、
top-level object では `field = "$"`、`reports[]` entry では `field = "reports[<i>]"` にします。
...
baseline / repeated MachineCheckResult JSON / schema / domain failure:
  ...
  expected_value = "valid_json" for JSON parse failure, otherwise <schema requirement name>
  actual_value = invalid_json | missing | wrong_type | unknown_field |
                 invalid_enum | invalid_hash_format | invalid_name_format | invalid_path |
                 null_not_allowed | non_positive_integer | integer_out_of_range |
                 order_violation | duplicate_field | duplicate_axiom_name | forbidden_field
...
`MachineCheckResult.error` から section 6 の normalized failure key rule で導出した object です。
...
nightly:
  ...
  - AI audit sidecar and validation responses only for copied-status-failed
    machine_result targets and normalized_comparison targets with comparison.status =
    disagreement / missing_checker_result / policy_failure / inconclusive
    when ReleasePolicy.ai_triage.enabled = true and ai_triage.required = true
...
high-trust:
  ...
  - retained MachineCheckResult artifacts in append-only storage
```
...
- source MachineCheckResult files
...
duplicate した unknown field 名または unknown field 名を `schema_path_component` で表現できない場合は
raw key を copy せず、top-level object では `field = "$"`、`inputs[]` entry では
`field = "inputs[<i>]"`、`hashes` object では `field = "inputs[<i>].hashes"` にします。
...
unknown field 名を `schema_path_component` で表現できない場合も、上の containing object path を使います。
...
artifact validator が artifact-local root marker `$` だけを返す場合も、
`inputs[<i>].artifact` に置換し、`inputs[<i>].artifact.$` にはしません。
...
kind = release_policy:
  ...
  release_policy.ai_triage.input_policy_hash
    -> inputs[<i>].artifact.ai_triage.input_policy_hash
...
kind = ai_audit_sidecar:
  $
    -> inputs[<i>].artifact
  source
    -> inputs[<i>].artifact.source
  input_policy
    -> inputs[<i>].artifact.input_policy
  ai
    -> inputs[<i>].artifact.ai
  classification
    -> inputs[<i>].artifact.classification
...
kind = challenge_output_store_manifest:
  challenge_output_store
    -> inputs[<i>].artifact
  challenge_output_store.schema
    -> inputs[<i>].artifact.schema
  challenge_output_store.entries[<j>]
    -> inputs[<i>].artifact.entries[<j>]
  challenge_output_store.entries[<j>].manifest_path
    -> inputs[<i>].artifact.entries[<j>].manifest_path
```

この表にない artifact validator の field shape は、artifact-local JSON path をそのまま
`inputs[<i>].artifact.<json_path>` に入れます。
ただしこの表にない root-level artifact failure でも、`<json_path> = "$"` の場合は
`inputs[<i>].artifact` を使い、末尾に `.$` を残してはいけません。
...
source store entry schema violation:
  ...
  actual_value = missing | wrong_type | unknown_field | invalid_enum |
                 invalid_path | invalid_hash_format | invalid_name_format | null_not_allowed |
                 non_positive_integer | integer_out_of_range | order_violation |
                 duplicate_field | duplicate_axiom_name | forbidden_field |
                 failure_key_mismatch | comparison_field_mismatch
...
MVP の `ReleaseAuditBundleManifest.artifacts[]` entry に kind-specific additional field はありません。
各 entry の known field は `kind`、`path`、`file_hash`、`hashes` だけです。
release bundle artifact kind ではない文字列は closed-set failure ではなく manifest schema の
`kind` enum violation として扱います。
`machine_check_request_error_result` と `normalize_error_result` も release bundle artifact kind ではないため、
`ReleaseAuditBundleManifest.artifacts[].kind` では `invalid_enum` です。

`release validate-bundle` / `audit_bundle` oracle で minimum audit envelope が valid な後の
`ReleaseAuditBundleManifest` full schema / order / duplicate validation priority は次で固定します。
この priority は referenced artifact file の read / hash / JSON / artifact schema validation、
`bundle_hash` self-hash validation、`bundle_id` derivation mismatch、closed-set / cross-artifact validation
より先に適用します。

```text
1. duplicate object key after minimum envelope:
   - non-envelope top-level member
   - artifacts[] entry member
   - artifacts[].hashes member

2. top-level bundle_id missing / wrong type / explicit null / invalid format

3. top-level unknown field

4. artifacts[] element object type / explicit null

5. artifacts[] entry member validation in entry field order:
   kind, path, file_hash, hashes
   path filename / file_hash consistency is evaluated after file_hash format
   validation and before hashes validation.

6. artifacts[].hashes member validation:
   - required hash field missing / wrong type / explicit null / invalid hash format
   - required-hash-none kind with non-empty hashes
   - unknown hash field

7. artifacts[] order violation

8. duplicate artifact (kind, path) pair

9. duplicate artifact path
```

同じ priority 内で複数 failure がある場合、top-level object、`artifacts[]` の小さい index、
その entry の `hashes` object の順で containing object を選びます。
同じ object 内では schema order を使います。
top-level schema order は `schema`、`bundle_id`、`bundle_hash`、`policy_hash`、
`artifact_hash`、`artifacts`、unknown field です。
`artifacts[]` entry schema order は `kind`、`path`、`file_hash`、`hashes`、unknown field です。
`hashes` object では artifact kind ごとの required hash field order を先に評価し、
required hash が `none` の kind では non-empty check を評価し、
最後に unknown field を bytewise lexicographic order で評価します。
artifact kind ごとの required hash field order は上の `hashes` table の記載順です。
unknown field 名を `schema_path_component` で表現できない場合は raw key を copy せず、
top-level object では `field = "manifest"`、`artifacts[]` entry では
`field = "manifest.artifacts[<j>]"`、`hashes` object では
`field = "manifest.artifacts[<j>].hashes"` にします。

full manifest schema / order / duplicate failure の field shape は次で固定します。
ここで `<j>` は `ReleaseAuditBundleManifest.artifacts[]` の 0-based index です。

```text
non-envelope top-level duplicate key:
...
artifacts[] entry duplicate key:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>]"
  error.expected_value = "unique_object_keys"
  error.actual_value = "duplicate_field"

artifacts[].hashes duplicate key:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].hashes"
  error.expected_value = "unique_object_keys"
  error.actual_value = "duplicate_field"

bundle_id missing / null / wrong type / invalid format:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.bundle_id"
  error.expected_value = "release_<64-lower-hex>"
  error.actual_value = missing | null_not_allowed | wrong_type | invalid_name_format

top-level unknown field:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest"
  error.expected_value = "absent"
  error.actual_value = "unknown_field"

artifacts[] element is not object:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>]"
  error.expected_value = "object"
  error.actual_value = wrong_type | null_not_allowed

artifacts[] entry kind missing / null / wrong type / invalid enum:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].kind"
  error.expected_value = "release_bundle_artifact_kind"
  error.actual_value = missing | null_not_allowed | wrong_type | invalid_enum

artifacts[] entry path missing / null / wrong type / invalid path:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].path"
  error.expected_value = "bundle_artifact_path:<kind>"
  error.actual_value = missing | null_not_allowed | wrong_type | invalid_path

artifacts[] entry file_hash missing / null / wrong type / invalid hash format:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].file_hash"
  error.expected_value = "sha256:<lower-hex>"
  error.actual_value = missing | null_not_allowed | wrong_type | invalid_hash_format

artifacts[] entry path filename / file_hash mismatch:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].path"
  error.expected_hash = entry file_hash
  error.actual_hash = "sha256:<filename_without_json>"

artifacts[] entry hashes missing / null / wrong type:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].hashes"
  error.expected_value = "object"
  error.actual_value = missing | null_not_allowed | wrong_type

artifacts[] entry unknown field:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>]"
  error.expected_value = "absent"
  error.actual_value = "unknown_field"

artifacts[].hashes required hash missing / null / wrong type / invalid hash format:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].hashes.<hash_field>"
  error.expected_value = "sha256:<lower-hex>"
  error.actual_value = missing | null_not_allowed | wrong_type | invalid_hash_format
...
required-hash-none kind with non-empty hashes:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].hashes"
  error.expected_value = "empty_object"
  error.actual_value = "non_empty"

artifacts[].hashes unknown field:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].hashes"
  error.expected_value = "absent"
  error.actual_value = "unknown_field"

artifacts[] order violation:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>]"
  error.expected_value = "artifacts_sorted_by_kind_path"
  error.actual_value = "order_violation"

duplicate artifact (kind, path) pair:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>]"
  error.expected_value = "unique_kind_path"
  error.actual_value = "duplicate_entry"

duplicate artifact path:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].path"
  error.expected_value = "unique_path"
  error.actual_value = "duplicate_path"
```

`artifacts[]` entry の `path` schema validation は workspace-relative path schema ではなく
bundle-local artifact path schema です。
`path` は `artifacts/<kind>/<file_hash_without_sha256_prefix>.json` の文字列形を満たし、
`kind` directory は同じ entry の `kind` と一致しなければなりません。
filename は `<64 lower-hex>.json` でなければならず、これらのいずれかに違反する場合は
上の `path` invalid path failure として扱います。
filename の lower-hex と同じ entry の `file_hash` から `sha256:` prefix を除いた値の一致は、
`file_hash` が schema-valid な後にだけ評価し、上の path filename / file_hash mismatch として扱います。
symlink escape など、schema-valid path を bundle root から解決した後の IO path failure は
full manifest schema failure ではなく、後述の referenced bundle file validation の
`bundle_local_path_inside_bundle_root` failure です。
`artifacts[]` order violation の `<j>` は、最初に `(kind, path)` が直前 entry より
bytewise 小さくなる後続 entry index です。
隣接 `(kind, path)` が等しい場合は `order_violation` ではなく
duplicate artifact `(kind, path)` pair として扱います。
duplicate artifact `(kind, path)` pair と duplicate artifact path の `<j>` は、
同じ key がすでに出現している最小の後続 entry index です。
...
参照先 `MachineCheckResult` が存在し、その target の
`result_hash` / `run_artifact_hash` self-consistency が通った後にだけ、
selector target equality を Step 8 class 5 として検査します。
baseline target では release target baseline result entry の
`result_hash`、`request_hash`、`checker_profile`、`policy_hash` と比較します。
repeated target では selector の `request_hash`、`checker_profile`、release target baseline
result entry の `result_hash`、`policy_hash` と比較します。
同じ target に self-consistency failure もある場合は、selector target mismatch を合成せず
`machine_check_result` self-consistency failure を先に返します。
...
checker_profile / result_hash / policy_hash mismatch は Step 8 class 5 として扱います。
ただし target の `MachineCheckResult` self-consistency が先に失敗する場合は、
その `machine_check_result` failure を返し、selector target mismatch は合成しません。
...
included `RunnerPolicy` file を使います。
この policy hash が一致しない場合は Step 8 class 5 として報告し、
field は `compare_validation_response[<i>].artifact.policy_hash`、
`expected_hash` は対象 normalized result 種別に対応する RunnerPolicy canonical hash、
`actual_hash` は parsed `CompareValidationResult.policy_hash` です。
保存済み `CompareValidationResult` object と再実行 object が canonical serialization 上で
一致しない場合も Step 8 class 5 として報告し、
field は `compare_validation_response[<i>].artifact`、
`expected_hash = sha256(canonical(re-run CompareValidationResult))`、
`actual_hash = sha256(canonical(saved CompareValidationResult))` です。
同じ `compare_validation_response` entry で policy hash mismatch と saved-vs-rerun mismatch が
同時に成立する場合は、policy hash mismatch を先に返します。
saved-vs-rerun mismatch は、保存済み `CompareValidationResult.policy_hash` が
対象 normalized result 種別の RunnerPolicy canonical hash と一致した後でだけ導出します。
対象 normalized result がこの2種類のどちらにも分類できない場合、
その `compare_validation_response` entry は forbidden です。
included `audit_sidecar_validation_response` は `mode = cross_artifact` だけを許可します。
`mode = schema_only` の response は standalone `AuditSidecarValidationResult` としては valid でも、
release bundle input としては artifact domain failure です。
`npa-check release bundle` では Step 6 の `input_schema_invalid`、
`npa-check release validate-bundle` / `audit_bundle` oracle では
`audit_bundle_invalid` とし、field は
`audit_sidecar_validation[<i>].artifact.mode`、
`expected_value = "cross_artifact_for_release_bundle"`、
`actual_value = "schema_only"` です。
この failure は optional sidecar との対応付けや status gating より前に報告します。
`status = failed` の `audit_sidecar_validation_response` は Step 8 class 3 の
status gating failure として報告し、optional sidecar との対応付け候補には含めません。
対応付け候補は、`mode = cross_artifact`、`status = valid`、かつ source kind ごとの
matching key field を schema-valid に持つ response だけです。
すべての included `ai_audit_sidecar` entry について、対応する `audit_sidecar_validation_response` entry を
parsed `AuditSidecarValidationResult` と parsed `AiAuditSidecar.source` から特定します。
共通 key は、response の `sidecar_file_hash` が included `ai_audit_sidecar` entry の
`file_hash` と一致すること、response の `input_policy_hash` が sidecar の
`input_policy.hash` と一致すること、および response の `source_kind` が sidecar の
`source.kind` と一致することです。
`source_kind = machine_result` では、`source_result_hash` が sidecar の
`source.result_hash` と一致しなければなりません。
sidecar が `source.normalized_result_hash` を持つ場合だけ、
`source_normalized_result_hash` も key に含めて同じ値と比較します。
sidecar が `source.normalized_result_hash` を持たない場合、
`source_normalized_result_hash` は kind-inapplicable omitted field として扱い、
存在してはいけません。
`source_kind = normalized_comparison` では、`source_normalized_result_hash` が sidecar の
`source.normalized_result_hash` と一致し、`source_result_hash` は kind-inapplicable omitted field として扱い、
存在してはいけません。
一致する response が0件または複数件ある場合は bundle invalid です。
さらに、`mode = cross_artifact` かつ `status = valid` の included
`audit_sidecar_validation_response` entry は、同じ source-kind-specific matching key で
included `ai_audit_sidecar` entry にちょうど1件対応しなければなりません。
0件または複数件に対応する response は、`audit_sidecar_validation_response`
class 4 extra / forbidden artifact failure です。
...
一致することを検査しなければなりません。
一致しない場合は Step 8 class 5 として報告し、
field は `audit_sidecar_validation[<j>].artifact`、
`expected_hash = sha256(canonical(re-run AuditSidecarValidationResult))`、
`actual_hash = sha256(canonical(saved AuditSidecarValidationResult))` です。
...
この場合は audit-sidecar validation response の再計算不一致を合成する前に、
`ai_audit_sidecar` 側の Step 8 class 5 として報告し、
field は `ai_sidecar[<i>].artifact.source`、
`expected_value = "release_audit_optional_sidecar_source"`、
`actual_value = "forbidden_source"` です。
release bundle 固有の optional sidecar source-value check も、
audit-sidecar validation response の再計算不一致より先に `ai_audit_sidecar`
側の Step 8 class 5 として報告します。
`source.kind = machine_result` で `source.run_artifact_hash` が release target selected raw result
または reproducibility repeated raw result の allowed set に含まれない場合は、
field = `ai_sidecar[<i>].artifact.source.run_artifact_hash`、
`expected_value = "release_optional_machine_result_source"`、
`actual_value = "forbidden_source"` です。
この allowed set は bundle 側で導出済みの raw result set であり、sidecar source から
`policy_hash` を読むことでは導出しません。
allowed raw result に解決できた後で `source.result_hash` または `source.request_hash` が
解決先 `MachineCheckResult` と一致しない場合は、その source member path を field にし、
`expected_hash` に sidecar source の hash、`actual_hash` に解決先 artifact の hash を入れます。
`source.kind = machine_result` の release bundle source-value check は、
allowed raw result 解決後に次の順で最初の1件だけを返します。
1. `source.result_hash` mismatch:
   field = `ai_sidecar[<i>].artifact.source.result_hash`、
   `expected_hash` = sidecar source result_hash、
   `actual_hash` = resolved `MachineCheckResult.result_hash`
2. `source.request_hash` mismatch:
   field = `ai_sidecar[<i>].artifact.source.request_hash`、
   `expected_hash` = sidecar source request_hash、
   `actual_hash` = resolved `MachineCheckResult.request_hash`
3. reproducibility repeated raw result で `source.normalized_result_hash` が存在する:
   field = `ai_sidecar[<i>].artifact.source.normalized_result_hash`、
   `expected_value = "absent_for_reproducibility_repeated_source"`、
   `actual_value = "present"`
4. release target selected raw result で `source.normalized_result_hash` が存在し、
   release target `NormalizedCheckResult.normalized_result_hash` と一致しない:
   field = `ai_sidecar[<i>].artifact.source.normalized_result_hash`、
   `expected_hash` = release target normalized result hash、
   `actual_hash` = sidecar source normalized_result_hash
5. release target selected raw result で `source.normalized_result_hash` が存在し、
   release target `NormalizedCheckResult.results[*].result_hash` が `source.result_hash` を含まない:
   field = `ai_sidecar[<i>].artifact.source.result_hash`、
   `expected_value = "member_of_release_target_normalized_result"`、
   `actual_value = "missing"`
`source.kind = normalized_comparison` で `source.normalized_result_hash` が release target
`NormalizedCheckResult.normalized_result_hash` と一致しない場合は、
field = `ai_sidecar[<i>].artifact.source.normalized_result_hash`、
`expected_hash` に release target normalized result hash、
`actual_hash` に sidecar source の hash を入れます。
...
`machine_check_request` / `machine_check_result` の closed-set identity source は
command mode ごとに固定します。
`npa-check release bundle` generation では、既存 candidate は explicit `--request-store` /
`--result-store` で渡された bundle-local store manifest entries からだけ導出します。
`machine_check_request` の existing identity は validated
`RequestStoreManifest.requests[].request_hash`、`machine_check_result` の
run-artifact existing identity は validated
`MachineResultStoreManifest.results[].run_artifact_hash` です。
store entry と referenced artifact の field mismatch は store / entry validation failure として
Step 8 closed-set validation より前に報告し、bundle-root scan、path 逆引き、外部 database fallback で
candidate を補ってはいけません。
`npa-check release validate-bundle` / `audit_bundle` では、existing identity は
materialized `ReleaseAuditBundleManifest.artifacts[]` entry の
`hashes.request_hash` / `hashes.run_artifact_hash` です。
closed-set exactness では parsed / recomputed artifact hash へ fallback して
manifest entry の `hashes.*` を補正してはいけません。
manifest entry の `hashes.*` が allowed identity と一致しないため exactness が失敗する場合は
class 4 missing / extra / duplicate を返します。
materialized artifact entry hash と parsed / recomputed artifact hash の不一致は、
その entry が materialized `hashes.*` identity で closed-set exactness を通過した後の
self-consistency failure として扱います。
ただし release target raw result selection で tuple により選ばれた
`machine_check_result` entry だけは、selection 後にその materialized
`hashes.run_artifact_hash` を allowed run identity として allowed run set に追加します。
この場合も allowed identity として使う値は parsed / recomputed `run_artifact_hash` ではなく、
manifest entry の `hashes.run_artifact_hash` です。
そのため tuple は一致するが materialized `hashes.run_artifact_hash` と parsed / recomputed
`run_artifact_hash` が不一致の entry は、closed-set exactness ではなく、後続の
self-consistency failure として報告します。
逆に tuple selection でも他の allowed run source でも選ばれていない entry は、
parsed / recomputed `run_artifact_hash` が allowed value に一致しても救済せず、
materialized `hashes.run_artifact_hash` に基づく class 4 extra / forbidden とします。
class 4 missing / extra / duplicate の structured payload は次で固定します。
ここで identity key string は RFC 8785 canonical JSON array とし、
`machine_check_request` では `["request_hash", request_hash]`、
`machine_check_result` の run artifact identity では
`["run_artifact_hash", run_artifact_hash]`、
release target tuple identity では
`["release_target_result", checker_profile, request_hash, result_hash, policy_hash]`
を使います。
missing では `expected_value` に不足した expected identity key string を入れ、
extra / forbidden / duplicate では `actual_value` に offending existing identity key string を入れます。
duplicate では同じ identity key がすでに出現している最小の後続 entry を `<i>` に使います。
`machine_check_result` の existing identity key string は常に
`["run_artifact_hash", run_artifact_hash]` です。
`["release_target_result", checker_profile, request_hash, result_hash, policy_hash]` は
release target raw result selection で candidate が0件だった場合の missing expected key だけに使い、
extra / forbidden / duplicate の existing identity としては使いません。

```text
missing machine_check_request:
  field = "machine_check_request"
  expected_value = identity key string
  actual_value = "missing"

extra / forbidden machine_check_request:
  field = "machine_check_request[<i>].artifact.request_hash"
  expected_value = "allowed_machine_check_request_identity"
  actual_value = identity key string

duplicate machine_check_request:
  field = "machine_check_request[<i>].artifact.request_hash"
  expected_value = "unique_machine_check_request_identity"
  actual_value = identity key string

missing machine_check_result:
  field = "machine_check_result"
  expected_value = identity key string
  actual_value = "missing"

extra / forbidden machine_check_result:
  field = "machine_check_result[<i>].artifact.run_artifact_hash"
  expected_value = "allowed_machine_check_result_identity"
  actual_value = identity key string

duplicate machine_check_result:
  field = "machine_check_result[<i>].artifact.run_artifact_hash"
  expected_value = "unique_machine_check_result_identity"
  actual_value = identity key string
```

`<i>` は generation では生成後の `ReleaseAuditBundleManifest.artifacts` の deterministic
kind/path order、validate-bundle / `audit_bundle` では materialized manifest order での
対象 kind の occurrence index です。
...
selector target equality は、その target の `MachineCheckResult.result_hash` /
`run_artifact_hash` self-consistency が通った後だけ Step 8 class 5 として扱います。
baseline target は release target baseline result entry の `result_hash`、`request_hash`、
`checker_profile`、`policy_hash` と比較し、repeated target は selector の
`request_hash`、`checker_profile`、release target baseline result entry の
`result_hash`、`policy_hash` と比較します。
同じ target に self-consistency failure もある場合は、`machine_check_result` failure を
先に返し、`auxiliary_result` selector target mismatch は合成しません。
`ChallengeReplayResult.checker_results[*]` が copy した result_hash / checker_profile と
resolved `MachineCheckResult` の不一致は、13 の prerequisite gate で定義する
challenge_replay_result class 5 source-key failure として報告します。
...
参照先 `MachineCheckResult` が存在し、その `result_hash` / `run_artifact_hash`
self-consistency が通った後でもこの tuple に一致しない場合は、
`auxiliary_result` class 5 selector target mismatch として報告します。
これは `machine_check_result` missing / forbidden failure でも
`normalized_check_result` source-copy mismatch でもありません。
field は
`auxiliary_result[<a>].artifact.selector.baseline_run_artifact_hash` とし、
`<a>` は required `reproducibility` entry の occurrence index です。
mismatch member は `result_hash`、`request_hash`、`checker_profile`、`policy_hash` の順で
最初に不一致になったものを使います。
hash member では `expected_hash` に release target result entry の値、
`actual_hash` に selected `MachineCheckResult` の対応する値
（`policy_hash` では `MachineCheckResult.policy.hash`）を入れます。
`checker_profile` では `expected_value` に release target result entry の
`checker_profile`、`actual_value` に selected `MachineCheckResult.checker.profile` を入れます。
その他の release target result entry では、同じ bundle 内に
serialized selection tuple `(checker_profile, request_hash, result_hash, policy_hash)` の
4 field が一致する `machine_check_result` entry がちょうど1件存在し、
それを選択 raw result とします。
この matching candidate は、Step 6 schema / domain validation を通過し、
上の4 field を parsed artifact から extract できる included `machine_check_result` entry だけです。
この tuple の `policy_hash` component は、parsed `MachineCheckResult.policy.hash` から extract します。
`MachineCheckResult` に top-level `policy_hash` field があると仮定してはいけません。
candidate selection と ambiguity 判定では `MachineCheckResult.result_hash` /
`run_artifact_hash` self-consistency をまだ要求せず、self-consistency が失敗する candidate も
tuple が一致するなら候補として数えます。
候補を self-consistency failure を理由に除外して、0件 missing や別 candidate の一意選択に
書き換えてはいけません。
...
field は2件目以降の matching candidate のうち最小 occurrence index の
`machine_check_result[<m>].artifact` とし、
`expected_value = "unique_release_target_raw_result_candidate"`、
`actual_value = "duplicate_candidate_for_normalized_check_result[<j>].artifact.results[<i>]"` とします。
`<j>` は release target `normalized_check_result` の occurrence index、`<i>` は対象
`results[]` entry の index、`<m>` は duplicate candidate として報告する
`machine_check_result` の occurrence index です。
複数の non-baseline result entry で ambiguity が成立する場合は、小さい `<i>` を先にし、
同じ `<i>` では2件目以降の candidate の小さい `<m>` を報告します。
この場合、どちらかの retry result を採用して `machine_check_result` missing / forbidden failure を
合成してはいけません。
候補が1件だけの場合、その entry を選択 raw result とし、続く
machine_check_result set prerequisite-clean 条件で self-consistency を検査します。
この selection が allowed run set に追加する concrete identity は、generation では
validated result store entry の `run_artifact_hash`、validate-bundle / `audit_bundle` では
materialized manifest entry の `hashes.run_artifact_hash` です。
selection tuple は candidate を選ぶためだけに使い、closed-set exactness の concrete
run identity として parsed / recomputed `run_artifact_hash` に置き換えてはいけません。
候補が1件選ばれた後は、`("release_target_result", checker_profile, request_hash, result_hash, policy_hash)`
key を allowed run set member として保持せず、上の concrete `run_artifact_hash` identity に置き換えます。
...
release target raw result selection rule 自体が追加で許可する baseline profile の
同じ tuple retry result は、`reproducibility.selector.repeated_run_artifact_hash` として
参照される1件だけです。
同じ tuple の run が `ChallengeReplayResult.checker_results[*].run_artifact_hash` など
別の allowed run source から独立に導入される場合、その entry はその source によって
allowed run set に含まれるのであって、release target baseline retry exception によって
許可されるのではありません。
同じ `run_artifact_hash` が repeated run source と challenge replay source の両方に現れる場合は、
allowed run set の union で1件に deduplicate します。
どの allowed run source にも属さない同じ tuple の baseline retry result は forbidden です。
...
`ChallengeCoverageSummary` schema / domain validation priority は次で固定します。
複数の violation が同時に存在する場合は、最初に該当したものだけを返します。
`summary_hash` self-hash mismatch と derived `summary_id` mismatch はこの schema / domain
validation を通過した後の artifact identity validation で扱います。

```text
1. top-level object type
2. duplicate top-level known field, in top-level field order
3. top-level schema missing / null / type / mismatch
4. top-level non-entries required member / null / type / value shape, in top-level field order
5. entries array missing / null / type
6. entries[] entry object type, by smaller entry index
7. duplicate entries[] entry known field, by smaller entry index and entry field order
8. entries[] entry required member / null / type / value shape, by smaller entry index and entry field order
9. unknown entries[] entry field, by smaller entry index and bytewise field name order
10. unknown top-level field, by bytewise field name order
11. entries order violation
12. duplicate (challenge_id, manifest_hash)
13. duplicate replay_result_hash
14. count relationship domain failure
```

top-level field order は次で固定します。
...
schema shape failure の artifact-local field / payload は次で固定します。
...
duplicate top-level known field:
  field = duplicated top-level field name
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

top-level schema missing / null / type:
  field = "schema"
  expected_value = "npa.phase8.challenge_coverage_summary.v1"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"
...
top-level non-entries required member / null / type:
  field = top-level field name
  expected_value = member expected_value from the table below
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

top-level non-entries value shape:
  field = top-level field name
  expected_value = member expected_value from the table below
  actual_value = member actual_value from the table below
...
entries[] entry object type:
  field = "entries[<i>]"
  expected_value = "object"
  actual_value = "wrong_type" | "null_not_allowed"

duplicate entries[] entry known field:
  field = "entries[<i>].<member>"
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"

entries[] entry required member / null / type:
  field = "entries[<i>].<member>"
  expected_value = member expected_value from the table below
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

entries[] entry value shape:
  field = "entries[<i>].<member>"
  expected_value = member expected_value from the table below
  actual_value = member actual_value from the table below
```

member value shape は次で固定します。

```text
summary_id:
  expected_value = "chcov_<64-lower-hex>"
  actual_value = "invalid_name_format"

summary_hash / policy_hash / artifact_hash / target_normalized_result_hash /
challenge_store_manifest_hash / result_store_manifest_hash /
entries[].manifest_hash / entries[].replay_result_hash:
  expected_value = "sha256:<lower-hex>"
  actual_value = "invalid_hash_format"

total_challenges / replayed_challenges / unexpected_acceptances:
  expected_value = "non_negative_i64"
  actual_value = "integer_out_of_range"

entries[].challenge_id:
  expected_value = "challenge_id"
  actual_value = "invalid_name_format"

entries[].comparison_status:
  expected_value = "NormalizedCheckResult.comparison.status"
  actual_value = "invalid_enum"
```

entries[] entry field order は `challenge_id`、`manifest_hash`、`replay_result_hash`、
`comparison_status` です。
unknown field の duplicate object key は unknown field priority 位置で報告し、
`expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` にします。
duplicate ではない unknown field では
`expected_value = "absent"`、`actual_value = "unknown_field"` にします。
unknown field 名を `schema_path_component` で表現できない場合は raw key を copy せず、
top-level object では `field = "$"`、entries[] entry では `field = "entries[<i>]"` にします。
release bundle の Step 6 など caller-prefixed field を使う場合、artifact-local `$` は
`coverage_summary.artifact`、root 以外の artifact-local path は
`coverage_summary.artifact.<artifact-local path>` に置換します。
entries order violation の `<i>` は、最初に `(challenge_id, manifest_hash)` が
直前 entry より component-wise bytewise 小さくなる後続 entry index です。
隣接 `(challenge_id, manifest_hash)` が等しい場合は `order_violation` ではなく
duplicate `(challenge_id, manifest_hash)` として扱います。
entries order violation では `field = "entries[<i>]"`、
`expected_value = "challenge_id_manifest_hash_bytewise_ascending"`、
`actual_value = "order_violation"` にします。
duplicate `(challenge_id, manifest_hash)` の `<i>` は、同じ pair がすでに出現している
最小の後続 entry index です。
この場合は `field = "entries[<i>].challenge_id"`、
`expected_value = "unique_challenge_id_manifest_hash_pair"`、
`actual_value = "duplicate_challenge_manifest"` にします。
duplicate `replay_result_hash` の tie-break も、同じ hash がすでに出現している
最小の後続 entry index です。
...
`replayed_challenges` が `entries.length` と一致しない場合は
`field = "replayed_challenges"`、`expected_value = "entries.length"`、
`actual_value = "count_mismatch"` の domain failure です。
`replayed_challenges` が `total_challenges` より大きい場合は
`field = "replayed_challenges"`、`expected_value = "at_most_total_challenges"`、
`actual_value = "count_mismatch"` の domain failure です。
`unexpected_acceptances` が `replayed_challenges` より大きい場合は
`field = "unexpected_acceptances"`、`expected_value = "at_most_replayed_challenges"`、
`actual_value = "count_mismatch"` の domain failure です。
複数の count relationship domain failure が同時に成立する場合は、
`replayed_challenges` vs `entries.length`、`replayed_challenges` vs `total_challenges`、
`unexpected_acceptances` vs `replayed_challenges` の順で最初の failure を返します。
...
同じ selected manifest entry で `checker_id` と `binary_id` が両方 mismatch する場合は、
`checker_id` mismatch を先に報告します。
...
この JSON path に unknown field または unknown field の duplicate key を含める場合は、
3 の `schema_path_component` rule を適用します。
raw key を表現できない場合は containing object path を使います。
CheckerIdentityManifest の containing object path は `checker_identity_manifest`、
`checker_identity_manifest.generated_by`、または
`checker_identity_manifest.checkers[<i>]` です。
...
`error.field` は重複した後続 unknown field の JSON path にします。
unknown field 名を `schema_path_component` で表現できない場合は raw key を copy せず、
上の containing object path を使います。
...
- model identity
- redaction policy
- source and input_policy references
- generated summary
- generated next actions
```

ここでの source and input_policy references は、`AiAuditSidecar.source` と
`AiAuditSidecar.input_policy` copied metadata を指します。
MVP の `AiAuditSidecar` には独立した `input_artifacts` / `input_artifact_list`
field を追加しません。
AI input construction の再現に必要な transient input は 7 の `ai_audit_prompt_input`
projection として再計算し、sidecar schema には保存しません。
...
MVP の default prompt payload は、7 の `ai_audit_prompt_input` projection で表します。
default `AiAuditInputPolicy.included_fields` は machine_result triage の baseline policy です。
この default policy を `source.kind = normalized_comparison` に使っても schema-valid ですが、
normalized_comparison では machine_result 専用 field が解決されず、`comparison.status` や
`comparison.status_reasons` も含まれません。
この場合、default policy 内の shared field path である `module` だけは
`NormalizedCheckResult.artifact.module` から解決され、`fields.module` に入ります。
`status` は unqualified machine_result-only field path なので、
`source.kind = normalized_comparison` では `comparison.status` として解釈せず omit します。
normalized_comparison diagnostic を出す運用では、下の normalized_comparison allowed set から
必要な `comparison.*` field path を含む policy を使います。
ReleasePolicy 配下で machine_result と normalized_comparison の両方を扱う場合は、
source kind ごとに別 policy を使うのではなく、両方の field path を含む単一の union policy を
`ReleasePolicy.ai_triage.input_policy_hash` に指定します。
PR mode や ad hoc validation では、`npa-check audit-sidecar validate` の各 invocation に
明示した1つの `AiAuditInputPolicy` file / hash がその sidecar の input policy source です。
default `AiAuditInputPolicy.included_fields` は次の field path だけを使います。
...
`certificate_hash`、`checker_binary_id`、`checker_binary_hash`、`checker_id`、
`checker_version`、`error.core_path`、`error.declaration`、`error.kind` は
source artifact に存在する場合だけ projection に入ります。
`checker_binary_id` と `checker_binary_hash` は checker process 起動後の result では存在しますが、
pre-launch failure では omit されるため、default `included_fields` に含まれていても
解決できない場合は `fields` に入れません。
`input_file_hash` と `expected_certificate_hash` は default prompt payload には含めません。
これらが必要な `machine_result` diagnostic では、policy の `included_fields` に明示し、
sidecar source に `source.normalized_result_hash` を入れます。
同じ policy を normalized_comparison 用 union policy として使う場合でも、
この2 field は machine_result source では normalized membership を要求します。
MVP では field path ごとの source-kind-only intent は表現しません。
`source.kind = normalized_comparison` の diagnostic では、`comparison.status` や
`comparison.status_reasons` などの normalized-comparison namespace の field path を使います。
...
必要な場合だけ、7 の allowed field path set の範囲内で policy に明示して追加します。
追加した情報は `AiAuditSidecar.input_policy.included_fields` に copied metadata として記録します。
MVP では `policy mode`、dependency hash path、relevant previous failures を表す
`included_fields` は定義しません。
`allow_source_text` / `allow_tactic_trace` は sidecar attachment field の保存可否を gate するだけで、
`ai_audit_prompt_input.v1` の field path allowlist を広げません。
MVP の AI input builder は source text、source excerpt、theorem statement、proof script、
tactic trace、elaboration trace、AI search trace を model input に渡してはいけません。
これらを AI input に含める必要が出た場合は、既存 field path に暗黙的な意味を足さず、
新しい field path、source mapping、redaction rule、schema version を追加します。
...
MVP の `TrainingExample.source` / `identity` / `input` field source は次で固定します。

```text
source.kind:
  fixed string "machine_result"
source.result_hash / request_hash / run_artifact_hash:
  resolved MachineCheckResult.result_hash / request_hash / run_artifact_hash

identity.artifact_input_file_hash:
  enclosing NormalizedCheckResult.artifact.input_file_hash
identity.artifact_expected_certificate_hash:
  enclosing NormalizedCheckResult.artifact.expected_certificate_hash
identity.certificate_hash:
  resolved MachineCheckResult.certificate_hash, when present
identity.checker_id:
  resolved MachineCheckResult.checker.id, when present
identity.checker_build_hash:
  resolved MachineCheckResult.checker.build_hash, when present
identity.checker_profile:
  resolved MachineCheckResult.checker.profile
identity.result_hash:
  resolved MachineCheckResult.result_hash
identity.policy_hash:
  resolved MachineCheckResult.policy.hash
identity.policy_version:
  resolved MachineCheckResult.policy.version

input.module:
  enclosing NormalizedCheckResult.artifact.module
input.checker_profile:
  matched NormalizedCheckResult.results[*].checker_profile
input.policy_id:
  resolved MachineCheckResult.policy.id
```

MVP の `TrainingExample.input` known field は `module`、`checker_profile`、`policy_id` だけです。
この3 field は required で、`TrainingExample.input` は closed-world object です。
`TrainingExample.input` は `AiAuditInputPolicy.included_fields` とは独立した schema であり、
`npa-check training export` は `AiAuditInputPolicy` file を読まず、任意の field path 追加も受け付けません。
ここでの allowlist discipline とは、上の3 field だけを copy し、
full certificate bytes、full proof term、source text、tactic trace、absolute path、secret を入れないという意味です。
`policy_id` は `TrainingExample.input` の field 名であり、source mapping は `MachineCheckResult.policy.id` です。
AI prompt policy の `policy.id` field path をそのまま training schema の key 名にしてはいけません。
`trust_mode` は MVP の `TrainingExample.input` には含めません。
`npa-check training export` は RunnerPolicy file や MachineCheckRequest file を入力に取らないため、
policy id から trust mode を推測してはいけません。
将来 `trust_mode` などを追加する場合は schema version を上げ、command input と source mapping を定義します。
...
deduplicate は、cross-store matching 後に各 normalized result entry から生成される
candidate `TrainingExample` が、同じ `run_artifact_hash` group 内で
3.3 の canonical serialization bytes として完全一致する場合だけ許可します。
candidate `TrainingExample` は上記の source mapping に従い、dedup 前の各
normalized result entry を enclosing source として作ります。
同じ `run_artifact_hash` group に bytewise different な candidate がある場合、
実装は先勝ち / 後勝ち / 任意の代表選択をしてはいけません。
この場合は `CommandError.reason_code = training_export_generation_failed`、
`field = "command"`、`actual_value = "training_export_generation_failed"` です。
group の検査順は `source.run_artifact_hash` の bytewise lexicographic order、
同じ group 内の candidate 比較順は normalized store manifest order、
その中の `NormalizedCheckResult.results[]` order で固定します。
...
JSON Lines file bytes は、record order に並べた各 `TrainingExample` object を
3.3 の canonical serialization rule で UTF-8 JSON bytes にし、各 record の直後に
single LF byte (`0x0a`) を1つ付けて連結したものです。
最終 record にも LF を付けます。
CRLF、BOM、空白 padding、blank line、pretty print、non-canonical key order は forbidden です。
`record_count = 0` の場合、JSON Lines file bytes は empty bytes で、LF だけの file を作ってはいけません。
`TrainingExportManifest.file_hash` はこの exact JSON Lines file bytes の SHA-256 です。
...
schema / order / duplicate failure の `<store>` prefix は `normalized_store` または
`result_store` に固定します。
manifest-local root marker `$` は `<store>` に置換し、root 以外の local path は
`<store>.<manifest-local path>` とします。
unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合は、
store manifest common rule の containing object path を使い、`normalized_store` /
`normalized_store.results[<i>]` または `result_store` / `result_store.results[<i>]` に置換します。
...
training export の validation / commit failure order は次で固定します。
この順序は同時 failure の reason_code tie-break order であり、field 説明順ではありません。

```text
1. CLI argument validation:
   duplicate singleton flag, unsupported flag, missing --json.
   No CommandError body.

2. input reference shape:
   input_reference_invalid

3. store file readable:
   input_file_unreadable

4. store file hash:
   input_hash_mismatch

5. store manifest JSON / schema / order / duplicate:
   input_store_manifest_invalid

6. store entry artifact validation:
   training_export_generation_failed

7. normalized result to machine result cross-store matching:
   training_export_generation_failed

8. run_artifact_hash dedup candidate consistency:
   training_export_generation_failed

9. output path conflict:
   output_path_conflict
...
Step 2 の内部優先順位は、`normalized_store`、`result_store`、`out.path`、
`manifest_out.path` の順です。
同じ store reference 内では、完全欠落、片側指定、path schema violation、
hash format violation の順で最初の failure を返します。
同じ store reference で path schema violation と hash format violation が同時に成立する場合は、
path schema violation を先に返します。
`out.path` と `manifest_out.path` では欠落を path schema violation より先に返します。
Step 3 から Step 5 で `normalized_store` と `result_store` の両方に failure がある場合は、
`normalized_store` を先に返します。
同じ store manifest 内の schema / order / duplicate failure tie-break は
store manifest common rule に従います。

Step 6 の store entry artifact validation では、まず `normalized_store` entry を
`normalized_store.results[]` order で検査し、次に `result_store` entry を
`result_store.results[]` order で検査します。
同じ entry 内では、referenced file missing / unreadable、invalid JSON、
artifact schema / domain failure、entry `file_hash` mismatch、artifact self-hash mismatch、
manifest-field mismatch の順で最初の failure を返します。
Step 6 の failure は詳細種別にかかわらず `training_export_generation_failed` の
default field shape を使い、`field = "command"`、
`actual_value = "training_export_generation_failed"` とします。

Step 7 の cross-store matching は、すべての store entry artifact validation が通った後にだけ評価します。
対象は normalized store manifest order の `NormalizedCheckResult`、その中の `results[]` order で
走査します。
各 `NormalizedCheckResult.results[*]` の
`(result_hash, request_hash, checker_profile, policy_hash)` に一致する
`MachineCheckResult` が result store に0件または2件以上ある場合、最初に該当した
normalized result entry で `training_export_generation_failed` を返します。
同じ `run_artifact_hash` が複数の normalized result entry から参照される deduplication は、
この cross-store matching がすべて成功した後にだけ行います。

Step 8 の dedup candidate consistency は、Step 7 がすべて成功した後、
output path conflict を調べる前に評価します。
同じ `run_artifact_hash` group では、最初の candidate `TrainingExample`
canonical bytes を比較基準として使い、後続 candidate の canonical bytes と比較します。
全 candidate が bytewise equal の group だけを1 record に deduplicate します。
最初に bytewise difference が見つかった group / candidate で
`training_export_generation_failed` を返します。

Step 9 と Step 10 で JSON Lines output と manifest output の両方に failure がある場合は、
`out.path` を `manifest_out.path` より先に返します。
JSON Lines output の配置が成功していない場合、manifest output の write / conflict を
成功条件として扱ってはいけません。
...
`npa-check audit-sidecar validate --schema-only --sidecar ...` は sidecar-local validation だけを行います。
ここでの sidecar-local validation には JSON schema、source shape、static forbidden field、
status / classification enum、hash string syntax、sidecar copied `input_policy.included_fields` の
local schema / domain、`required_for_input_field` を含めます。
source artifact、store manifest、または input policy file は読みません。
...
`--attempt` が整数でない、0 以下、または i64 範囲外の場合は CLI argument validation error であり、
`MachineCheckResult` や `MachineCheckRequestErrorResult` を作りません。
...
comparison_field_mismatch` のいずれかを入れます。
...
schema / order / duplicate failure の `<store>` prefix は caller field 名を使い、
manifest-local root marker `$` は `<store>` に置換し、root 以外の local path は
`<store>.<manifest-local path>` とします。
unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合は、
manifest 種別ごとの containing object path を使い、`axiom_report_store` /
`axiom_report_store.reports[<i>]` または `result_store` / `result_store.results[<i>]` に置換します。
...
schema / order / duplicate failure の `<store>` prefix は caller field 名を使い、
`challenge_store`、`replay_store`、または `result_store` のいずれかに固定します。
manifest-local root marker `$` は `<store>` に置換し、root 以外の local path は
`<store>.<manifest-local path>` とします。
`challenge_store` は `ChallengeOutputStoreManifest` validator の virtual root
`challenge_output_store` を `challenge_store` に置換します。
たとえば `challenge_output_store.entries[<i>].manifest_path` は
`challenge_store.entries[<i>].manifest_path`、unrepresentable unknown key の fallback
`challenge_output_store.entries[<i>]` は `challenge_store.entries[<i>]` です。
`replay_store` と `result_store` は store manifest common rule の local `$` /
`results[<i>]` をそれぞれ `replay_store` / `replay_store.results[<i>]`、
`result_store` / `result_store.results[<i>]` に置換します。
...
3. optional response / sidecar status gating:
   ...
   included CompareValidationResult.status != valid では
   field = "compare_validation_response[<i>].artifact.status"、
   expected_value = "valid"、actual_value = "failed"。
   optional AiAuditSidecar に対応する response が0件の場合は
   field = "ai_sidecar[<i>].validation_response"、
   expected_value = "exactly_one_audit_sidecar_validation_response"、
   actual_value = "missing"。
   複数件の場合は同じ field / expected_value で actual_value = "duplicate"。
   対応する response が1件で AuditSidecarValidationResult.status != valid の場合は
   field = "audit_sidecar_validation[<j>].artifact.status"、
   expected_value = "valid"、actual_value = "failed"。
   ここで <i> は対象 ai_sidecar の occurrence index、<j> は対応する
   audit_sidecar_validation_response の occurrence index。
   この class 内では included response 自体の status failure を先に評価し、
   その後で ai_sidecar と validation response の対応付け cardinality を評価します。
   この status-first rule は class 3 内の local priority であり、
   下の release bundle artifact kind order より優先します。
   複数の included response status failure が同時に成立する場合は、
   release bundle artifact kind order を response artifact kind に制限した順序
   （audit_sidecar_validation_response、compare_validation_response）で選び、
   同じ response artifact kind 内では occurrence order で最初の failure を返します。
   `AuditSidecarValidationResult.status != valid` の response は対応付け候補にせず、
   matching key field が omit されていても missing / extra response へ分類しません。
   `status = valid` の response で source kind 上 required な matching key field が
   omit されている場合は Step 6 schema / domain failure であり、この class には到達しません。
   source kind 上 inapplicable な omitted field は対応付け failure ではありません。

4. release audit bundle closed-set cardinality:
   required artifact kind missing、duplicate、extra / forbidden artifact kind。
   ReleasePolicy.ai_triage.enabled = false で included ai_audit_sidecar が存在する場合は
   field = "ai_sidecar[<i>]"、
   expected_value = "absent_when_ai_triage_disabled"、actual_value = "present"。
   ReleasePolicy.ai_triage.enabled = false で included audit_sidecar_validation_response が存在する場合は
   field = "audit_sidecar_validation[<j>]"、
   expected_value = "absent_when_ai_triage_disabled"、actual_value = "present"。
   これらの forbidden artifact failure は、下の
   audit_sidecar_validation_response-to-ai_sidecar 対応付け failure より先に評価します。
   `mode = cross_artifact` かつ `status = valid` の included
   audit_sidecar_validation_response が included ai_audit_sidecar に0件対応する場合は
   field = "audit_sidecar_validation[<j>].ai_sidecar"、
   expected_value = "exactly_one_ai_audit_sidecar"、actual_value = "missing"。
   複数件に対応する場合は同じ field / expected_value で actual_value = "duplicate"。
   この failure の artifact kind は audit_sidecar_validation_response。
   included compare_validation_response が included normalized_check_result に0件対応する場合は
   field = "compare_validation_response[<i>].normalized_result"、
   expected_value = "exactly_one_normalized_check_result"、actual_value = "missing"。
   複数件に対応する場合は同じ field / expected_value で actual_value = "duplicate"。
   同じ normalized_result_hash に対する compare_validation_response が複数ある場合は
   field = "compare_validation_response[<i>].normalized_result_hash"、
   expected_value = "unique_compare_validation_response"、actual_value = "duplicate"。
   対応する normalized_check_result が release target でも challenge replay target でもない場合は
   field = "compare_validation_response[<i>].normalized_result"、
   expected_value = "release_or_challenge_replay_normalized_result"、actual_value = "forbidden"。
   これらの failure の artifact kind は compare_validation_response。
   ここで ai_sidecar の <i>、audit_sidecar_validation の <j>、
   compare_validation_response の <i> は、それぞれ対象 artifact kind の occurrence index です。
   duplicate-response では同じ normalized_result_hash を持つ2件目以降のうち、
   occurrence index が最小の compare_validation_response を報告します。
...
5. cross-artifact identity / policy / mode / status / recomputation mismatch:
   ...
   normalized_check_result source-copy、normalized comparison recomputation、auxiliary selector、
   required auxiliary_result status、compare response recomputation、audit-sidecar recomputation などの不一致。
   normalized_check_result source-copy mismatch は
   `normalized_check_result` class 5 failure であり、
   field は `normalized_check_result[<j>].artifact.results[<i>].<member>`。
   full normalized_check_result set validation で同じ normalized_check_result entry を検査する場合は、
   source-copy mismatch を
   embedded comparison recomputation mismatch より先に返す。
   compare_validation_response policy hash mismatch は
   field = "compare_validation_response[<i>].artifact.policy_hash"、
   expected_hash = target-specific RunnerPolicy canonical hash、
   actual_hash = parsed CompareValidationResult.policy_hash。
   compare_validation_response recomputation mismatch は
   field = "compare_validation_response[<i>].artifact"、
   expected_hash = sha256(canonical(re-run CompareValidationResult))、
   actual_hash = sha256(canonical(saved CompareValidationResult))。
   同じ compare_validation_response entry では policy hash mismatch を
   recomputation mismatch より先に返す。
   ai_audit_sidecar source outside release audit sidecar source closed set は
   field = "ai_sidecar[<i>].artifact.source"、
   expected_value = "release_audit_optional_sidecar_source"、
   actual_value = "forbidden_source"。
   ai_audit_sidecar release-bundle source-value mismatch も
   audit_sidecar_validation_response recomputation mismatch より先に返す。
   audit_sidecar_validation_response recomputation mismatch は
   field = "audit_sidecar_validation[<j>].artifact"、
   expected_hash = sha256(canonical(re-run AuditSidecarValidationResult))、
   actual_hash = sha256(canonical(saved AuditSidecarValidationResult))。
   ここで sha256(canonical(...)) は診断用の canonical object digest であり、
   artifact の result_hash ではありません。
```
...
ただし下流 artifact kind の expected set、optional response binding、
または class 5 semantic validation の入力が別 artifact kind の内容から導出される場合、
前提 artifact が prerequisite-clean になるまで、その下流 kind の
missing / forbidden / duplicate failure、source-copy mismatch、recomputation mismatch、
selector / source-value mismatch、status binding mismatch を合成してはいけません。
この prerequisite gate は、依存する下流 kind の class 4 closed-set /
response-binding failure と、下の gate table が明示する依存 class 5 validation
（source-copy、recomputation、selector / source-value、status binding、downstream expected-set derivation）
に適用します。
...
normalized_check_result:
  ...
  Full normalized_check_result set validation uses prerequisites in this order:
    ...
    4. prerequisite-clean machine_check_result set for normalized_check_result source-copy validation.
  If any prerequisite is not prerequisite-clean, report the first non-clean prerequisite in this order
  before deriving normalized_check_result source-copy mismatch.
  The machine_check_result prerequisite applies only to full normalized_check_result set validation and
  does not block the earlier release target comparison recomputation used only to derive
  challenge replay normalized_result expected identities.
  After prerequisites 1-3 are clean, the normalized_check_result expected set is derivable.
  normalized_check_result class 4 closed-set exactness for missing / extra / duplicate /
  forbidden entries is therefore evaluated as a normal class 4 failure and is not gated
  by prerequisite 4. Global Step 8 class order and artifact kind order still apply:
  if an independent machine_check_result class 4 failure and a normalized_check_result
  class 4 failure both exist, machine_check_result is reported first by artifact kind order;
  if the machine_check_result prerequisite failure is class 5 and normalized_check_result
  closed-set exactness has a class 4 failure, the normalized_check_result class 4 failure
  is reported first.
  After normalized_check_result closed-set exactness succeeds, prerequisite 4 is evaluated
  before source-copy validation, before full-set embedded comparison recomputation for
  allowed normalized results, before replay-to-normalized / summary-to-replay status
  binding validation, and before deriving downstream expected sets from
  `normalized_check_result`.
  If prerequisite 4 is not clean, return that `machine_check_result` prerequisite failure
  and do not synthesize any of those later normalized_check_result class 5 failures.
  Here `machine_check_result` prerequisite failure names the prerequisite tree being
  evaluated, not the artifact kind to rewrite the failure into. The returned failure
  preserves the source artifact kind, class, field, and payload of the first failure
  produced while evaluating that prerequisite tree. For example,
  `ChallengeReplayResult.checker_results[*]` reference-copy mismatch remains a
  `challenge_replay_result` class 5 failure, and a reproducibility selector failure
  remains an `auxiliary_result` class 5 failure.
  ...
  challenge_replay_result set is prerequisite-clean for normalized_check_result iff:
    ...
    - the set exactly matches ChallengeCoverageSummary.entries[*].replay_result_hash
      using the `result_hash` value component of the challenge_replay_result identity.
      The tuple wrapper `("result_hash", result_hash)` is only the missing-key ordering
      representation and is not compared byte-for-byte against `replay_result_hash`.
    ...
    - each ChallengeReplayResult is matched to the unique summary entry whose
      replay_result_hash equals that ChallengeReplayResult's `result_hash` value component,
    ...
    `checker_results[*]` reference-copy mismatches against resolved MachineCheckResult
    are not part of the normalized_check_result prerequisite. They belong only to
    the machine_check_result prerequisite below, because normalized_check_result
    expected-set derivation intentionally does not require
    `checker_results[*].run_artifact_hash` extraction.
  ...
  normalized_check_result set is prerequisite-clean iff:
    ...
    - source-copy validation succeeds for every allowed included normalized_check_result
      entry against the selected source MachineCheckResult artifacts,
    ...
    Source-copy validation uses the `MachineCheckResult` source-copy set in section 6.
    For the release target normalized_check_result, the selected source artifacts are the
    release target raw results chosen by the machine_check_result allowed run set rule.
    Each release target `results[]` entry is paired with the selected raw result for that same
    entry as established by the release target raw result selection rule. If that selection is
    missing, ambiguous, or fails the equality required by that rule, report the earlier
    machine_check_result prerequisite failure instead of a source-copy mismatch.
    Equality mismatch reporting uses the rule-specific member order
    result_hash / request_hash / checker_profile / policy_hash; this does not change the
    serialized selection tuple order.
    For a challenge replay normalized_check_result, the selected source artifacts are the
    `MachineCheckResult` artifacts referenced by `checker_results[*].run_artifact_hash`
    from every included `ChallengeReplayResult` whose `normalized_result_hash` equals that
    normalized result. A challenge replay source artifact is paired with the unique saved
    normalized `results[]` entry whose `checker_profile` equals
    `MachineCheckResult.checker.profile`.
    If no such entry exists, report normalized_check_result class 5 with
    `field = "normalized_check_result[<j>].artifact.results"`、
    `expected_value` には selected source の actual `MachineCheckResult.checker.profile` string を入れ、
    `actual_value = "missing_entry_for_source_profile"`。
    If more than one such entry exists, report the second matching entry by smaller
    `results[]` index with
    `field = "normalized_check_result[<j>].artifact.results[<i>].checker_profile"`、
    `expected_value = "unique_checker_profile"`、
    `actual_value = "duplicate_entry_for_source_profile"`。
    If a saved normalized `results[]` entry has no selected source artifact with the same
    `checker_profile`, report normalized_check_result class 5 with
    `field = "normalized_check_result[<j>].artifact.results[<i>].checker_profile"`、
    `expected_value = "selected_source_checker_profile"`、
    `actual_value` には saved normalized entry の actual `checker_profile` string を入れる。
    After this pairing succeeds, `result_hash`、`request_hash`、`policy_hash`、and
    `checker_profile` themselves are still checked by the source-copy set table.
    Challenge replay source-copy pairing failures are reported before member value
    mismatches. If multiple pairing failures exist in the same normalized_check_result,
    report duplicate saved `checker_profile` by the smaller second `results[]` index first,
    then saved normalized entries without a selected source by smaller `results[]` index,
    then selected sources without a saved normalized entry by `challenge_replay_result`
    occurrence order and `checker_results[]` order.
    Source-copy mismatch is reported as normalized_check_result class 5
    after class 4 closed-set exactness and before embedded comparison recomputation.
    If multiple selected source artifacts map to the same normalized `results[]` entry,
    compare sources in `challenge_replay_result` occurrence order, then
    `checker_results[]` order inside that replay result; the first source whose
    source-copy member differs from the saved normalized entry provides the
    expected hash/value payload.
    Multiple source-copy mismatches in the same normalized_check_result are reported by
    smaller `results[]` index first, then by the source-copy set table order in section 6.
...
compare_validation_response:
  Its normalized_result binding, duplicate-response, target classification
  class 4 failures, policy-hash class 5 failure, and recomputation class 5 failure
  require prerequisite-clean normalized_check_result set.
  If normalized_check_result set is not prerequisite-clean, report that
  normalized_check_result prerequisite failure first and do not derive
  compare_validation_response missing / duplicate / duplicate-response /
  target-forbidden / policy-hash mismatch / recomputation mismatch failures.
  This prerequisite does not block Step 6 schema / domain validation of the
  included CompareValidationResult artifact, nor class 3
  CompareValidationResult.status failure.
...
machine_check_result:
  ...
  challenge_replay_result set is prerequisite-clean for machine_check_result iff:
    ...
    `checker_results[*].run_artifact_hash` extraction only derives the expected
    machine_check_result identities. It does not trust the copied
    `checker_results[*].result_hash` or `checker_results[*].checker_profile`
    values; those are checked after the referenced MachineCheckResult entries exist.
  reproducibility auxiliary_result set is prerequisite-clean iff:
    ...
    entries exist and those targets have passed MachineCheckResult result_hash /
    run_artifact_hash self-consistency, selector target equality mismatches are
    `auxiliary_result` class 5 selector target failures. For
    `baseline_run_artifact_hash`, compare the selected MachineCheckResult against the
    release target baseline result entry in `result_hash`, `request_hash`,
    `checker_profile`, `policy_hash` order and use
    `field = "auxiliary_result[<a>].artifact.selector.baseline_run_artifact_hash"`.
    For `repeated_run_artifact_hash`, compare the selected MachineCheckResult against
    `selector.request_hash`, `selector.checker_profile`, and the release target baseline
    result entry `result_hash` / `policy_hash` in `request_hash`, `checker_profile`,
    `result_hash`, `policy_hash` order and use
    `field = "auxiliary_result[<a>].artifact.selector.repeated_run_artifact_hash"`.
    In both cases hash members use `expected_hash` / `actual_hash`, and
    `checker_profile` uses `expected_value` / `actual_value`; expected values come from
    the selector or release target baseline result entry, and actual values come from the
    selected MachineCheckResult. If baseline_run_artifact_hash and repeated_run_artifact_hash are
  ...
  machine_check_result set is prerequisite-clean iff:
    ...
    - every included ChallengeReplayResult.checker_results[*] reference that contributed
      a run_artifact_hash to the allowed run set has copied result_hash and checker_profile
      values matching the resolved MachineCheckResult,
    - reproducibility selector target equality succeeds for the referenced baseline and
      repeated MachineCheckResult entries,
    - every parsed request_hash is schema-valid and extractable.
    MachineCheckResult self-consistency failures are `machine_check_result` class 5
    identity / source-key failures. `result_hash` failure uses
    `field = "machine_check_result[<i>].artifact.result_hash"`.
    `<i>` is the `machine_check_result` occurrence index in the deterministic
    release bundle artifact kind order.
    If artifact entry `hashes.result_hash` exists and differs from parsed
    `MachineCheckResult.result_hash`, use `expected_hash = artifact entry hashes.result_hash`
    and `actual_hash = parsed MachineCheckResult.result_hash`. Otherwise use
    `expected_hash = recomputed MachineCheckResult.result_hash` and
    `actual_hash = parsed MachineCheckResult.result_hash`.
    `run_artifact_hash` failure uses
    `field = "machine_check_result[<i>].artifact.run_artifact_hash"` with the same
    artifact-entry-first payload rule. During `npa-check release bundle` generation,
    there is no materialized artifact entry yet, so this self-consistency check only
    needs parsed vs recomputed hashes after earlier input / store validation has succeeded.
    Multiple MachineCheckResult self-consistency failures are reported by
    `machine_check_result` occurrence order, then `result_hash` before
    `run_artifact_hash`.
    The allowed run set is considered derivable only after release target raw result
    selection has no non-baseline ambiguity. If that ambiguity exists, report the
    machine_check_result class 5 prerequisite failure with
    `field = "machine_check_result[<m>].artifact"`,
    `expected_value = "unique_release_target_raw_result_candidate"`, and
    `actual_value = "duplicate_candidate_for_normalized_check_result[<j>].artifact.results[<i>]"`.
    Choose the smaller release target `results[]` index `<i>` first, then the smaller
    duplicate candidate `machine_check_result` occurrence index `<m>`. Do not evaluate
    machine_check_result closed-set exactness, missing entries, or forbidden entries
    from a partially derived allowed run set.
    Reproducibility selector target equality is evaluated only after machine_check_result
    class 4 exactness and the referenced target MachineCheckResult self-consistency have
    passed. If the baseline or repeated target has a self-consistency failure, report that
    machine_check_result class 5 failure first and do not synthesize an auxiliary_result
    selector target mismatch from untrusted target fields.
    After class 4 exactness and MachineCheckResult self-consistency have passed,
    ChallengeReplayResult checker_results reference-copy mismatches and reproducibility
    selector target equality mismatches are both class 5 failures in the same
    `machine_check_result` prerequisite tree. If both exist, report the
    `challenge_replay_result` class 5 failure first by the global artifact kind order;
    the bullet order above is a cleanliness condition list, not an override of that
    tie-break.
    A ChallengeReplayResult checker_results reference-copy mismatch is a
    challenge_replay_result class 5 source-key failure, even though it makes the
    machine_check_result set non-clean. The resolved source is the unique included
    MachineCheckResult whose parsed run_artifact_hash equals
    `checker_results[<k>].run_artifact_hash`, after machine_check_result class 4
    exactness and MachineCheckResult self-consistency have passed.
    result_hash mismatch uses
    `field = "challenge_replay_result[<i>].artifact.checker_results[<k>].result_hash"`,
    `expected_hash = resolved MachineCheckResult.result_hash`, and
    `actual_hash = ChallengeReplayResult.checker_results[<k>].result_hash`.
    checker_profile mismatch uses
    `field = "challenge_replay_result[<i>].artifact.checker_results[<k>].checker_profile"`,
    `expected_value = resolved MachineCheckResult.checker.profile`, and
    `actual_value = ChallengeReplayResult.checker_results[<k>].checker_profile`.
    Multiple checker_results reference-copy mismatches are reported by
    challenge_replay_result occurrence order, then `checker_results[]` index, then
    result_hash before checker_profile.
...
machine_check_request:
  ...
  MachineCheckRequest self-consistency failures are `machine_check_request` class 5
  identity / source-key failures with
  `field = "machine_check_request[<i>].artifact.request_hash"`.
  `<i>` is the `machine_check_request` occurrence index in the deterministic
  release bundle artifact kind order.
  If artifact entry `hashes.request_hash` exists and differs from parsed
  `MachineCheckRequest.request_hash`, use `expected_hash = artifact entry hashes.request_hash`
  and `actual_hash = parsed MachineCheckRequest.request_hash`. Otherwise use
  `expected_hash = recomputed MachineCheckRequest.request_hash` and
  `actual_hash = parsed MachineCheckRequest.request_hash`.
  During `npa-check release bundle` generation, there is no materialized artifact entry
  yet, so this self-consistency check only needs parsed vs recomputed request_hash
  after earlier input / store validation has succeeded. Multiple MachineCheckRequest
  self-consistency failures are reported by `machine_check_request` occurrence order.
...
audit_sidecar_validation_response:
  Its recomputation class 5 failure requires the matched included ai_audit_sidecar
  source validation to be clean.
  If the matched included ai_audit_sidecar source is outside the release audit
  bundle AI sidecar source closed set, report that ai_audit_sidecar class 5
  source failure first and do not derive audit_sidecar_validation_response
  recomputation mismatch for that response.
  If the matched included ai_audit_sidecar source kind prerequisites are not
  prerequisite-clean, report that prerequisite failure first and do not derive
  audit_sidecar_validation_response recomputation mismatch for that response.
  If the matched included ai_audit_sidecar source passes closed-set membership
  and prerequisites but fails release-bundle source-value checks, report that
  ai_audit_sidecar class 5 source-value failure first and do not derive
  audit_sidecar_validation_response recomputation mismatch for that response.
  Source-value checks include machine_result run_artifact_hash membership in
  the release optional machine_result source set, result_hash / request_hash
  equality with the resolved MachineCheckResult, machine_result optional
  normalized_result_hash equality / membership when present, reproducibility
  repeated-source normalized_result_hash absence, and normalized_comparison
  normalized_result_hash equality with the release target NormalizedCheckResult.
  This prerequisite does not block Step 6 schema / domain validation of the
  included AuditSidecarValidationResult artifact, schema_only mode domain failure,
  class 3 AuditSidecarValidationResult.status failure, or class 4 response-to-sidecar
  cardinality failures for status-valid cross_artifact responses.
```
...
この表は missing expected key の表現と並び順だけを定義します。
generation / validate-bundle で既存 candidate をどの manifest field から読むかは、
各 artifact kind の closed-set identity source 規則に従い、この表を candidate discovery rule として
読んではいけません。
...
machine_check_request: ("request_hash", request_hash)
machine_check_result:
  missing run_artifact_hash reference:
    ("run_artifact_hash", run_artifact_hash)
  release target NormalizedCheckResult.results[*] raw result with zero matching candidate:
    ("release_target_result", checker_profile, request_hash, result_hash, policy_hash)
normalized_check_result: normalized_result_hash
challenge_manifest: manifest_hash
challenge_replay_result: ("result_hash", result_hash)
...
Release target raw result selection が1件の `machine_check_result` candidate を選べた場合、
その selected concrete raw result はこの missing expected key table へ追加しません。
その candidate は `machine_check_result` closed-set exactness の allowed existing identity として、
generation では validated result store entry の `run_artifact_hash`、
validate-bundle / `audit_bundle` では materialized manifest entry の
`hashes.run_artifact_hash` で扱います。
...
ReleasePolicy.ai_triage.enabled = false による `ai_audit_sidecar` /
`audit_sidecar_validation_response` forbidden artifact failure は、
status-valid response の対応付けを行う前に報告します。
ただし included status-valid `audit_sidecar_validation_response` が対応する included
`ai_audit_sidecar` を持たない、または複数の `ai_audit_sidecar` に対応する場合は、
上の class 4 command-specific field shape で余剰 / 曖昧な response として報告します。
included `compare_validation_response` が対応する included `normalized_check_result` を
持たない、複数持つ、同じ `normalized_result_hash` で重複する、または対象外 normalized result を
指す場合も、上の class 4 command-specific field shape で報告します。
ただしこの compare response binding failure は、上の prerequisite gate で定義する
normalized_check_result set が prerequisite-clean な場合だけ合成します。
`compare_validation_response` の policy hash / recomputation class 5 failure と
`audit_sidecar_validation_response` の recomputation class 5 failure も、
上の prerequisite gate を満たす場合だけ合成し、field shape は上の
command-specific 定義に従います。
...
ai_audit_sidecar:
  $ -> ai_sidecar[<i>].artifact
  source -> ai_sidecar[<i>].artifact.source
  input_policy -> ai_sidecar[<i>].artifact.input_policy
  ai -> ai_sidecar[<i>].artifact.ai
  classification -> ai_sidecar[<i>].artifact.classification

audit_sidecar_validation_response:
  $ -> audit_sidecar_validation[<i>].artifact

compare_validation_response:
  $ -> compare_validation_response[<i>].artifact

request_store_manifest:
  request_store / request_store.$
    -> request_store.artifact

machine_result_store_manifest:
  result_store / result_store.$
    -> result_store.artifact

normalized_result_store_manifest:
  normalized_store / normalized_store.$
    -> normalized_store.artifact

challenge_output_store_manifest:
  challenge_output_store / challenge_output_store.$
    -> challenge_store.artifact
...
`challenge_output_store.entries[<j>].manifest_path` は
`challenge_store.artifact.entries[<j>].manifest_path` として報告します。
`AiAuditSidecar` の sidecar-local root marker `$` は
`ai_sidecar[<i>].artifact` として報告し、
`ai_sidecar[<i>].artifact.$` にはしません。
`CompareValidationResult` と `AuditSidecarValidationResult` の response-local root marker `$` も
それぞれ `compare_validation_response[<i>].artifact`、
`audit_sidecar_validation[<i>].artifact` として報告し、末尾に `.$` を残しません。
...
`machine_check_request[<i>]`、`machine_check_result[<i>]`、
`normalized_check_result[<i>]` は release bundle command の direct repeatable input ではなく
bundle-local store manifest / target normalized result から導出されますが、field shape 上の
`<i>` は同じ kind-specific occurrence index rule を使います。
`npa-check release bundle` generation では、生成予定
`ReleaseAuditBundleManifest.artifacts` の deterministic `(kind, path)` order における
同じ artifact kind だけの 0-based occurrence index を使い、source store manifest entry index や
CLI flag occurrence index を使ってはいけません。
generation で exact duplicate store entries が staging / store merge により deduplicate されている場合は、
deduplicate 後の生成予定 artifact entry を1件として数えます。
`npa-check release validate-bundle` / `audit_bundle` では、materialized manifest の
schema / order validation 後の同じ occurrence index を使います。
...
domain validation failure を `CommandError` として報告する場合、7 の `AiAuditInputPolicy`
...
この prefix 置換は `field` だけに適用し、`expected_value` / `actual_value` は
7 の `AiAuditInputPolicy` field shape の固定値をそのまま使います。
...
最初の member を報告します。
minimum audit envelope member 以外の top-level duplicate key、`artifacts[]` entry 内の
duplicate key、または `artifacts[].hashes` 内の duplicate key は、minimum audit envelope が
valid な後の bundle schema invalid として `audit_bundle_invalid` にします。
この場合の field shape は `ReleaseAuditBundleManifest` full schema priority と同じで、次で固定します。
...
artifacts[].hashes duplicate key after minimum envelope:
  error.reason_code = audit_bundle_invalid
  error.field = "manifest.artifacts[<j>].hashes"
  error.expected_value = "unique_object_keys"
  error.actual_value = "duplicate_field"
...
次に `artifacts[]` の array order、同じ entry 内では entry object、`hashes` object の順で
最初の duplicate を返します。
...
`integer_out_of_range`、`forbidden_field`、`failure_key_mismatch`、
`comparison_field_mismatch`、`duplicate_axiom_name`、`duplicate_request_hash`、
`duplicate_run_artifact_hash`、`duplicate_normalized_result_hash`、`duplicate_result_hash`、
`duplicate_challenge_manifest`、`duplicate_challenge_id`、`duplicate_manifest_path`、
`duplicate_axiom_report_hash`、`duplicate_module` のいずれかを入れます。
`input_schema_invalid`、`input_store_manifest_invalid`、または `input_store_entry_invalid` が
artifact / store validator の schema / domain failure を包む場合は、委譲先 validator が
固定した `actual_value` をそのまま使います。
その固定値が上の common list に含まれていなくても、generic な値へ置き換えてはいけません。
たとえば `profile_set_mismatch`、`profile_order_mismatch`、`duplicate_profile`、
`duplicate_binary_id`、`missing_budget_entry`、`unexpected_budget_entry`、
`invalid_fixed_value`、`empty_string`、`present`、`true` は、該当 validator が定義した場合に
そのまま許可されます。
...
この一覧は endpoint wrapper object の path field だけを対象にします。
`ChallengeManifest reference path` は `/machine/check/challenge/requests` と
`/machine/check/challenge/replay` の wrapper field `challenge_manifest.path` を指します。
`/machine/check/challenge` の wrapper path validation target は wrapper field の
`policy.path` だけです。
同 endpoint の `generation_request.base_certificate.path`、
`generation_request.imports.manifest`、`generation_request.output.store_manifest_path`、
`generation_request.output.manifest_path`、`generation_request.output.mutated_certificate_path` は
inline `ChallengeGenerationRequest` artifact の validation target であり、
API wrapper path validation target ではありません。
そのためこれらの path schema violation は `ApiError` ではなく `CommandError` として返します。

API wrapper の `ApiError` first-failure order は次に固定します。
dispatch では endpoint path、method、body JSON syntax の順に検査し、
body を parse できた後だけ wrapper object validation に進みます。
wrapper object validation では、まず top-level wrapper object 自体の wrong type / explicit null、
次に known top-level wrapper field の duplicate、required field missing、known field value shape、
unknown top-level field の順で最初の1件だけを返します。
top-level wrapper object 自体の failure では `field = "$"`、
`expected_value = "object"`、`actual_value = wrong_type | null_not_allowed` にします。
top-level known field の tie-break は endpoint ごとの wrapper field order に従います。
unknown top-level field が複数ある場合は raw field name の bytewise order で最小のものを返します。
unknown top-level field の duplicate object key は known field duplicate priority ではなく
unknown field priority 位置で
`expected_value = "unique_object_keys"`、`actual_value = "duplicate_field"` として返します。
したがって required known field missing と unknown top-level field duplicate が同時に存在する場合は、
required known field missing を先に返します。

Endpoint wrapper field order：

```text
/machine/check/certificate:
  check_request, policy, attempt
/machine/check/normalize:
  policy, request_store, artifact_selector, machine_results
/machine/check/compare:
  policy, normalized_result
/machine/check/challenge:
  generation_request, policy
/machine/check/challenge/requests:
  challenge_manifest, policy, request_output_dir, request_store_output_path
/machine/check/challenge/replay:
  coverage_required, challenge_manifest, policy, request_store, result_store, normalized_store
/machine/check/audit-sidecar/validate:
  schema_only, sidecar, result_store, normalized_store, input_policy
```

known field value shape の内部では、reference object 自体の missing / wrong type / explicit null を先に返し、
reference object が存在する場合は reference-local schema order で最初の wrapper schema failure を返します。
API wrapper schema validation での path member 位置は missing / wrong type / explicit null /
duplicate key だけを扱い、workspace-relative path grammar、symlink escape、workspace 外解決などの
path domain validation は含めません。
`RunnerPolicyReference` は 4.1 の `kind`、`path`、`hash` order を使いますが、
API wrapper schema phase の `path` step は string shape だけを検査します。
したがって同じ reference object 内で `policy.path` の workspace path validation failure と
`policy.hash` の invalid hash format が同時に成立する場合は、
`policy.hash` の `api_request_schema_invalid` を先に返します。
store reference と ChallengeManifest reference は `kind`、`path`、`manifest_hash` の順ですが、
同様に API wrapper schema phase の `path` step は string shape だけを検査するため、
`*.path` の workspace path validation failure と `*.manifest_hash` の invalid hash format が
同時に成立する場合は `*.manifest_hash` の `api_request_schema_invalid` を先に返します。
`sidecar` reference は `path`、`input_policy` validation reference は `path`、`hash` の順です。
inline artifact field は、wrapper validation では top-level field の missing / wrong type /
explicit null と container type だけを検査し、inline artifact 内部の schema / domain failure は
endpoint 固有 validation に渡します。
`/machine/check/normalize` の `machine_results` wrapper field は required non-empty array です。
`machine_results` 自体の missing / wrong type / explicit null / empty array は
`ApiError.reason_code = api_request_schema_invalid` とし、`field = "machine_results"`、
`expected_value = "non_empty_array"`、
`actual_value = missing | wrong_type | null_not_allowed | empty_array` にします。
`machine_results` が non-empty array として wrapper validation を通過した後は、
各 element を inline `MachineCheckResult` artifact として endpoint-specific intrinsic validation に渡します。
この array index order は、6 の normalizer validation order で定義する
`MachineCheckResult` intrinsic validation の input occurrence order でもあります。
`machine_results[<i>]` element 自体が explicit null または object 以外の場合は `ApiError` ではなく
`NormalizeErrorResult.error.reason_code = machine_result_wrong_schema` とし、normalizer の caller-prefix rule で
`error.field = "machine_results[]"`、`expected_value = "npa.phase8.machine_check_result.v1"`、
`actual_value = null_not_allowed | wrong_type` にします。
element が object だが top-level `schema` member が invalid な場合は、
`error.field = "machine_results[].schema"` を使います。
`/machine/check/normalize` の `artifact_selector` は endpoint-specific domain object なので、
top-level field 名の duplicate だけを wrapper failure とし、存在する value / member の failure は
`selector_schema_invalid` に渡します。
`/machine/check/certificate` の `attempt` wrapper field は optional です。
missing の場合は `attempt = 1` として扱い、存在する場合は positive i64 JSON integer だけを許可します。
`attempt` が wrong type / explicit null / 0 以下 / i64 範囲外の場合は
`ApiError.reason_code = api_request_schema_invalid` とし、`field = "attempt"`、
`expected_value = "positive_i64"`、
`actual_value = wrong_type | null_not_allowed | non_positive_integer | integer_out_of_range` にします。

wrapper schema validation failure と workspace path validation failure が同時に成立する場合は、
wrapper schema validation failure を先に返します。
wrapper schema validation が通過した後、複数の endpoint wrapper path が workspace path validation に
失敗する場合は endpoint wrapper field order に従い、同じ reference object 内では
`path` member を返します。
ただし `/machine/check/audit-sidecar/validate` の mode-dependent active validation reference については、
この section の下の duplicate-free active reference path rule と 7 の validation order を優先します。
...
`expected_value` に schema requirement 名、
`actual_value` に `missing`、`wrong_type`、`unknown_field`、`invalid_enum`、
`invalid_hash_format`、`null_not_allowed`、`duplicate_field`、`empty_array`、
`non_positive_integer`、`integer_out_of_range` のいずれかを入れます。
duplicate key の場合は `field` に duplicated field の JSON path、
`expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を入れます。
unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合は、
raw key を copy せず nearest containing object path を使います。
top-level wrapper object は `field = "$"`、nested reference / output wrapper object は
その object の field path、たとえば `policy`、`request_store`、`result_store`、
`normalized_store`、`input_policy`、`challenge_manifest`、`out`、`manifest_out` を使います。
...
`status = failed` では `error` と `error.kind` が required です。
...
`CompareValidationResult` response schema は closed-world です。
response-local JSON path の root marker は `$` です。
top-level object 自体の wrong type / null では `field = "$"`、
`expected_value = "object"`、`actual_value = "wrong_type"` または `"null_not_allowed"` にします。
top-level object の unknown field または duplicate unknown field 名を
`schema_path_component` で表現できない場合は、raw key を copy せず
`field = "$"` を使います。
`error` object 内の unknown field または duplicate unknown field 名を
`schema_path_component` で表現できない場合は、raw key を copy せず
`field = "error"` を使います。
known field の duplicate key はその known field の response-local JSON path を使います。
top-level field は `schema`、`status`、`error`、`normalized_result_hash`、`policy_hash`、
`embedded_comparison_status`、`recomputed_comparison_status` だけです。
`error` object 内の known field は `kind`、`reason_code`、`field`、
`expected_value`、`actual_value`、`expected_hash`、`actual_hash` だけです。
これ以外の field は unknown field です。
top-level `schema` missing / null / wrong type / mismatch では
`field = "schema"`、
`expected_value = "npa.phase8.compare_validation_result.v1"`、
`actual_value` に `missing`、`null_not_allowed`、`wrong_type`、または `invalid_enum` を入れます。
top-level `status` missing / null / wrong type / invalid enum では
`field = "status"`、
`expected_value = "CompareValidationResult.status"`、
`actual_value` に `missing`、`null_not_allowed`、`wrong_type`、または `invalid_enum` を入れます。
`status = valid` では `error` は forbidden で、
`field = "error"`、`expected_value = "absent_for_status"`、
`actual_value = "forbidden_field"` にします。
`status = failed` では `error` と `error.kind` が required です。
`status = failed` で `error` が missing、explicit null、または object 以外の場合は、
field = `error`、`expected_value = "object"`、
`actual_value` に `missing`、`null_not_allowed`、または `wrong_type` を入れます。
`error.kind` は上の `CompareValidationResult.error.kind` enum のいずれかでなければなりません。
`error.kind` の missing / null / wrong type / invalid enum は response schema / domain failure とし、
field = `error.kind`、`expected_value = "CompareValidationResult.error.kind"` を使います。
`error.reason_code` は `error.kind = policy_failure` の場合だけ required です。
`error.kind = policy_failure` で `error.reason_code` が missing / null / wrong type / invalid enum の場合は、
field = `error.reason_code`、
`expected_value = "CompareValidationResult.policy_failure.reason_code"`、
`actual_value` に `missing`、`null_not_allowed`、`wrong_type`、または `invalid_enum` を入れます。
`error.kind != policy_failure` で `error.reason_code` が存在する場合は forbidden で、
field = `error.reason_code`、`expected_value = "absent_for_error_kind"`、
`actual_value = "forbidden_field"` です。
同じ `error` object 内で `error.kind` failure と `error.reason_code` failure が同時に成立する場合は、
`error.kind` failure を先に返します。
この section で omit と定義した top-level metadata が存在する場合は forbidden です。
現在の `error.kind` では到達しない metadata には `expected_value = "absent_for_error_kind"`、
`policy_failure` 内の `error.reason_code` により存在しない metadata には
`expected_value = "absent_for_reason_code"`、同じ reason_code 内で reference hash を
valid に読めたかどうかで存在しない metadata には
`expected_value = "absent_for_reference_state"` を使います。
...
`CompareValidationResult` response schema / domain failure が複数成立する場合は、
次の順で最初の1件だけを返します。

```text
1. top-level object type
2. duplicate top-level known field, in top-level field order
3. top-level schema required / null / type / mismatch
4. top-level status required / null / type / invalid enum
5. status = valid error forbidden
6. status = failed error object required / null / type
7. duplicate error known field, in error field order
8. status = failed error.kind required / null / type / invalid enum
9. status = failed error.reason_code required / forbidden / null / type / invalid enum
10. status / error-kind dependent top-level metadata required / forbidden / type / hash / enum,
    in top-level metadata order
11. error payload member required / forbidden / type / hash / scalar validation,
    in error payload order
12. duplicate unknown error field, then unknown error field,
    by bytewise field name order within each group
13. duplicate unknown top-level field, then unknown top-level field,
    by bytewise field name order within each group
```

top-level field order は `schema`、`status`、`error`、`normalized_result_hash`、
`policy_hash`、`embedded_comparison_status`、`recomputed_comparison_status` です。
top-level metadata order は `normalized_result_hash`、`policy_hash`、
`embedded_comparison_status`、`recomputed_comparison_status` です。
error field order は `kind`、`reason_code`、`field`、`expected_value`、`actual_value`、
`expected_hash`、`actual_hash` です。
error payload order は `field`、`expected_hash`、`actual_hash`、
`expected_value`、`actual_value` です。
duplicate known field は `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。
item 2 と item 7 の duplicate known field は known field だけを扱います。
duplicate unknown error field は item 12、duplicate unknown top-level field は item 13 で扱います。
item 12 / 13 の内部では、まず duplicate unknown field を field name の bytewise order で報告し、
duplicate unknown がない場合だけ non-duplicate unknown field を bytewise order で報告します。
duplicate unknown field では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。
non-duplicate unknown field では `expected_value = "absent"`、
`actual_value = "unknown_field"` を使います。
`CompareValidationResult.error` payload member presence は次で固定します。
表の required payload members 以外の known error member は forbidden です。
forbidden payload member では `field = "error.<member>"`、
`expected_value = "absent_for_error_kind"`、`"absent_for_reason_code"`、
または `"absent_for_payload_shape"`、
`actual_value = "forbidden_field"` を使います。

```text
error.kind / reason_code / source                         required payload members
normalized_result_file_unreadable                         field, actual_value
normalized_result_json_invalid                            field, actual_value
normalized_result_schema_invalid                          field, expected_value, actual_value
normalized_artifact_hash_mismatch                         field, expected_hash, actual_hash
comparison_mismatch                                       field, expected_hash, actual_hash
normalized_result_hash_mismatch                           field, expected_hash, actual_hash
policy_failure / policy_reference_invalid / reference schema
                                                            field, expected_value, actual_value
policy_failure / policy_reference_invalid / policy JSON parse
                                                            field, actual_value
policy_failure / policy_reference_invalid / policy schema
                                                            field, expected_value, actual_value
policy_failure / policy_file_unreadable                    field, actual_value
policy_failure / policy_hash_mismatch                      field, expected_hash, actual_hash
```

forbidden payload member の `expected_value` は次で固定します。
`error.reason_code` が `error.kind != policy_failure` で存在する場合は、上の
`error.reason_code` forbidden rule に従い `absent_for_error_kind` を使います。
上の表で選択された non-policy `error.kind` row の payload required set に含まれない
`field` / `expected_value` / `actual_value` / `expected_hash` / `actual_hash` は
`absent_for_payload_shape` です。
`error.kind = policy_failure` で、現在の `error.reason_code` では到達しない
payload member は `absent_for_reason_code` です。
同じ `policy_reference_invalid` reason_code 内で、reference schema / policy JSON parse /
policy schema の source subcase により到達しない payload member は
`absent_for_payload_shape` です。
required payload member の missing / null / wrong type は、その payload member path に
`actual_value = "missing"`、`"null_not_allowed"`、または `"wrong_type"` を入れます。
`error.field` の `expected_value` は `field_path`、
...
release bundle command / validator が included `compare_validation_response` artifact の
schema / domain failure を報告する場合は、response-local `$` を
`compare_validation_response[<i>].artifact` に置換し、root 以外の path は
`compare_validation_response[<i>].artifact.<response-local path>` として報告します。
...
`actual_value = "invalid_json"` にします。
`normalized_result_schema_invalid` では `error.field` に
`NormalizedCheckResult` artifact-local の invalid field JSON path を入れ、
`normalized_result.` prefix は付けません。
たとえば `results[<i>]`、`results[<i>].failure_key`、
`comparison`、`comparison.disagreements[<i>]` をそのまま使います。
unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合も、
`NormalizedCheckResult` schema validator の fallback path をそのまま使い、
`normalized_result.results[<i>]` や `normalized_result.comparison` にはしません。
`expected_value` に schema requirement 名、
`actual_value` に `missing`、`wrong_type`、`unknown_field`、
`invalid_enum`、`invalid_hash_format`、`invalid_name_format`、
`null_not_allowed`、`order_violation`、`duplicate_field`、`duplicate_entry`、
`forbidden_field`、`failure_key_mismatch`、`comparison_field_mismatch`、
`empty_string`、`invalid_string_format`、`non_positive_integer`、
`integer_out_of_range` のいずれか、または top-level schema mismatch の実 schema string を入れます。
...
`policy_failure` の `error.reason_code` field shape は上の
`CompareValidationResult.policy_failure.reason_code` rule に従います。
`policy_reference_invalid` では、reference object 自体が missing / wrong type / explicit null の場合
`error.field = "policy"`、`expected_value = "RunnerPolicyReference"`、
`actual_value` に `missing`、`wrong_type`、または `null_not_allowed` を入れます。
reference object が存在し、その member が不正な場合は
...
unknown field では `policy.<unknown_field_name>` です。
`policy.<unknown_field_name>` は unknown field 名が `schema_path_component` に一致する場合だけ使い、
一致しない場合は `error.field = "policy"` にします。
`expected_value` に schema requirement 名、
`actual_value` に `missing`、`wrong_type`、`unknown_field`、`invalid_enum`、
`invalid_path`、`invalid_hash_format`、`null_not_allowed`、`order_violation`、
`duplicate_field` のいずれかを入れます。
...
`AuditSidecarValidationResult` の field population は mode、status、sidecar parse state、
および source kind の解決可否で固定します。
次の paragraph は `npa-check audit-sidecar validate` が response を生成するときの field population rule であり、
保存済み response の standalone schema / domain validation requiredness を直接定義するものではありません。
保存済み `AuditSidecarValidationResult` を単体 artifact として schema / domain validation する場合は、
response 自身の `mode`、`status`、`error.reason_code`、`source_kind`、および field presence / type / hash format
だけを検査します。
対応する `AiAuditSidecar` の source field 有無や値との一致は、standalone response schema / domain validation では
検査せず、release bundle optional sidecar matching または CI diagnostic required sidecar matching の
context binding として検査します。
したがって CI diagnostic gate の pre-candidate validation item 2 でも、
`source_kind = machine_result` の `source_normalized_result_hash` は response schema 上 optional metadata として扱い、
存在すれば valid hash format を要求し、omit されても schema / domain failure にはしません。
その response が特定 sidecar に対して `source_normalized_result_hash` を持つべきか、または omit すべきかは
required diagnostic の `required_ai_sidecar_validation_source_mismatch`、
または release bundle の response-to-sidecar matching failure として扱います。
response 生成時の field population rule は次で固定します。
...
生成時に `source_kind = machine_result` では、`source.result_hash` が valid hash として読めた場合だけ
`source_result_hash` が required です。
`source.result_hash` 自体の `sidecar_schema_invalid` では `source_result_hash` を omit します。
生成時に sidecar source に `normalized_result_hash` がある場合だけ
...
生成時に `source_kind = normalized_comparison` では、`source.normalized_result_hash` が valid hash として
読めた場合だけ `source_normalized_result_hash` が required で、`source_result_hash` は omit します。
保存済み response の standalone schema / domain validation では、上の生成時 rule への適合を
sidecar file から再推論してはいけません。
standalone validation は response-local な `mode` / `status` / `error.reason_code` /
`source_kind` / validation-state による required / forbidden と、存在する metadata の型・hash 構文だけを
下の priority order で検査します。
特定 sidecar source に照らした `source_normalized_result_hash` の required / forbidden / equality は、
standalone schema / domain failure ではなく context binding failure です。
standalone schema / domain validation の top-level metadata presence matrix は次で固定します。
`metadata state` は重なり得る predicate ではなく、次の order で最初に一致した1件を採用する
exclusive phase enum です。
`status` が `valid` / `failed` 以外の場合は先の status invalid enum failure で止まり、
この matrix には到達しません。
...
`optional` metadata は omit してよく、存在する場合は型・hash 構文・enum だけを検査します。

```text
metadata state:
  evaluate in this order and take the first matching state.

  unreadable_sidecar:
    status = failed and error.reason_code = sidecar_file_unreadable

  unparsed_sidecar_json:
    status = failed and error.reason_code = sidecar_json_invalid

  sidecar_schema_failed:
  ...
  before_input_policy:
    status = failed and (
      error.reason_code in validation_reference_missing / validation_reference_schema_invalid
      or error.reason_code = forbidden_sidecar_field
        and policy_gated_forbidden_field did not match
    )

  input_policy_reached:
    status = valid
    or status = failed and no earlier metadata state matched

common metadata:
  sidecar_file_hash:
    forbidden if metadata state = unreadable_sidecar;
    required otherwise.

  input_policy_hash:
    forbidden if mode = schema_only;
    forbidden if mode = cross_artifact and metadata state is one of:
      unreadable_sidecar / unparsed_sidecar_json / sidecar_schema_failed / before_input_policy;
    required if mode = cross_artifact and metadata state is one of:
      policy_gated_forbidden_field / input_policy_reached.

  source_kind:
    forbidden if mode = schema_only;
    forbidden if mode = cross_artifact and metadata state is unreadable_sidecar or unparsed_sidecar_json;
    optional if mode = cross_artifact and metadata state = sidecar_schema_failed;
    required if mode = cross_artifact and metadata state is neither unreadable_sidecar
      nor unparsed_sidecar_json nor sidecar_schema_failed.

source metadata:
  source_kind omitted or forbidden:
    source_result_hash = forbidden
    source_normalized_result_hash = forbidden

  source_kind = machine_result:
    source_result_hash = optional if metadata state = sidecar_schema_failed;
    source_result_hash = required otherwise.
    source_normalized_result_hash = optional.

  source_kind = normalized_comparison:
    source_result_hash = forbidden.
    source_normalized_result_hash = optional if metadata state = sidecar_schema_failed;
...
`error.reason_code = sidecar_schema_invalid`、`source_kind = machine_result` の saved response では、
`source_result_hash` は optional です。
一方で `mode = cross_artifact`、`status = valid`、`source_kind = machine_result` では、
`source_result_hash` は required です。
`AuditSidecarValidationResult` response schema は closed-world です。
response-local JSON path の root marker は `$` です。
top-level object 自体の wrong type / null では `field = "$"`、
`expected_value = "object"`、`actual_value = "wrong_type"` または `"null_not_allowed"` にします。
top-level object の unknown field または duplicate unknown field 名を
...
`field = "error"` を使います。
known field の duplicate key はその known field の response-local JSON path を使います。
top-level field は `schema`、`mode`、`status`、`error`、`sidecar_file_hash`、
`input_policy_hash`、`source_kind`、`source_result_hash`、
`source_normalized_result_hash` だけです。
`error` object 内の known field は `kind`、`reason_code`、`field`、
`expected_value`、`actual_value`、`expected_hash`、`actual_hash` だけです。
これ以外の field は unknown field です。
top-level `schema` missing / null / wrong type / mismatch では
`field = "schema"`、
`expected_value = "npa.phase8.audit_sidecar_validation_result.v1"`、
`actual_value` に `missing`、`null_not_allowed`、`wrong_type`、または `invalid_enum` を入れます。
top-level `mode` missing / null / wrong type / invalid enum では
`field = "mode"`、
`expected_value = "AuditSidecarValidationResult.mode"`、
`actual_value` に `missing`、`null_not_allowed`、`wrong_type`、または `invalid_enum` を入れます。
top-level `status` missing / null / wrong type / invalid enum では
`field = "status"`、
`expected_value = "AuditSidecarValidationResult.status"`、
`actual_value` に `missing`、`null_not_allowed`、`wrong_type`、または `invalid_enum` を入れます。
`status = valid` では `error` は forbidden で、
`field = "error"`、`expected_value = "absent_for_status"`、
`actual_value = "forbidden_field"` にします。
`status = failed` では `error`、`error.kind`、`error.reason_code` が required で、
`error.kind` は `audit_sidecar_validation_failure` に固定します。
`status = failed` で `error` が missing、explicit null、または object 以外の場合は、
field = `error`、`expected_value = "object"`、
`actual_value` に `missing`、`null_not_allowed`、または `wrong_type` を入れます。
`error.kind` の missing / null / wrong type / invalid enum は response schema / domain failure とし、
field = `error.kind`、`expected_value = "audit_sidecar_validation_failure"`、
`actual_value` に `missing`、`null_not_allowed`、`wrong_type`、または `invalid_enum` を入れます。
`error.reason_code` の missing / null / wrong type / invalid enum は response schema / domain failure とし、
field = `error.reason_code`、
`expected_value = "AuditSidecarValidationResult.error.reason_code"`、
`actual_value` に `missing`、`null_not_allowed`、`wrong_type`、または `invalid_enum` を入れます。
`error.reason_code` が valid enum でも `mode` と互換しない場合は response schema / domain failure とし、
`field = "error.reason_code"`、
`expected_value = "AuditSidecarValidationResult.error.reason_code_for_mode"`、
`actual_value = AuditSidecarValidationResult.error.reason_code` を入れます。
`status = failed` で許可される `error.reason_code` は `mode` ごとに次で固定します。
...
同じ `error` object 内で `error.kind` failure と `error.reason_code` failure が同時に成立する場合は、
`error.kind` failure を先に返します。
`mode = schema_only` で `source_kind`、`source_result_hash`、
`source_normalized_result_hash`、または `input_policy_hash` が存在する場合は forbidden で、
...
2. duplicate top-level known field, in top-level field order
3. top-level schema required / null / type / mismatch
4. top-level mode required / null / type / invalid enum
5. top-level status required / null / type / invalid enum
6. status = valid error forbidden
7. status = failed error object required / null / type
8. duplicate error known field, in error field order
9. status = failed error.kind required / null / type / invalid enum
10. status = failed error.reason_code required / null / type / invalid enum /
    mode-incompatible
11. mode / source-kind / validation-state dependent top-level metadata
    required / forbidden / type / hash / enum, in top-level metadata order
12. error payload member required / forbidden / type / hash / scalar validation,
    in error payload order
13. duplicate unknown error field, then unknown error field,
    by bytewise field name order within each group
14. duplicate unknown top-level field, then unknown top-level field,
    by bytewise field name order within each group
```

top-level field order は `schema`、`mode`、`status`、`error`、`sidecar_file_hash`、
`input_policy_hash`、`source_kind`、`source_result_hash`、
`source_normalized_result_hash` です。
top-level metadata order は `sidecar_file_hash`、`input_policy_hash`、`source_kind`、
`source_result_hash`、`source_normalized_result_hash` です。
error field order は `kind`、`reason_code`、`field`、`expected_value`、`actual_value`、
`expected_hash`、`actual_hash` です。
error payload order は `field`、`expected_hash`、`actual_hash`、
`expected_value`、`actual_value` です。
duplicate known field は `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。
item 2 と item 8 の duplicate known field は known field だけを扱います。
duplicate unknown error field は item 13、duplicate unknown top-level field は item 14 で扱います。
item 13 / 14 の内部では、まず duplicate unknown field を field name の bytewise order で報告し、
duplicate unknown がない場合だけ non-duplicate unknown field を bytewise order で報告します。
duplicate unknown field では `expected_value = "unique_object_keys"`、
`actual_value = "duplicate_field"` を使います。
non-duplicate unknown field では `expected_value = "absent"`、
`actual_value = "unknown_field"` を使います。
`AuditSidecarValidationResult.error` payload member presence は次で固定します。
表の required payload members 以外の known error member は forbidden です。
forbidden payload member では `field = "error.<member>"`、
`expected_value = "absent_for_reason_code"` または `"absent_for_payload_shape"`、
`actual_value = "forbidden_field"` を使います。
...
```text
error.reason_code / source                                  required payload members
sidecar_file_unreadable                                     field, actual_value
sidecar_json_invalid                                        field, actual_value
sidecar_schema_invalid                                      field, expected_value, actual_value
forbidden_sidecar_field                                     field, actual_value
validation_reference_missing                                field, actual_value
validation_reference_schema_invalid                         field, expected_value, actual_value
input_policy_file_unreadable                                field, actual_value
input_policy_json_invalid                                   field, expected_value, actual_value
input_policy_schema_invalid                                 field, expected_value, actual_value
input_policy_hash_mismatch                                  field, expected_hash, actual_hash
input_policy_field_mismatch                                 field, expected_value, actual_value
result_store_manifest_hash_mismatch                         field, expected_hash, actual_hash
result_store_manifest_invalid / file unreadable or JSON      field, actual_value
result_store_manifest_invalid / schema or domain             field, expected_value, actual_value
normalized_store_manifest_hash_mismatch                     field, expected_hash, actual_hash
normalized_store_manifest_invalid / file unreadable or JSON  field, actual_value
normalized_store_manifest_invalid / schema or domain         field, expected_value, actual_value
referenced_file_hash_mismatch                               field, expected_hash, actual_hash
referenced_artifact_hash_mismatch                           field, expected_hash, actual_hash
referenced_artifact_value_mismatch                          field, expected_value, actual_value
source_result_not_found                                     field, expected_hash
source_normalized_result_not_found                          field, expected_hash
source_hash_mismatch                                        field, expected_hash, actual_hash
source_id_mismatch                                          field, expected_value, actual_value
normalized_result_missing_source                            field, expected_hash
prompt_hash_mismatch                                        field, expected_hash, actual_hash
```

forbidden payload member の `expected_value` は次で固定します。
現在の `error.reason_code` では到達しない payload member は
`absent_for_reason_code` です。
同じ `result_store_manifest_invalid` または `normalized_store_manifest_invalid`
reason_code 内で、file unreadable / JSON source subcase と schema / domain
source subcase の違いにより到達しない payload member は `absent_for_payload_shape` です。
required payload member の missing / null / wrong type は、その payload member path に
`actual_value = "missing"`、`"null_not_allowed"`、または `"wrong_type"` を入れます。
`error.field` の `expected_value` は `field_path`、
`error.expected_hash` / `error.actual_hash` の `expected_value` は `sha256:<lower-hex>`、
...
`actual_value = "unknown_field"` を使います。
hash member の invalid hash format では `actual_value = "invalid_hash_format"` を使います。
release bundle command / validator が included `audit_sidecar_validation_response` artifact の
schema / domain failure を報告する場合は、response-local `$` を
`audit_sidecar_validation[<i>].artifact` に置換し、root 以外の path は
`audit_sidecar_validation[<i>].artifact.<response-local path>` として報告します。
...
3. sidecar の closed-world schema、source shape、常時 forbidden field、
   source-kind-dependent static forbidden field を検査する
...
10. sidecar source を store から解決し、source hash / id / normalized membership、
    classification、sidecar status を検査する
11. `ai_audit_prompt_input` projection を再計算し、ai.prompt_hash を検査する
```
...
sidecar schema / 常時 forbidden field / source-kind-dependent static forbidden field に失敗した場合は
step 3 の error を返します。
...
3 の source-kind-dependent static forbidden field は、参照先 artifact を読まず、
sidecar 内の `source.kind` だけで forbidden と判定できる field です。
MVP では `source.kind = normalized_comparison` の
`classification.checker_error_kind` がこれに該当します。
...
top-level path は valid だが値の shape が invalid な policy-gated field は、
7 へ進まず step 3 の `sidecar_schema_invalid` として返します。
...
reference object 内の generic unknown field または unknown field の duplicate key を報告する場合は、
unknown field 名が `schema_path_component` に一致するときだけ
`result_store.<unknown>`、`input_policy.<unknown>`、または `normalized_store.<unknown>` を使います。
一致しない場合は raw key を copy せず、`result_store`、`input_policy`、または `normalized_store`
の containing object path を使います。
...
`normalized_store` は sidecar が schema-valid な `source.normalized_result_hash` を持つ場合だけ
required reference object として扱います。
`source.normalized_result_hash` が wrong type、explicit null、または invalid hash format の場合は
step 3 の `sidecar_schema_invalid` で停止し、step 5 の reference requiredness は評価しません。
sidecar が schema-valid な `source.normalized_result_hash` を持たない場合に `normalized_store` が完全に omit されても
...
`expected_hash` は validation reference の `result_store.manifest_hash` または
`normalized_store.manifest_hash`、`actual_hash` は読み込んだ manifest file bytes の
sha256 です。
...
`null_not_allowed`、`duplicate_field`、`empty_string`、`empty_array`、
`invalid_string_format`、`non_positive_integer`、`integer_out_of_range` のいずれかを入れます。
unknown field または duplicate unknown field 名を `schema_path_component` で表現できない場合は、
7 の `AiAuditSidecar` sidecar-local containing object path を使い、raw key を copy してはいけません。
...
`actual_value = "invalid_json"` にします。
`input_policy_schema_invalid` では `error.field` に 7 の
`AiAuditInputPolicy` input-policy-prefixed field shape を使い、
`expected_value` / `actual_value` も 7 の `AiAuditInputPolicy` field shape が
定義する固定値をそのまま使います。
...
ただし `input_policy.included_fields` は array なので、array を直接
`expected_value` / `actual_value` に入れてはいけません。
policy file 側と sidecar 側の `included_fields` array をそれぞれ RFC 8785 canonical JSON bytes にし、
その UTF-8 string を `expected_value` / `actual_value` に入れます。
たとえば empty array は `[]`、`["module","status"]` はそのまま
`["module","status"]` という string として入れます。
...
`checker_profile` や `AiAuditSidecar.status` のような non-hash field mismatch では
`referenced_artifact_value_mismatch` を使い、`error.field` には
`result_store.results[].checker_profile`、sidecar-local path の `status`、または
...
sidecar-local `AiAuditSidecar.status` の mismatch として
`error.field = "status"`、`expected_value` に 7 の許可 status set 名
（`sidecar_status:summarized_only` または `sidecar_status:triage_or_summary`）、
...
`prompt_hash_mismatch` では `error.field = "ai.prompt_hash"`、
`expected_hash` に validator が再計算した `ai_audit_prompt_input` projection hash、
`actual_hash` に `AiAuditSidecar.ai.prompt_hash` を入れます。
`AiAuditSidecar.ai.prompt_hash` の missing / explicit null / wrong type / invalid hash format は
step 3 の `sidecar_schema_invalid` で拒否済みなので、`prompt_hash_mismatch` には進みません。
...
release audit bundle validator と CI diagnostic required-sidecar binding step だけが、
それぞれの target selection context を使って exact selected raw result rule を追加で検査します。
...
許可されない場合は `referenced_artifact_value_mismatch` を返します。
step 10 が成功した後、step 11 で validator は 7 の `ai_audit_prompt_input` projection を
再計算します。
このとき `AiAuditSidecar.input_policy` copied metadata ではなく、step 6 で hash と copied metadata が
一致すると確認済みの input policy file の値を source of truth として使います。
ただし `agent`、`model`、`source` は sidecar に保存された値を使います。
再計算した projection hash が `AiAuditSidecar.ai.prompt_hash` と一致しない場合は
`prompt_hash_mismatch` を返します。
schema-only mode では source artifact と input policy file を読まないため、
`ai.prompt_hash` の再計算と mismatch 検査を行ってはいけません。
...
`normalized_store` は sidecar が schema-valid な `source.normalized_result_hash` を持つ場合 required です。
sidecar が schema-valid な `source.normalized_result_hash` を持たない場合、`normalized_store` は optional です。
...
store manifest schema / domain validation priority は次で固定します。
複数の violation が同時に存在する場合は、最初に該当したものだけを返します。

```text
1. top-level object type
2. duplicate top-level known field, in the order schema, requests/results
3. top-level schema missing / null / type / mismatch
4. requests/results array missing / null / type
5. entry object type, by smaller entry index
6. duplicate entry known field, by smaller entry index and entry field order
7. entry required member / null / type / value shape, by smaller entry index and entry field order
8. unknown entry field, by smaller entry index and bytewise field name order
...
store manifest schema shape failure の field / payload は次で固定します。
`<root>`、`<entries>`、`<schema>` は manifest 種別ごとに次へ置換します。
...
duplicate top-level known field:
  field = <root>.schema or <root>.<entries>
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"
...
requests/results array missing / null / type:
  field = <root>.<entries>
  expected_value = "array"
  actual_value = "missing" | "null_not_allowed" | "wrong_type"

entry object type:
  field = <root>.<entries>[<i>]
  expected_value = "object"
  actual_value = "wrong_type" | "null_not_allowed"

duplicate entry known field:
  field = <root>.<entries>[<i>].<member>
  expected_value = "unique_object_keys"
  actual_value = "duplicate_field"
...
entry member value shape は次で固定します。
...
entry field order は manifest 種別ごとに次で固定します。

```text
request store:
  request_hash, path, file_hash
machine result store:
  result_hash, request_hash, run_artifact_hash, checker_profile, path, file_hash
normalized result store:
  normalized_result_hash, artifact_hash, path, file_hash
challenge replay store:
  challenge_id, manifest_hash, result_hash, artifact_hash, path, file_hash
```
...
duplicate ではない unknown field では
`expected_value = "absent"`、`actual_value = "unknown_field"` にします。
entries order violation の `<i>` は、最初に sort key が直前 entry より小さくなる後続 entry index です。
隣接 sort key が等しいだけの場合は `order_violation` ではなく unique key duplicate として扱います。
entries order violation の field / payload は manifest 種別ごとに次で固定します。

```text
request store:
  field = request_store.requests[<i>]
  expected_value = "request_hash_bytewise_ascending"
  actual_value = "order_violation"

machine result store:
  field = result_store.results[<i>]
  expected_value = "run_artifact_hash_bytewise_ascending"
  actual_value = "order_violation"

normalized result store:
  field = normalized_store.results[<i>]
  expected_value = "normalized_result_hash_bytewise_ascending"
  actual_value = "order_violation"

challenge replay store:
  field = replay_store.results[<i>]
  expected_value = "result_hash_bytewise_ascending"
  actual_value = "order_violation"
```

unique key duplicate の `<i>` は、同じ key がすでに出現している最小の後続 entry index です。
同じ `<i>` で複数の unique key duplicate が成立する場合は、manifest 種別ごとの unique key order で
最初の key を返します。
unique key order は request store では `request_hash`、`path`、
machine result store では `run_artifact_hash`、`path`、
normalized result store では `normalized_result_hash`、`path`、
challenge replay store では `result_hash`、`path`、`(challenge_id, manifest_hash)` です。
store manifest の local JSON path で unknown field または duplicate unknown field 名を
`schema_path_component` で表現できない場合は、raw key を copy せず manifest-local containing object path を使います。
request store manifest の containing object path は `$` または `requests[<i>]`、
machine result store manifest の containing object path は `$` または `results[<i>]`、
normalized result store manifest の containing object path は `$` または `results[<i>]`、
challenge replay store manifest の containing object path は `$` または `results[<i>]` です。
caller error body では root marker `$` を store root prefix に置き換えます。
たとえば request store manifest local `$` は `request_store`、local `requests[<i>]` は
`request_store.requests[<i>]` です。machine result store は `result_store` /
`result_store.results[<i>]`、normalized result store は `normalized_store` /
`normalized_store.results[<i>]`、challenge replay store は `replay_store` /
`replay_store.results[<i>]` を使います。
release staging など別の caller prefix を使う場合も同じ規則で、その caller が定義する
store root prefix に local suffix を付けます。
...
- missing source MachineCheckResult artifact makes sidecar invalid
...
- malformed raw output normalized entries keep checker_binary_id and checker_binary_hash for launched results but omit checker_id and checker_build_hash
- normalized comparison reports checker_binary_id_mismatch as policy_failure when launched checker_binary_id differs from policy allowlist
...
- MachineCheckRequest.request_id uses the same non-empty visible ASCII id grammar as MachineCheckResult request_id/result_id
...
- schema validators never copy raw unknown keys that fail schema_path_component into error.field and use the fixed containing object path instead
- MachineCheckResult schema validators map unrepresentable unknown keys to caller-prefixed containing object paths such as machine_results[].runner and result_store.results[].checker
- MachineCheckResult axioms_used allows known-empty [] but rejects invalid name order and duplicate axiom names with fixed machine_result_schema_invalid fields
- MachineCheckResult declarations_checked is optional non-negative i64 metadata and rejects negative or overflow values as integer_out_of_range
- schema field path indexes reject leading zeros and values outside non-negative i64
...
- npa-check run rejects malformed --attempt as a CLI argument error without MachineCheckResult or MachineCheckRequestErrorResult
- /machine/check/certificate rejects malformed attempt as ApiError api_request_schema_invalid with field attempt
...
- malformed RunnerPolicyReference reports member-level failures in fixed kind/path/hash priority, with policy.path before policy.hash when both are invalid
- RunnerPolicyReference duplicate unknown members report duplicate_field at unknown-member priority
...
- CheckerRawResult raw schema field paths use the fixed raw_schema_field_path grammar and unrepresentable unknown keys report the containing object path
...
- AiAuditInputPolicy included_fields uses source-kind local namespaces and does not treat unqualified status as comparison.status
- AiAuditInputPolicy file validation is source-kind-independent and accepts the union of machine_result and normalized_comparison allowed field paths
- AiAuditInputPolicy allows checker_binary_id / checker_binary_hash for machine_result and comparison.status_reasons for normalized_comparison
- sidecar copied input_policy.included_fields uses the same source-kind union allowlist and does not reject source-kind-inapplicable field paths
- machine_result sidecar using input_file_hash or expected_certificate_hash in sidecar copied input_policy.included_fields requires source.normalized_result_hash even in schema-only mode
- machine_result ai_audit_prompt_input omits checker_binary_id / checker_binary_hash for pre-launch failures where MachineCheckResult.checker lacks those fields
- ai_audit_prompt_input omits absent optional source fields instead of encoding null placeholders
- ai_audit_prompt_input keeps `fields` as an empty object when no included field path resolves and never omits the fields member
- normalized_comparison diagnostics use an explicit policy containing comparison.* field paths instead of relying on the machine_result default prompt payload
- ReleasePolicy ai_triage uses a single AiAuditInputPolicy hash for both machine_result and normalized_comparison sidecars, requiring a union policy when both are produced
- ReleasePolicy union policy shared field paths apply to the current source kind, so input_file_hash / expected_certificate_hash require source.normalized_result_hash on machine_result sidecars and cannot validate reproducibility repeated raw result sidecars
- ai_audit_prompt_input redaction is metadata in v1 and does not mask, truncate, or rewrite included structured field values
- ai.prompt_hash is computed from the RFC 8785 canonical ai_audit_prompt_input projection
- ai_audit_prompt_input includes source_run_artifact_hash for machine_result sidecars
- ai_audit_prompt_input.fields uses literal included_fields strings such as "error.kind" as direct keys, not nested objects
- AiAuditSidecar.ai.agent is a stable renderer/template version id and changes when prompt rendering rules change
- AiAuditSidecar.ai.model is a stable provider/model snapshot identity, not a mutable alias
- AiAuditSidecar.ai.agent and ai.model reject empty, whitespace/control-containing, and non-ASCII strings with fixed sidecar_schema_invalid shapes
- schema-only audit-sidecar validation treats ai.model stability as a producer obligation and does not reject mutable-looking aliases by contacting providers
- cross-artifact audit-sidecar validation recomputes ai.prompt_hash and reports prompt_hash_mismatch with ai.prompt_hash field
- schema-only audit-sidecar validation does not recompute or verify ai.prompt_hash
- schema-only audit-sidecar validation still applies sidecar-local copied included_fields domain checks and required_for_input_field
- schema-only audit-sidecar validation does not enforce policy-gated top-level source/tactic field permission flags
- sidecar copied input_policy id/version/hash/redaction malformed values use fixed sidecar_schema_invalid field shapes
- sidecar copied input_policy.included_fields missing / null / wrong type / element wrong type uses fixed sidecar_schema_invalid field shapes
- sidecar copied input_policy.included_fields schema/domain priority is independent of source.normalized_result_hash presence
- AiAuditSidecar does not define an input_artifacts field; source/input_policy references and ai_audit_prompt_input cover that role
- comparison array fields in ai_audit_prompt_input are copied as whole arrays without sorting, truncation, or payload filtering
- default prompt payload does not include policy mode, dependency hash path, or relevant previous failures in MVP
- allow_source_text / allow_tactic_trace do not expand ai_audit_prompt_input.v1 or permit AI input builder to send source/tactic text to the model
...
- MachineCheckResult policy_failure errors enforce reason_code-specific payload family and allowed field rules
- MachineCheckResult checker identity mismatch uses hash payload only for hash fields and value payload for checker.id / checker.binary_id
- MachineCheckResult pre-launch checker identity mismatch reports checker.id before checker.binary_id when both mismatch
- MachineCheckResult reason_code field compatibility uses a closed final error.field set and never double-prefixes delegated source artifact paths
- MachineCheckResult RunnerPolicy delegated field paths keep 4.1 typed key-slot grammars such as budgets.<profile> while raw unknown keys use delegated_schema_path_component fallback rules
- MachineCheckResult / RunnerPolicy invalid budgets keys use budget_key_path_component only for safe reporting and otherwise fall back to budgets
- MachineCheckResult delegated schema field paths reject unrepresentable raw keys and fall back to fixed containing object paths
- MachineCheckResult optional payload reason_codes treat partial payload member presence as payload-bearing and report missing paired payload members deterministically
- MachineCheckResult forbidden member failures choose absent_for_error_kind vs absent_for_error_reason_code by fixed priority
- MachineCheckResult error object uses fixed kind-specific validation priority for ordinary, decode/noncanonical, and reason-coded errors before unknown error fields
- MachineCheckResult error schema/domain failures use fixed expected_value / actual_value shapes for kind, reason_code, field compatibility, payload members, forbidden members, and unknown members
- MachineCheckResult infrastructure reason_code is closed enum
- MachineCheckResult infrastructure errors enforce reason_code-specific error payload member rules
- checked-result sidecar omits classification.checker_error_kind
- NormalizedCheckResult failed entry includes failure_key
- NormalizedCheckResult failed entry validates nested error before failure_key presence / shape / derived equality
- NormalizedCheckResult failed entry rejects failure_key that is not derived from sibling error
- NormalizedCheckResult failed entry reports failure_key_mismatch with concrete results[<i>].failure_key field
...
- NormalizedCheckResult results[] required and conditional required field shape failures use fixed expected_value / actual_value fields
...
- NormalizedCheckResult process_launched=false identity fields are schema invalid, not comparison malformed_process_state
- NormalizedCheckResult results are ordered by RunnerPolicy profile order
- NormalizedCheckResult comparison static status-dependent shape failures are normalized_result_schema_invalid, not comparison_mismatch
- NormalizedCheckResult comparison static validation includes nested disagreements[] and status_reasons[] entry schema/domain validation
- NormalizedCheckResult comparison static validation reports malformed nested disagreements[] / status_reasons[] entries before status-dependent fixed shape mismatch
...
- NormalizedCheckResult comparison disagreements member value shape failures use fixed expected_value / actual_value fields
...
- NormalizedCheckResult comparison status_reasons entry schema validation uses fixed priority and field shapes
- NormalizedCheckResult comparison status_reasons checks reason_code / error_kind / kind compatibility before conditional payload validation
- NormalizedCheckResult comparison status_reasons checks reason_code / field compatibility for payload-bearing reasons
- NormalizedCheckResult comparison copied policy_failure status_reasons use the MachineCheckResult.error.field final closed set without recomputing runner-precheck prefixes
- NormalizedCheckResult comparison copied optional-payload status_reasons treat partial payload member presence as payload-bearing
- NormalizedCheckResult comparison copied malformed raw-output status_reasons accept only checker_raw fields matching raw_schema_field_path grammar
...
- comparison-generated checker_binary_id_mismatch uses field results[].checker_binary_id with value payload
- comparison status_reasons uses full status_reason_sort_key including reason_code, result_hash, error_kind, and payload values
...
- comparison inconclusive source results are limited to the fixed process_launched / error.kind / reason_code closed set
- comparison emits every reason in the selected priority class and emits no lower-priority reasons
- comparison disagreement entries are emitted for every deterministic mismatch
- comparison status_reasons sort treats omitted sort components as empty strings
...
- challenge replay and materialization map unrepresentable ChallengeManifest unknown keys to challenge_manifest or challenge_manifest.<containing_object>
...
- challenge generate validates ChallengeOutputStoreManifest schema/domain failures with fixed top-level, entry index, order-violation, and unique-key duplicate priority
...
- store entry schema invalid actual_value preserves delegated artifact validator values including non_positive_integer and forbidden_field
- store entry artifact validation order is unreadable, JSON, schema, file_hash, self-hash, manifest-field mismatch
- store entry hash mismatch payload uses recomputed-vs-parsed hashes for self-hash failures and manifest-vs-parsed hashes for manifest-field failures
...
- malformed RequestStoreReference reports member-level failures in fixed kind/path/manifest_hash priority, with request_store.path before request_store.manifest_hash when both are invalid
- RequestStoreReference duplicate unknown members report duplicate_field at unknown-member priority
...
- normalize evaluates multiple MachineCheckResult intrinsic validation inputs in CLI argument order and stops at the first failing input before artifact_selector, uniqueness, or policy validation
...
- normalize machine_result_schema_invalid allows invalid_path / non_positive_integer / integer_out_of_range / forbidden_field / empty_string / invalid_string_format actual_value where MachineCheckResult schema requires them
- normalize MachineCheckResult.result_id schema failures use the wildcard caller-prefixed field machine_results[].result_id
- MachineCheckResult schema/domain first failure uses fixed top-level, nested object, status-dependent, metadata, and unknown-field priority
- NormalizedCheckResult top-level schema validation reports fixed field / expected_value / actual_value for schema, normalized_result_id, top-level hashes, artifact, policy, results, comparison, and unknown fields
- NormalizedCheckResult duplicate unknown top-level fields and artifact/policy duplicate unknown members are reported at unknown-field priority with fixed bytewise tie-breaks
...
- normalize artifact_selector schema/domain first failure uses fixed object, duplicate, module, request_hash, and unknown-field priority
...
- normalize duplicate checker_profile tie-break uses the first repeated input in CLI argument order or API machine_results array order, not policy profile order
- normalize duplicate checker_profile inputs return duplicate_checker_profile_result before omitted-selector single-artifact ambiguity checks
...
- store manifest schema shape failures use manifest-kind-specific root fields, schema strings, entry array fields, and member expected_value / actual_value pairs
- store manifest schema/domain first failure uses fixed top-level, entry index, order-violation, and unique-key duplicate priority
- store manifest order_violation uses manifest-kind-specific indexed fields and expected_value strings
- store manifest unique-key duplicate tie-break uses the smallest repeated entry index and manifest-kind-specific unique key order
- store manifest unique-key duplicate reasons use fixed expected_value / actual_value pairs
...
- API wrapper simultaneous schema failures use endpoint wrapper field order and reference-local schema order
- API wrapper schema failures are reported before workspace path validation failures, except audit-sidecar active validation reference path rules
- API wrapper unknown top-level duplicate fields are reported at unknown-field priority after required known fields
- API wrapper invalid hash format is reported before same-reference workspace path validation failure
- API normalize rejects missing, non-array, null, or empty machine_results wrapper as ApiError
- API normalize evaluates machine_results intrinsic validation in array index order and stops at the first failing element before artifact_selector, uniqueness, or policy validation
- API normalize treats non-object machine_results[] elements as NormalizeErrorResult machine_result_wrong_schema, not ApiError
...
- API wrapper duplicate unknown fields report duplicate_field at unknown-field priority, not unknown_field
...
- omitted normalize selector is rejected with selector_ambiguous / zero_results when the first required profile has no result
...
- NormalizeErrorResult / CompareValidationResult / challenge CommandError policy_reference_invalid use the shared RunnerPolicyReference member priority
...
- challenge replay store manifest schema validators map unrepresentable unknown keys to replay_store or replay_store.results[<i>]
- coverage-summary input_store_manifest_invalid replaces ChallengeOutputStoreManifest challenge_output_store paths with challenge_store paths
- audit-sidecar step 3 sidecar schema/source shape/static forbidden failures use the fixed intra-step priority
- audit-sidecar step 3 reports optional source member malformed before copied input_policy.included_fields and forbidden source member presence
- audit-sidecar step 3 reports machine_result source.normalized_result_id without source.normalized_result_hash before copied input_policy.included_fields and required_for_input_field
- AiAuditSidecar source field shapes report fixed expected_value / actual_value for required hashes, optional malformed hashes, and optional source ids
- MachineCheckRequest.request_id, MachineCheckResult.request_id/result_id, NormalizedCheckResult.normalized_result_id, NormalizedCheckResult.results[].result_id, and AiAuditSidecar source ids use the same non-empty visible ASCII id grammar
- malformed optional source.normalized_result_hash is sidecar_schema_invalid before normalized_store reference requiredness is evaluated
- audit-sidecar sidecar_schema_invalid maps unrepresentable unknown keys to sidecar-local containing object paths such as $, source, input_policy, ai, or classification
- audit-sidecar step 5 validation reference failures use the fixed missing/partial/schema/path priority
- sidecar result/normalized store manifest hash mismatch uses expected_hash from validation reference and actual_hash from manifest file bytes
...
- audit-sidecar sidecar status permission mismatch uses closed expected_value names sidecar_status:summarized_only and sidecar_status:triage_or_summary
- audit-sidecar sidecar status permission mismatch reports sidecar-local AiAuditSidecar.status as error.field = status
...
- AiAuditSidecar summary and suggested_next_actions enforce non-empty text / required non-empty array shapes with fixed sidecar_schema_invalid values
- AiAuditSidecar suggested_next_actions required/null/wrong-type/empty status-dependent failures are reported before summary item-15 field schema failures
- AiAuditSidecar step 3 reports source-kind-dependent static forbidden fields before cross-artifact validation
...
- policy-gated source/tactic sidecar fields are attachment/output fields and are never included in ai.prompt_hash for ai_audit_prompt_input.v1
- policy-gated source/tactic sidecar top-level explicit null and wrong type return sidecar_schema_invalid with string_or_string_array expected_value
- AiAuditInputPolicy.included_fields rejects all policy-gated source/tactic field names explicitly and does not rely on ambiguous list references
...
- CI diagnostic required AI sidecar gate uses explicit ordered ci_diagnostic_targets[] target context plus ai_sidecars[], audit_sidecar_validation_results[], and ai_sidecar_diagnostic_results[] diagnostic artifact lists with path, file_hash, and artifact; multiple distinct target contexts are allowed, duplicate artifact_hash / normalized_result_hash is an evaluation failure, and MVP has no normative diagnostic store manifest
- CI diagnostic required AI sidecar gate is a typed library-input gate, not a raw JSON wrapper schema; list container and entry object shape errors are rejected before gate invocation, and post-validation artifact values must be derived from path/file_hash bytes validation rather than trusted as independent caller input
- CI diagnostic required AI sidecar gate returns an ordered AiSidecarDiagnosticResult sequence recomputed from the current target context, sidecars, and validation responses for target satisfaction, but returns a single AiSidecarDiagnosticEvaluationFailure for target context duplicate, explicit list input validation, or matching saved diagnostic result validation failures before candidate selection
- CI diagnostic required AI sidecar gate rejects malformed explicit list path/file_hash metadata before file read with fixed AiSidecarDiagnosticEvaluationFailure reason codes, fields, expected_value, actual_value, and path-before-file_hash priority
- CI diagnostic required AI sidecar gate resolves saved results by policy_hash / input_policy_hash / normalized_result_hash, validates target context and all saved diagnostic result entries before candidate selection, and requires matching saved diagnostic result target_count to match the derived target list length before selected raw resolution and status = passed only for NormalizedCheckResult values whose derived diagnostic target set is non-empty
- CI diagnostic required AI sidecar gate treats saved AiSidecarDiagnosticResult policy_hash / input_policy_hash / normalized_result_hash values that are syntactically valid but do not match the active context as nonmatching keys, not response schema/domain failures
- CI diagnostic required AI sidecar gate ignores AiSidecarDiagnosticResult entries whose policy_hash / input_policy_hash / normalized_result_hash do not match an expected non-empty CI diagnostic target, and also ignores entries for NormalizedCheckResult values whose derived diagnostic target set is empty
- CI diagnostic required AI sidecar recomputation sets status = passed with omitted error only when the derived target list is empty or every target satisfaction check succeeds, otherwise status = failed with the first target satisfaction failure by fixed target and local priority
- CI diagnostic required AI sidecar pass evaluation requires the matching saved AiSidecarDiagnosticResult to be RFC 8785 canonical-serialization identical to the recomputed sequence entry; saved result file_hash is not used as the comparison identity and saved-vs-recomputed mismatch is a pass failure, not AiSidecarDiagnosticEvaluationFailure
- CI diagnostic required AI sidecar pass evaluation reports missing, duplicate, not-passed, and saved-vs-recomputed mismatch with fixed required_ai_sidecar_diagnostic_result_* reason codes, fields, payloads, and target/list-index priority
- CI diagnostic required AI sidecar target satisfaction requires selected raw MachineCheckResult resolution for machine_result targets, exactly one candidate AiAuditSidecar, and exactly one AuditSidecarValidationResult matched first by sidecar_file_hash; validation response cardinality is evaluated before mode/status/source checks, a single sidecar_file_hash-matching schema_only response is validation_mode_mismatch even when status = failed, a single sidecar_file_hash-matching cross_artifact failed response is validation_failed, failed responses without sidecar_file_hash are non-candidates, and a single cross_artifact valid response is then checked against ReleasePolicy.ai_triage.input_policy_hash and the source-kind-specific matching key with fixed field order
- CI diagnostic required machine_result sidecar target satisfaction reports selected raw resolution missing / duplicate before sidecar matching, does not use source.run_artifact_hash for candidate sidecar cardinality, and then requires exact selected raw run_artifact_hash binding in addition to AuditSidecarValidationResult.status = valid; a unique wrong retry source is reported as AiSidecarDiagnosticResult.error.reason_code = required_ai_sidecar_source_mismatch on ai_sidecar[<i>].artifact.source.run_artifact_hash
...
- CI diagnostic required machine_result sidecar targets are copied-status-failed NormalizedCheckResult.results[] entries counted before selected raw resolution; their required sidecar sources derive exactly one selected raw MachineCheckResult with checker_profile / request_hash / result_hash / policy_hash, report required_ai_sidecar_selected_raw_missing / duplicate otherwise, select candidate sidecars by source.kind plus source.request_hash / source.result_hash / optional source.normalized_result_hash, and validate source.run_artifact_hash only after a unique candidate is found
- machine_result sidecar source.normalized_result_hash, when present, must match the CI diagnostic target normalized_result_hash for required diagnostics or the release target normalized_result_hash for release-bundle optional sidecars, and is forbidden for reproducibility repeated raw results
...
- release stage-bundle-inputs maps ReleasePolicy release_policy virtual root to inputs[<i>].artifact fields
...
- release stage-bundle-inputs maps AiAuditSidecar sidecar-local $ and containing object paths to inputs[<i>].artifact fields without emitting inputs[<i>].artifact.$
...
- release stage-bundle-inputs maps ChallengeOutputStoreManifest challenge_output_store virtual root to inputs[<i>].artifact fields
...
- nightly AI sidecar diagnostic artifacts are required only for copied-status-failed machine_result targets and normalized_comparison targets with comparison.status disagreement / missing_checker_result / policy_failure / inconclusive when ReleasePolicy.ai_triage.enabled and ai_triage.required are both true
- CI diagnostic required AI sidecar targets are derived before selected raw resolution from copied-status-failed NormalizedCheckResult.results[] entries and normalized_comparison statuses disagreement / missing_checker_result / policy_failure / inconclusive, and remain outside ReleaseAuditBundleManifest
...
- release bundle validates reproducibility selector target request_hash / checker_profile / result_hash / policy_hash equality only after referenced MachineCheckResult entries exist and have passed self-consistency, and reports target mismatches as auxiliary_result class 5 failures on the corresponding selector run_artifact_hash field
...
- release bundle compares ChallengeCoverageSummary.entries[*].replay_result_hash to the result_hash value component of challenge_replay_result identity, while the ("result_hash", result_hash) tuple is only the missing-key ordering representation
- release bundle generation uses bundle-local RequestStoreManifest.requests[].request_hash and MachineResultStoreManifest.results[].run_artifact_hash as machine_check_request / machine_check_result existing closed-set identity keys, while validate-bundle uses ReleaseAuditBundleManifest.artifacts[].hashes.request_hash / run_artifact_hash
- release validate-bundle never falls back to parsed or recomputed request_hash / run_artifact_hash for machine_check_request / machine_check_result closed-set exactness when materialized artifacts[].hashes.* does not match the allowed identity
- release bundle reports machine_check_request / machine_check_result class 4 missing, extra / forbidden, and duplicate failures with the fixed canonical identity-key payload shapes
- release bundle treats the expected artifact identity key table as missing-order key representation only, and does not use it as the source for existing machine_check_request / machine_check_result candidate discovery
- release bundle machine_check_request / machine_check_result duplicate class 4 failures report the smallest later entry with the same identity key, and release_target_result tuple keys are used only for zero-candidate missing machine_check_result failures
- challenge_replay_result class 5 source-key failures within the same included replay for normalized_check_result use fixed priority: result_hash self-consistency, normalized_result_hash missing, release target normalized_result_hash collision, while missing or non-unique summary bindings are reported as challenge_coverage_summary schema/domain failures or challenge_replay_result class 4 exact-set failures
...
- release bundle reports ChallengeReplayResult.checker_results[*] result_hash / checker_profile copy mismatches against the resolved MachineCheckResult as machine_check_result-prerequisite-only challenge_replay_result class 5 source-key failures with fixed fields
...
- release bundle reports reproducibility baseline_run_artifact_hash and repeated_run_artifact_hash target tuple mismatches after referenced MachineCheckResult entries exist and pass self-consistency as auxiliary_result class 5 failures with the corresponding selector run_artifact_hash field shape
- release bundle reports MachineCheckResult self-consistency failures before reproducibility selector target equality mismatches that would depend on the same target fields
- release bundle reports challenge_replay_result checker_results reference-copy mismatches before reproducibility selector target equality mismatches when both are class 5 failures inside the machine_check_result prerequisite tree
- release bundle treats non-baseline release target raw result selection ambiguity as machine_check_result prerequisite class 5 failure with the duplicate candidate machine_check_result[<m>].artifact field shape, not as a synthesized missing or forbidden entry
- release bundle evaluates non-baseline release target raw result selection ambiguity before machine_check_result closed-set exactness and never derives missing/forbidden machine_check_result entries from an ambiguous allowed run set
- release bundle release target raw result selection counts Step 6-valid tuple-matching MachineCheckResult entries before self-consistency, and does not filter candidates by result_hash / run_artifact_hash self-consistency
- release validate-bundle uses materialized hashes.run_artifact_hash as the concrete allowed run identity for a tuple-selected release target raw MachineCheckResult, reports hash mismatches on that selected entry as self-consistency failures, and does not rescue non-selected extra entries by parsed or recomputed run_artifact_hash
- release bundle optional machine_result ai_sidecar source resolution derives the release-target selected raw result set by reproducibility.selector.baseline_run_artifact_hash for the baseline release target result and by checker_profile / request_hash / result_hash / policy_hash tuple for non-baseline release target results, unions the reproducibility repeated raw result set from selector.repeated_run_artifact_hash, then resolves sidecar membership by source.run_artifact_hash because AiAuditSidecar.source has no policy_hash field
- release bundle non-baseline raw result selection extracts the tuple policy_hash component from MachineCheckResult.policy.hash, never from a nonexistent top-level MachineCheckResult.policy_hash field
- release bundle permits a baseline-profile same-tuple retry only when its run_artifact_hash is introduced by reproducibility repeated_run_artifact_hash or by another independent allowed run source such as challenge replay, and forbids same-tuple baseline retries with no allowed run source
...
- release bundle MachineCheckResult self-consistency failures use machine_check_result[<i>].artifact.result_hash / run_artifact_hash fields with artifact-entry-first payloads in validate-bundle and parsed-vs-recomputed payloads during generation
- release bundle MachineCheckRequest self-consistency failures use machine_check_request[<i>].artifact.request_hash with artifact-entry-first payloads in validate-bundle and parsed-vs-recomputed payloads during generation
- release bundle generation uses generated ReleaseAuditBundleManifest.artifacts deterministic kind/path order, not source store entry index, for machine_check_request[<i>], machine_check_result[<i>], and normalized_check_result[<i>] field shapes
- release bundle prerequisite gates apply to dependent class 5 source-copy, recomputation, selector/source-value, and status binding validations, not only class 4 closed-set failures
...
- release bundle validates NormalizedCheckResult source-copy after machine_check_result set is prerequisite-clean and before embedded comparison recomputation
- release bundle treats normalized_check_result class 4 closed-set failures as not gated by the machine_check_result source-copy prerequisite, while same-class independent machine_check_result failures still follow artifact kind order
- release bundle source-copy validation pairs release target entries by selected raw result relation and challenge replay entries by checker_profile, with fixed class 5 failures for missing or duplicate pairings
- release bundle returns a non-clean machine_check_result prerequisite before normalized_check_result source-copy, full-set comparison recomputation, or replay/summary status binding failures
- release bundle preserves the source artifact kind/class/field/payload of failures found inside the machine_check_result prerequisite tree instead of rewriting them to machine_check_result
- release bundle source-copy pairing payloads use actual checker_profile string values where specified, not literal prose placeholders
...
- ChallengeCoverageSummary schema/domain first failure uses fixed top-level, entries index, order-violation, unique-key duplicate, and count-relationship priority
...
- release validate-bundle reports duplicate non-envelope top-level keys, artifacts[] entry keys, and artifacts[].hashes keys as audit_bundle_invalid with fixed field shape and deterministic tie-break
- release validate-bundle full ReleaseAuditBundleManifest schema reports bundle_id, top-level unknown fields, artifacts[] entry shape, hashes object shape, order violations, and duplicate artifact keys with fixed priority and field shapes
...
- release bundle optional response / sidecar status gating uses fixed fields for failed compare response status, missing/duplicate sidecar validation response, and failed audit-sidecar validation response status
- release bundle class 3 evaluates included response status failures before artifact kind order, then orders response status failures by response artifact kind and occurrence
- release bundle reports failed audit_sidecar_validation_response status before sidecar response matching, and failed responses are never matching candidates even when matching key fields are omitted
- release bundle matches audit_sidecar_validation_response using source-kind-specific keys and treats source-kind-inapplicable omitted fields as normal, not missing matching keys
- release bundle rejects status-valid audit_sidecar_validation_response entries that match zero or multiple ai_audit_sidecar entries as class 4 extra / forbidden response failures
- release bundle rejects schema_only audit_sidecar_validation_response as Step 6 input_schema_invalid / audit_bundle_invalid before sidecar response matching or status gating
- release bundle treats ai_audit_sidecar and audit_sidecar_validation_response as forbidden artifacts before response matching when ReleasePolicy.ai_triage.enabled is false
- release bundle reports compare_validation_response class 4 failures with fixed fields for missing, duplicate, duplicate-response, and target-forbidden normalized results
- release bundle derives compare_validation_response response-binding class 4 failures only after normalized_check_result set is prerequisite-clean
- release bundle reports compare_validation_response policy hash mismatch before compare_validation_response recomputation mismatch for the same entry
- release bundle reports ai_audit_sidecar source-value mismatch before audit_sidecar_validation_response recomputation mismatch for the matched response
- release bundle machine_result ai_audit_sidecar source-value mismatch uses the same expected_hash sidecar / actual_hash resolved artifact direction as AuditSidecarValidationResult source_hash_mismatch
- release bundle reports reproducibility repeated raw result normalized_result_hash presence before release-target normalized_result_hash equality / membership checks
...
- release bundle machine_check_result missing-entry keys distinguish run_artifact_hash references from zero-candidate release_target_result tuple keys
...
- release bundle command / validator maps AiAuditSidecar sidecar-local $ and containing object paths to ai_sidecar[<i>].artifact fields without emitting ai_sidecar[<i>].artifact.$
- release bundle command / validator maps CompareValidationResult and AuditSidecarValidationResult response-local $ to compare_validation_response[<i>].artifact and audit_sidecar_validation[<i>].artifact without emitting .artifact.$
...
- release bundle command / validator strips store manifest virtual roots including challenge_output_store before constructing artifact fields such as challenge_store.artifact.entries[<i>].manifest_path
...
- ReleaseAuditBundleManifest derives compare_validation_response policy-hash and recomputation mismatches only after normalized_check_result set is prerequisite-clean
- ReleaseAuditBundleManifest reports ai_audit_sidecar source closed-set failure before audit_sidecar_validation_response recomputation mismatch
...
- CompareValidationResult, AuditSidecarValidationResult, AiSidecarDiagnosticResult, and AiSidecarDiagnosticEvaluationFailure are transient responses without result_hash
- AiSidecarDiagnosticResult response schema is closed-world, uses fixed schema / hash / status / target_count / error priority, forbids error when status = passed, restricts failed payload fields by reason_code, and leaves active-context policy_hash / input_policy_hash / normalized_result_hash equality to saved-result matching rather than schema/domain validation
- AiSidecarDiagnosticEvaluationFailure is closed-world with fixed self-schema and payload-member actual_value shapes, is the transient envelope for target context duplicate, file/hash/JSON/schema failures in explicit diagnostic artifact lists, and saved diagnostic target_count mismatches, preserves delegated schema/domain payload family including actual_value-only payloads, and prefixes saved response schema/domain failures with ai_sidecar_diagnostic[<k>].artifact
- CompareValidationResult and AuditSidecarValidationResult response schemas are closed-world, use response-local $ fallback for unrepresentable unknown keys, and forbid error when status is valid
- CompareValidationResult and AuditSidecarValidationResult response schemas map unrepresentable unknown keys inside error objects to response-local error, not response-local $
- CompareValidationResult and AuditSidecarValidationResult top-level schema / status / mode failures use fixed field, expected_value, and actual_value shapes
- CompareValidationResult and AuditSidecarValidationResult response error objects require or forbid field / expected_value / actual_value / expected_hash / actual_hash by error kind and reason_code
- CompareValidationResult and AuditSidecarValidationResult forbidden top-level metadata uses discriminator-specific expected_value values such as absent_for_reason_code, absent_for_reference_state, absent_for_source_kind, and absent_for_validation_state
- CompareValidationResult failed responses require error.kind, require error.reason_code only for policy_failure, and reject reason_code for non-policy failures
- CompareValidationResult and AuditSidecarValidationResult failed responses report malformed error object before nested error.kind / error.reason_code schema failures
- CompareValidationResult and AuditSidecarValidationResult response schema / domain failures use fixed first-failure order across top-level object, status/mode, error object, metadata, payload, and unknown fields
- CompareValidationResult and AuditSidecarValidationResult duplicate unknown fields report duplicate_field at unknown-field priority with fixed bytewise tie-breaks
- CompareValidationResult and AuditSidecarValidationResult forbidden payload members choose absent_for_error_kind, absent_for_reason_code, or absent_for_payload_shape by fixed discriminator and subcase rules
- CompareValidationResult and AuditSidecarValidationResult reject error.field strings outside the selected payload shape with unknown_field
- AuditSidecarValidationResult input_policy_field_mismatch serializes included_fields arrays as RFC 8785 canonical JSON strings in expected_value and actual_value
...
- CommandError schema invalid preserves delegated artifact/store validator actual_value values such as profile_set_mismatch, invalid_fixed_value, present, true, and empty_string
...
- CompareValidationResult normalized_result_schema_invalid uses NormalizedCheckResult-local field paths such as results[<i>] and comparison, not normalized_result.* paths
- NormalizeErrorResult uses error.kind = normalize_failure
- NormalizedCheckResult source-copy set excludes result_id, derived artifact_hash, failure_key, entry order, and comparison
- NormalizedCheckResult results[].result_id is still an immediate normalizer copy invariant even though cross-artifact source-copy validation excludes it
- Normalizer source-copy invariant violations are implementation errors and never produce NormalizeErrorResult, NormalizationWriteResult, or partial NormalizedCheckResult artifacts
- release validate-bundle reports saved NormalizedCheckResult source-copy mismatches as audit_bundle_invalid with normalized_check_result[<j>].artifact.results[<i>].<member> fields
...
- AxiomReportStoreManifest schema shape failures use fixed manifest-local fields and axiom_report_store caller-prefixed fields
...
- auxiliary store manifest invalid maps unrepresentable unknown keys to axiom_report_store / axiom_report_store.reports[<i>] or result_store / result_store.results[<i>]
...
- reproducibility MachineCheckResult schema/domain failure accepts the same actual_value set as MachineCheckResult validation, including integer_out_of_range, duplicate_axiom_name, and forbidden_field
...
- training export deduplicates repeated run_artifact_hash records only when all candidate TrainingExample canonical bytes are identical
...
- training export reports simultaneous failures by fixed validation order: input reference, store readable/hash/schema, store entries, cross-store matching, dedup candidate consistency, output conflict, then output write
- training export store manifest invalid maps unrepresentable unknown keys to normalized_store / normalized_store.results[<i>] or result_store / result_store.results[<i>]
...
- training export serializes JSON Lines as one RFC 8785 canonical TrainingExample JSON object plus LF per record, including the final record, and uses empty bytes for zero records
- TrainingExample.input MVP fields are exactly module, checker_profile, and policy_id; trust_mode is not inferred from policy id
