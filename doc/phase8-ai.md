...
```text
1. error object type
2. duplicate key in error object
...
10. unknown field, bytewise field name order
```
...
4. process_launched conditional required / forbidden fields that apply to the current
   process_launched value, in the order
   checker_binary_hash, checker_id, checker_build_hash
...
`process_launched = false` で `checker_binary_hash`、`checker_id`、または
...
9. unknown field in comparison, bytewise field name order
```
...
`duplicate_field`、`duplicate_axiom_name`、`forbidden_field` のいずれかを入れます。
...
12. unknown error field, by bytewise field name order
13. unknown top-level field, by bytewise field name order
```
...
`forbidden_field`、`failure_key_mismatch`、`comparison_field_mismatch` のいずれかを入れます。
...
12. error payload member required / forbidden / type / hash / scalar validation,
    in error payload order
13. unknown error field, by bytewise field name order
14. unknown top-level field, by bytewise field name order
```
...
- normalize machine_result_schema_invalid allows invalid_path / non_positive_integer / integer_out_of_range / forbidden_field actual_value where MachineCheckResult schema requires them
