...
- [ ] `crates/npa-kernel` に universe constraints の構造化型を追加し、`zero / succ / max / imax / param` と整合させる。
- [ ] declaration ごとの universe params / constraints を kernel declaration と certificate declaration に表現できるようにする。
- [ ] constraint canonicalization と deterministic hash を `crates/npa-cert` に追加する。
- [ ] unresolved universe meta を certificate encode / decode / verifier / reference checker が拒否する。
- [ ] Option A の equality-only universe policy を MVP の既定として明記し、cumulativity は明示 feature flag なしでは入れない。

受け入れ条件:

- [ ] `List.map` 相当の empty constraint と `max u v <= w` 相当の non-empty constraint が canonical hash を持つ。
- [ ] universe param の順序、重複、unknown param、non-canonical level expression が deterministic error になる。
- [ ] certificate hash / import hash が universe constraints によって安定して変化する。
- [ ] reference checker と fast verifier が constraint canonical bytes を同じ意味で検査する。
...
- [ ] `/graph/dependencies` / `/graph/related` / `/graph/query` 相当の Human API wrapper を `crates/npa-api` に追加する。
- [ ] Phase 7 premise retrieval が precomputed theorem graph snapshot を ranking feature として使えるようにする。
- [ ] graph proximity score は proof acceptance / checker verdict に影響しないことを test で固定する。
- [ ] proof minimization が graph を使って不要 import candidate を提案できるようにする。
- [ ] graph extraction を candidate ごとの hot path に入れない performance regression を追加する。

受け入れ条件:

- [ ] declaration ごとの direct / transitive dependencies と related theorem query が deterministic に返る。
- [ ] graph result node は certificate-bound public export に限定される。
- [ ] graph score の有無で final certificate hash と checker result は変わらない。
- [ ] Phase 7 retrieval は graph snapshot missing 時も既存の deterministic fallback を持つ。
