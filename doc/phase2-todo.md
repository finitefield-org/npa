...
この TODO は `doc/phase2.md` を正とし、現在の `crates/npa-cert` 実装との差分を
実装タスクに分解したものです。
...
主な未実装範囲は、`doc/phase2.md` の 7.1 / 11.1.1 / 12.7 で追加された
...
`doc/phase2.md` 12.7 の項目を、そのまま test checklist として実装します。
...
```text
- doc/phase2.md 11.1.1 の producer API が crates/npa-cert に実装されている
...
- Def / Inductive の hash payload contract が doc/phase2.md と一致している
- doc/phase2.md 12.7 の producer separation tests が自動テストで通る
- cargo fmt --all
- cargo clippy --workspace --all-targets -- -D warnings
