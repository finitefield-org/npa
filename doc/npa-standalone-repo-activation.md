...
- `doc/npa-mathlib-public-release-plan.md`
...
- Source provenance is documented in `fixtures/npa-std/README.md`: the
  checked-in `source.npa` files are source-package skeletons that fix module
  membership and import intent, and certificate contents are regenerated by
  the deterministic Rust core-module builder selected with
...
- `generated/package-lock.json`, `generated/axiom-report.json`,
  ...
  `sha256:836762f3c973db1ec9217a841ca38f8d031ca8534178ea8704f11459382f5f96`.
- `cargo run -q -p npa-cli -- package check --root fixtures/npa-std --json`
  passed.
- `cargo run -q -p npa-cli -- package build-certs --root fixtures/npa-std --check --json`
  passed.
...
- Deliverables:
  - Update `doc/registry-readiness.md` or a successor release evidence record.
  - Update `doc/npa-mathlib-public-release-plan.md` with activation status.
