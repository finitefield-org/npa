...
CLR-08 の external checker package command は実装済みです。ただし release evidence として
有効になるのは、`npa-checker-ext` executable が fresh checkout または documented CI
environment で build / 配置済みで、runner policy と checker binary registry がその
binary identity と executable hash を明示している場合だけです。
...
この command は package manifest、package lock、canonical `.npcert`、import certificates、
runner policy、checker registry、checker executable、axiom policy だけを読む source-free
verification path です。`.npa` source、replay / meta files、theorem index、AI traces、
network registry data、hidden package caches は checker input ではありません。registry が
実行可能な `npa-checker-ext` を指していない場合、command は structured diagnostic で失敗します。
...
package verify-certs
  ...
  `--checker-registry` を必須にし、registry-selected `npa-checker-ext` を runner-owned
  policy で起動して MachineCheckResult JSON diagnostics を
...
`npa-package-release.yml` は release base 用に同じ artifact checks に加えて、
