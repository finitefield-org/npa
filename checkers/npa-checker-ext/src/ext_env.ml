...
let level_succ level = Ext_level.Succ level
...
let builtin_hash_tag dotted =
  ...
  | "Setoid" -> Some "npa.quotient-v1.builtin.setoid.v1"
  | "RelEquiv" -> Some "npa.quotient-v1.builtin.rel-equiv.v1"
  | "Setoid.mk" -> Some "npa.quotient-v1.builtin.setoid.mk.v1"
  | "Setoid.r" -> Some "npa.quotient-v1.builtin.setoid.r.v1"
  | "Quotient" -> Some "npa.quotient-v1.builtin.quotient.v1"
  | "Quotient.mk" -> Some "npa.quotient-v1.builtin.quotient.mk.v1"
  | "Quotient.sound" -> Some "npa.quotient-v1.builtin.quotient.sound.v1"
  | "Quotient.lift" -> Some "npa.quotient-v1.builtin.quotient.lift.v1"
  | "Quotient.lift2" -> Some "npa.quotient-v2.builtin.quotient.lift2.v1"
  | "Quotient.indProp" -> Some "npa.quotient-v3.builtin.quotient.ind-prop.v1"
  | _ -> None
...
let setoid level carrier = app (builtin_const "Setoid" [ level ]) carrier

let rel_equiv level carrier relation =
  apps (builtin_const "RelEquiv" [ level ]) [ carrier; relation ]

let setoid_relation level carrier setoid lhs rhs =
  apps (builtin_const "Setoid.r" [ level ]) [ carrier; setoid; lhs; rhs ]

let quotient level carrier setoid =
  apps (builtin_const "Quotient" [ level ]) [ carrier; setoid ]

let quotient_mk level carrier setoid value =
  apps (builtin_const "Quotient.mk" [ level ]) [ carrier; setoid; value ]

let setoid_type level = pi (sort (level_succ level)) (sort (level_succ level))

let rel_equiv_type level =
  let relation_ty = pi (bvar 0) (pi (bvar 1) (sort Ext_level.Zero)) in
  pi (sort (level_succ level)) (pi relation_ty (sort Ext_level.Zero))

let setoid_mk_type level =
  let relation_ty = pi (bvar 0) (pi (bvar 1) (sort Ext_level.Zero)) in
  let equivalence_ty = rel_equiv level (bvar 1) (bvar 0) in
  let setoid_ty = setoid level (bvar 2) in
  pi (sort (level_succ level)) (pi relation_ty (pi equivalence_ty setoid_ty))

let setoid_relation_type level =
  pi
    (sort (level_succ level))
    (pi (setoid level (bvar 0)) (pi (bvar 1) (pi (bvar 2) (sort Ext_level.Zero))))

let quotient_type level =
  pi
    (sort (level_succ level))
    (pi (setoid level (bvar 0)) (sort (level_succ level)))

let quotient_mk_type level =
  pi
    (sort (level_succ level))
    (pi (setoid level (bvar 0)) (pi (bvar 1) (quotient level (bvar 2) (bvar 1))))

let quotient_sound_type level =
  let relation_premise = setoid_relation level (bvar 3) (bvar 2) (bvar 1) (bvar 0) in
  let quotient_for_s = quotient level (bvar 4) (bvar 3) in
  let lhs = quotient_mk level (bvar 4) (bvar 3) (bvar 2) in
  let rhs = quotient_mk level (bvar 4) (bvar 3) (bvar 1) in
  let equality = eq (level_succ level) quotient_for_s lhs rhs in
  pi
    (sort (level_succ level))
    (pi
       (setoid level (bvar 0))
       (pi (bvar 1) (pi (bvar 2) (pi relation_premise equality))))

let quotient_lift_type carrier_level result_level =
  let relation_premise =
    setoid_relation carrier_level (bvar 5) (bvar 3) (bvar 1) (bvar 0)
  in
  let lhs = app (bvar 3) (bvar 2) in
  let rhs = app (bvar 3) (bvar 1) in
  let compatibility_result = eq (level_succ result_level) (bvar 5) lhs rhs in
  let compatibility_ty =
    pi (bvar 3) (pi (bvar 4) (pi relation_premise compatibility_result))
  in
  pi
    (sort (level_succ carrier_level))
    (pi
       (sort (level_succ result_level))
       (pi
          (setoid carrier_level (bvar 1))
          (pi
             (pi (bvar 2) (bvar 2))
             (pi compatibility_ty (pi (quotient carrier_level (bvar 4) (bvar 2)) (bvar 4))))))

let quotient_lift2_type carrier_level result_level =
  let relation_lhs =
    setoid_relation carrier_level (bvar 7) (bvar 5) (bvar 3) (bvar 2)
  in
  let relation_rhs =
    setoid_relation carrier_level (bvar 8) (bvar 6) (bvar 2) (bvar 1)
  in
  let lhs = apps (bvar 6) [ bvar 5; bvar 3 ] in
  let rhs = apps (bvar 6) [ bvar 4; bvar 2 ] in
  let compatibility_result = eq (level_succ result_level) (bvar 8) lhs rhs in
  let compatibility_ty =
    pi
      (bvar 3)
      (pi
         ...
         (pi
            (bvar 5)
            (pi (bvar 6) (pi relation_lhs (pi relation_rhs compatibility_result)))))
  in
  pi
    (sort (level_succ carrier_level))
    (pi
       ...
       (pi
          ...
          (pi
             ...
             (pi
                compatibility_ty
                (pi
                   (quotient carrier_level (bvar 4) (bvar 2))
                   (pi (quotient carrier_level (bvar 5) (bvar 3)) (bvar 5)))))))

let quotient_ind_prop_type level =
  let quotient_for_s = quotient level (bvar 1) (bvar 0) in
  let motive_ty = pi quotient_for_s (sort Ext_level.Zero) in
  let mk_case_result = app (bvar 1) (quotient_mk level (bvar 3) (bvar 2) (bvar 0)) in
  let mk_case_ty = pi (bvar 2) mk_case_result in
  pi
    (sort (level_succ level))
    (pi
       (setoid level (bvar 0))
       (pi motive_ty
          (pi mk_case_ty
             (pi (quotient level (bvar 3) (bvar 2)) (app (bvar 2) (bvar 0))))))
...
let builtin_signature builtin_name decl_interface_hash =
  if builtin_decl_interface_hash builtin_name <> Some decl_interface_hash then None
    ...
    let signature =
      ...
      | "Setoid" -> Some ([ name [ "u" ] ], setoid_type (level_param "u"))
      | "RelEquiv" -> Some ([ name [ "u" ] ], rel_equiv_type (level_param "u"))
      | "Setoid.mk" -> Some ([ name [ "u" ] ], setoid_mk_type (level_param "u"))
      | "Setoid.r" -> Some ([ name [ "u" ] ], setoid_relation_type (level_param "u"))
      | "Quotient" -> Some ([ name [ "u" ] ], quotient_type (level_param "u"))
      | "Quotient.mk" -> Some ([ name [ "u" ] ], quotient_mk_type (level_param "u"))
      | "Quotient.sound" -> Some ([ name [ "u" ] ], quotient_sound_type (level_param "u"))
      | "Quotient.lift" ->
          Some
            ( [ name [ "u" ]; name [ "v" ] ],
              quotient_lift_type (level_param "u") (level_param "v") )
      | "Quotient.lift2" ->
          Some
            ( [ name [ "u" ]; name [ "v" ] ],
              quotient_lift2_type (level_param "u") (level_param "v") )
      | "Quotient.indProp" ->
          Some ([ name [ "u" ] ], quotient_ind_prop_type (level_param "u"))
      | _ -> None
    in
