...
let reject_source_path flag value =
  if has_suffix value ".npa" then Some (flag ^ " must not point to .npa source")
  else None
...
let rec parse args options =
  ...
  | "--output" :: value :: rest -> (
      ...
      | None ->
          if value <> "json" then Parse_error "--output must be json"
...
let run args =
  match args with
  | [ "--version" ] -> ok (Ext_result.checker_id ^ " " ^ Ext_result.checker_version ^ "\n")
