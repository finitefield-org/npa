...
| `CG-24` remaining interface-to-`L2` upgrades | `CG-T47` through `CG-T55` |
...
| `CG-T47` through `CG-T55` | upgrade remaining interface, no-derived, and boundary statements to `L2` derived certificates, or split out an explicit prerequisite blocker before source edits |
...
- Tasks:
  - Replace the Hall no-`L2` boundary with a derived certificate route from
    finite matching, neighborhood, cardinality, and set-system lemmas.
  - Upgrade max-flow/min-cut and Konig-style interfaces from assumed law
    packages to derived statements over certified flow, cut, and matching
    evidence.
  - Keep algorithm traces explicit, but prove the mathematical correctness
    statements from trace certificates rather than treating execution as a
    trusted boundary.
- Deliverables:
  - `L2` derived matching, flow, cut, and Konig theorem modules with updated
    source, replay, metadata, certificates, and AI theorem index entries.
- Acceptance criteria:
  - Hall, max-flow/min-cut, and Konig targets no longer expose `NoL2`,
    `not_l2`, or ownership-boundary theorem declarations for the upgraded
    statements.
  - Any target that still lacks prerequisites is split into a smaller pending
    prerequisite task before source edits, rather than being silently left as an
    interface theorem.
- Verification:
...
- Tasks:
  ...
  - Replace the perfect-graph boundary target with derived coloring and clique
...
- Acceptance criteria:
  - Advanced theorem names with `interface_statement` or boundary statements
    are either replaced by derived `L2` statements or moved to a separately
    tracked prerequisite blocker.
  - No finite Ramsey or finite extremal target remains interface-only solely
...
- Verification:
  ...
  - `cargo run -p npa-proof-corpus -- --changed-only --verified-cache authoring`
  - `./scripts/check-corpus-authoring.sh`
  - `rg -n "interface_statement|boundary_statement" proofs/Proofs/Ai/Combinatorics/Ramsey proofs/Proofs/Ai/Graph/Ramsey proofs/Proofs/Ai/Graph/Extremal proofs/Proofs/Ai/Graph/Coloring/Advanced`
  - `git diff --check`
...
- Acceptance criteria:
  ...
  - Any remaining major structural theorem is blocked only by named topology or
    minor-theory prerequisites, not by an untracked interface wrapper.
- Verification:
  ...
  - `cargo run -p npa-proof-corpus -- --changed-only --verified-cache authoring`
  - `./scripts/check-corpus-authoring.sh`
  - `rg -n "interface_statement|boundary_statement" proofs/Proofs/Ai/Graph/Planar proofs/Proofs/Ai/Graph/Embedding proofs/Proofs/Ai/Graph/Topological proofs/Proofs/Ai/Graph/Minor`
  - `git diff --check`
...
- Tasks:
  ...
  - Replace symmetric-function and association-scheme representation
  ...
  - Split any representation-theory prerequisite that is outside the CG primary
    route into a named dependency task instead of retaining an implicit
    interface theorem.
- Deliverables:
...
- Acceptance criteria:
  - Boundary declarations are removed or replaced for upgraded
...
- Tasks:
  ...
  - Route finite-field, coding-theory, entropy, and probability prerequisites to
    their primary roadmaps instead of leaving untracked interface boundaries.
- Deliverables:
