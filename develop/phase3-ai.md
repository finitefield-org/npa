...
従来の `develop/phase3-human.md` は、人間が読み書きしやすい表層言語を canonical core AST に落とすための
...
- Human Surface は develop/phase3-human.md の別 profile であり、AI 候補生成、tactic execution、search、replay、verify の既定経路に入れない
...
term 単体 API でも `MachineGlobalScopeEntry` の `decl_interface_hash` は
`MachineKernelEnvView` が持つ checked environment view と一致しなければならず、
不一致は elaboration 前に拒否します。
...
`MachineTermCheckResult.core_hash` は owner context を含まない Phase 1 core expression structural hash です。
hash domain は `NPA-PHASE1-EXPR-0.1` で、Phase 2 `NPA-TERM-0.1` とは分離します。
`MachineTermCheckResult.contextual_core_hash` は elaborated core term の構造に加えて、参照した global の
`MachineGlobalScopeEntry` 座標と `decl_interface_hash` に commit します。同じ表示名でも異なる
`decl_interface_hash` を持つ import / checked current declaration / current generated declaration は別 hash になります。
hash domain は `NPA-PHASE3-MACHINE-TERM-CONTEXT-0.1` です。
`contextual_core_hash` は Phase 2 `term_hash` や Phase 5 `MachineExprView.core_hash` と同じ値として扱ってはいけません。

`MachineTermAst` は `"npa.phase3.machine-term-source.v1"` canonical bytes から decode された parsed term AST です。
concrete fields は public API から構築できない opaque data とし、外部 caller は decoder を経由して取得します。
decoder は parser が生成できない resolver 後の internal node、例えば `Local` term tag を受け付けません。
...
`MachineTermElabContext` の local context は outer-to-inner order で渡します。最後の local が de Bruijn index `0`
になります。`expected` は同じ local context / universe parameter context の下で解釈される core expression です。

`MachineTermElabContext` 内部の global scope は exact name lookup 用の閉じた map です。
...
この scope を構築します。
`direct_verified_modules` はこの lookup scope に入る direct imports だけです。
`available_verified_modules` は kernel env が direct imports の型検査に必要な transitive dependency を解決するための closure であり、
その export を lookup scope に追加してはいけません。
内部 `kernel_env` は同じ verified imports / checked current declaration から作った checked environment であり、
...
`CurrentGenerated.decl_interface_hash` は Phase 2 `GlobalRef::LocalGenerated` dependency と同じく、
親 checked current declaration の `decl_interface_hash` と一致しなければなりません。
...
- develop/phase3-ai.md に AI fast path の境界を固定する
...
M9 の CI gate では、wall-clock latency や allocation の固定しきい値を置きません。
実行環境差で flake しやすいためです。代わりに、AI 探索で大量候補を再投入しても壊れない
決定性・状態非依存・resource guard を regression test として固定します。

`same input same output` の対象は次に限定します。
...
diagnostic の人間向け message 文字列、wall-clock timing、allocator の実装詳細は
trusted output として扱いません。
...
- same input から same parsed AST が返る
- same term-level check input から same core hash / contextual core hash / constants が返る
- same failed candidate から same diagnostic kind / structured payload / suggestion が返る
- oversized canonical input は大きな allocation 前に reject される
- cargo fmt --all
