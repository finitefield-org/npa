...
`develop/phase6-human.md` は、人間が読み書きする標準ライブラリの構成、命名、定理セット、属性方針を扱います。
...
`develop/phase2.md`, using the owner certificate context that produced the expression for `GlobalRef` encoding.
...
MVP `SimpSafe` rules are exactly the following small terminating rules from `develop/phase6-human.md`:
...
```text
- develop/phase6-human.md を人間向け標準ライブラリ設計として固定する
- develop/phase6-ai.md は machine artifact / wire contract / validation order の正本にする
