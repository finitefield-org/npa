...
Layer 3D-B, kernel quotient foundation:

| Source corpus module | Public module | Notes |
| --- | --- | --- |
| `Proofs.Ai.Algebra.AbstractGroupQuotient` | `Mathlib.Algebra.Group.Kernel.Quotient` | Kernel relation setoid, kernel quotient carrier, quotient constructor, quotient-to-codomain map, and representative computation facts. |
| `Proofs.Ai.Algebra.AbstractGroupQuotientMul` | `Mathlib.Algebra.Group.Kernel.Quotient.Mul` | Representative multiplication and compatibility with kernel relation equivalence. |
| `Proofs.Ai.Algebra.AbstractGroupQuotientGroup` | `Mathlib.Algebra.Group.Kernel.Quotient.Group` | Quotient multiplication, identity, inverse, and group law facts. |
| `Proofs.Ai.Algebra.AbstractGroupQuotientHom` | `Mathlib.Algebra.Group.Kernel.Quotient.Hom` | Multiplicativity of the canonical quotient-to-codomain map on arbitrary quotient elements. |

Layer 3D-B kernel quotient closure audit is fixed in
`develop/npa-mathlib-layer3d-b-quotient-foundation-closure-audit.md`. The
selected closure is limited to `npa-std v0.1.0`, the released
`npa-mathlib v0.1.7` baseline, and the four new kernel quotient modules. It
uses `Mathlib.Algebra.Group.Kernel.Quotient*` rather than the broader
`Mathlib.Algebra.Group.Quotient` namespace because the selected route is the
...
- `npa-mathlib v0.1.6` is published as a public GitHub Release with the
...
- The `v0.1.6` release bundle hash is
...
- The `v0.1.5` downstream smoke materialized from the published release bundle
  ...
  `Mathlib.Algebra.Group.Subgroup`.
- The `v0.1.6` downstream smoke materialized from the published release bundle
  imports only release-bundle certificate bytes for `Std.Logic.Eq`,
  `Mathlib.Logic.EqReasoning`, `Mathlib.Algebra.Group.Basic`,
  `Mathlib.Algebra.Group.Subgroup`, and
  `Mathlib.Algebra.Group.Subgroup.Order`.
- The `v0.1.7` downstream smoke materialized from the published release bundle
  imports only release-bundle certificate bytes for `Std.Logic.Eq`,
  `Mathlib.Logic.EqReasoning`, `Mathlib.Algebra.Group.Basic`,
...
Status: Completed for the first Layer 3C subgroup containment/order release in
`npa-mathlib v0.1.6`.
...
2. Completed: mapped the source module to the `Mathlib.*` namespace according
   to `npa-mathlib/docs/namespace-policy.md`.
3. Completed: kept package name `npa-mathlib` and used the existing
   `npa-std v0.1.0` hash-pinned imports.
4. Completed: added `Mathlib/Algebra/Group/Subgroup/Order/` in the standalone
   `npa-mathlib` repository.
5. Completed: kept `allow_custom_axioms = false` and
...
6. Completed: regenerated certificates and generated package artifacts:
   `package-lock.json`, `axiom-report.json`, `theorem-index.json`, and
   `publish-plan.json`.
7. Completed: updated downstream source-free smoke to import the Layer 3C
   certificate closure from release-bundle bytes.
8. Completed: ran package gates for `npa-mathlib` and the downstream smoke.
...
Status: Completed for the first Layer 3D-A group kernel/image release in
`npa-mathlib v0.1.7`.
...
3. Completed: kept package name `npa-mathlib` and used the existing
   `npa-std v0.1.0` hash-pinned imports.
4. Completed: added `Mathlib/Algebra/Group/Kernel/` and
   `Mathlib/Algebra/Group/Image/` in the standalone `npa-mathlib` repository.
5. Completed: did not create a separate `Mathlib.Algebra.Group.Hom` module in
   this release.
...
7. Completed: regenerated certificates and generated package artifacts:
   `package-lock.json`, `axiom-report.json`, `theorem-index.json`, and
   `publish-plan.json`.
8. Completed: updated downstream source-free smoke to import the Layer 3D-A
   certificate closure from release-bundle bytes.
9. Completed: ran package gates for `npa-mathlib` and the downstream smoke.
10. Completed: published `npa-mathlib v0.1.7` after release bundle, downstream
