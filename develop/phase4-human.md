...
AI 向けの高速・構造化された Machine Tactic は `develop/phase4-ai.md` の責務であり、この Human syntax を既定入力にしません。
...
AI 経路は `develop/phase4-ai.md` の `MachineTacticCandidate` JSON を受け取り、Human text tactic parser を通らずに
...
AI 向けの同等機能は、`develop/phase4-ai.md` の Machine Tactic として、Human parser を経由しない高速経路で実装します。
