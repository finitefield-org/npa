...
それらの machine artifact / wire contract / validation order は `develop/phase6-ai.md` が正本です。
...
certificate artifact path、canonical module order、package locator rule は `develop/phase6-ai.md` の固定 table に従います。
...
Human source build / local debug view では、各モジュールから次のような
per-module 派生ファイルを出してよいです。
...
ただし、これらの per-module JSON は Human Profile 側の説明・デバッグ・ローカル cache 用 view です。
Phase 6 AI MVP の wire artifact set は `develop/phase6-ai.md` の `Std.machine-*.json` 群が正本であり、
...
このリポジトリで正本として commit するのは、Human source layout / skeleton、Rust implementation / tests / docs です。
...
Human source / IDE metadata としては、最低限これを受け付けます。
...
ここでの属性は human-facing source annotation / IDE hint です。
Phase 6 AI MVP の `MachineStdTheoremEntry.attributes` は source annotation をそのまま載せず、
validated rewrite/simp profile から導出した `simp` / `rw` と、`apply` mode に対応する `apply` だけを emit します。
`intro` / `elim` / `refl` / `trans` / `congr` は Human 側の検索・表示ヒントとしては使えますが、
`npa.stdlib.theorem-index.mvp.v1` には出しません。
これにより AI 経路は Human 属性表を解釈せず、certificate-bound な machine profile だけから候補集合を再構成できます。
...
`Std.Nat` の MVP release module 自体には、次のような `Std.Algebra.Basic` 由来の定理を追加しません。
追加すると `Std.Nat` が `Std.Algebra.Basic` に依存し、`std.nat.mvp` の direct import / closure と矛盾します。
Phase 6 のテストで必要なら、release module ではない test fixture か将来の `Std.Nat.Algebra` が
`Std.Nat` と `Std.Algebra.Basic` の両方を import して次を証明します。
...
この JSON は人間向け theorem search view の形を示す sketch です。
AI Profile の正本 schema は `MachineStdTheoremIndex` / `MachineStdTheoremEntry` であり、
`global_ref`, `statement_core_hash`, `statement_head`, `modes`, `rewrite_descriptors`,
`axiom_dependencies` などを certificate verifier output と rewrite/simp profile から再構成します。
MVP の machine theorem index では `proof_term_size = null` で、non-null は stale metadata として拒否します。
上の `rewrite` object は、人間向け表示では便利ですが、AI artifact では validated `rewrite_descriptors` に対応します。
...
ここでの `intro` / `elim` は Human IDE の表示・検索カテゴリです。
AI MVP theorem index では `Intro` / `Elim` attributes を emit せず、該当定理は `apply` mode / `apply` attribute として扱います。
...
この list は **simp-lite に入れない定理** の一覧です。
Phase 6 AI MVP の rewrite profile では、このうち次だけを exact `RwOnly` として emit します。
...
一方、`Nat.mul_comm` / `Nat.mul_assoc` / `List.map_comp` など、AI Profile の `RwOnly` 固定集合にない定理は
`npa.stdlib.mvp.v1` の rewrite profile にも出しません。
より強い正規化や可換・結合変形は将来の専用 tactic に回します。
...
ただし現在の kernel が標準 `Eq.rec` family head を `Std.Logic` の `AxiomDecl` として表現する場合だけ、
その exact `Std.Logic.Eq.rec` を kernel-standard exception として axiom report / allowlist に明示してよいです。
任意の custom axiom、`Classical.choice`、`funext`、`propext` は MVP 標準ライブラリに入れません。
...
- `Eq.rec` を標準 axiom として表現する kernel では、その exact exception だけが axiom report / allowlist に出る
- Nat/List/Algebra の proof が `Eq.rec` を使う場合、その entry は imported `Std.Logic Eq.rec` として verifier-derived
  `module_axioms` / `transitive_axioms` に出てよいが、custom axiom は出ない
