...
enum Phase9IndependentCheckerProfile {
    Phase8MvpReference,        // tag 0
    QuotientV1Reference,       // tag 1, implemented as IndependentCheckerQuotientV1Reference
}
...
`Phase9IndependentCheckerProfile` は tag 0 の `Phase8MvpReference` を default / MVP profile とし、
P9H-12 で tag 1 の quotient-capable `QuotientV1Reference` を opt-in profile として追加します。
...
checker support matrix は次で固定します。
...
QuotientConstruction:
  Phase8MvpReference は core v0.1 / Phase 8 MVP profile と同じく quotient をサポートしない。
  MvpV1 validator は envelope / options / payload の deterministic validation と deterministic rejection までは行ってよいが、
  `quotient_v1` certificate を採用して Phase8MvpReference checker に渡してはいけない。
  QuotientV1Reference は `quotient_v1` primitive refs / `eq` public interface を feature-specific に検証し、
  kernel / source-free reference checker が同じ quotient-capable profile で受理した場合だけ
  `QuotientConstruction` success payload を返す。
  primitive refs / `eq` public interface mismatch は checker support ではなく
  QuotientConstruction feature-specific validation error として扱う。
  caller が明示的に QuotientV1Reference を指定しない限り、AI hot path の default は Phase8MvpReference の
  deterministic UnsupportedFeature のままである。
...
この節は `quotient_v1` feature profile の deterministic protocol を定義します。
core v0.1 / Phase 8 MVP / `Phase8MvpReference` checker profile では quotient は非対応です。
したがって `Phase9AiProfileVersion::MvpV1` で `Phase8MvpReference` を選んだ request は、
quotient certificate adoption の success path を持ちません。
P9H-12 で追加した `QuotientV1Reference` profile を caller が明示した場合だけ、
`quotient_v1` 内の最小 success path を有効化します。これは core v0.1 MVP や
default AI hot path に quotient を含める意味ではありません。
...
`quotient_v1` QuotientConstruction profile の deterministic protocol cap は次で固定します。
...
`Rejected { error = EnvelopeMalformed, feature_error = None }`
として拒否します。
`quotient_v1` QuotientConstruction profile では task-local global ref を使わず、params は de Bruijn local binder だけで参照します。
...
`Rejected { error = TargetFingerprintMismatch, feature_error = None }`
として拒否します。
`expected_decl_hash = None` の場合、quotient-capable profile では validator は check success response に
再計算した `decl_certificate_hash` を返します。
...
`quotient_v1` profile の quotient primitive は Phase 9 Human Profile の `Setoid` / `Quotient` primitive に合わせます。
...
`quotient_v1` profile では feature report は profile selection bit であり、certificate ごとに custom descriptor bytes を持ちません。
...
`quotient_v1` profile の `MachineQuotientOperationCandidate` は unary lift だけを表します。
...
multi-argument lift、dependent motive、quotient-to-quotient operation の追加自動展開は
`quotient_v1` の最小 profile では `UnsupportedFeature` です。
...
したがって `quotient_v1` profile の quotient declaration identity は proof-sensitive です。
...
`quotient_v1` profile の `/machine/phase9/quotient/check` は operation declaration hash を返さず、compatibility 検査だけを行います。
したがって quotient-capable profile の success response では、
`QuotientConstruction.decl_certificate_hash` は quotient type declaration だけの identity です。
...
`quotient_v1` profile validator は operation ごとの `DefDecl` bytes を success payload に含めず、success 前の Phase 8 independent checker にも
...
この採用条件は quotient-capable independent checker profile 用です。
`Phase8MvpReference` では、下の条件を満たす候補であっても certificate 実行前に `UnsupportedFeature` として拒否します。
`QuotientV1Reference` では、下の条件と checker recheck を満たす場合だけ success payload を返します。
...
`SmtSolver` / `SmtLogic` / `SmtEncoderVersion` / `SmtCommandProfile` / `SmtCertificateFormat` /
...
`AletheOpaqueV1` / `LfscOpaqueV1` は native proof bytes を opaque artifact として hash / size / schema validation するだけで、
solver-native proof rule を trusted に実行しません。
`SolverResultOnlyV1` は `unsat` など solver result だけが渡されたことを表す rejection-only format です。
別 solver の native proof format を成功扱いする場合は、別の非空 `SmtRuleRegistryProfile` と checker support entry を
追加してから有効化します。
...
Human SMT schema は、normalized IR の `problem_hash` とは別に deterministic SMT-LIB bytes から
`smt_problem_hash` を計算します。
P9H-13 の SMT-LIB generator は QF propositional / EUF / simple LIA の closed table だけを持ち、
`set-option :produce-proofs true`、`set-logic`、declaration、assertion、`check-sat`、`get-proof` を
canonical order で出力します。
`SmtSymbol.ascii` は raw SMT-LIB identifier ではないため、SMT-LIB bytes では pipe-quoted symbol として
deterministic に render します。
...
`Rejected { error = UnsupportedFeature, feature_error = Some(SmtCertificate(UnsupportedFragment)) }`
として拒否します。
...
proof payload validation は `certificate_format` ごとに分岐します。
`MvpProofNodeTableV1` は canonical `SmtProofNodeTable` として decode し、malformed table bytes は
`Rejected { error = EnvelopeMalformed, feature_error = Some(SmtCertificate(MalformedProofPayload)) }`
として拒否します。
`AletheOpaqueV1` / `LfscOpaqueV1` は native proof bytes を opaque artifact として扱い、非空 bytes、
declared artifact `file_hash` / `size_bytes`、および format-tagged `payload_hash` だけを検査します。
opaque payload を検査できても `MvpEmptyRegistryV1` では solver-native rule が存在しないため success にはなりません。
`SolverResultOnlyV1` は bytes が exact `unsat\n` であることだけを schema validation した後、
`Rejected { error = UnsupportedFeature, feature_error = Some(SmtCertificate(SolverResultOnly)) }`
として拒否します。

`MvpProofNodeTableV1` proof payload validation 内の deterministic order は次で固定します。

```text
0. Inline bytes acquisition and byte cap check, or Artifact path validation /
   declared size cap check / file read / file_hash / size_bytes check
1. full SmtProofNodeTable canonical decode
   - table tag / field boundary / scalar field decode
   - nodes vector length prefix and node count cap
   - nested CoreExpr / SmtExpr / SmtSortExpr count cap validation
2. payload_hash recomputation and comparison
3. table shape validation against selected candidate / encoded problem / command context
```
...
`Rejected { error = PayloadHashMismatch, feature_error = None }` です。
path validation を通過した artifact bytes を取得できない場合は `Error::ArtifactUnavailable` です。
step 1 の canonical framing violation、nested decode violation、または `nodes.len() > 1_000_000` は
`Rejected { error = EnvelopeMalformed, feature_error = Some(SmtCertificate(MalformedProofPayload)) }`
として拒否し、`payload_hash` mismatch より優先します。
step 1 が成功した後に `payload_hash` が一致しない場合は
`Rejected { error = PayloadHashMismatch, feature_error = None }` です。
step 2 の hash input は、step 1 で full canonical decode できた provided bytes そのものです。
validator は table を再serializeして hash input を作ってはいけません。
step 2 で `payload_hash` が一致しても、step 3 の table shape validation が失敗した場合は
対応する structured SMT feature error で拒否します。
...
opaque_payload_hash =
  sha256(
    "npa.phase9_ai.smt.opaque_proof_payload.v1"
    || certificate_format canonical bytes
    || length_prefixed raw proof bytes
  )
```

`MvpProofNodeTableV1` の `proof_payload` bytes は canonical SMT proof node table として decode できなければなりません。
...
同様に `/machine/phase9/quotient/check` は `Phase8MvpReference` の間は deterministic rejection surface だけを持ち、
quotient certificate success は返しません。
P9H-12 の `QuotientV1Reference` profile を明示した場合だけ、quotient certificate success path を有効化します。
...
EnvelopeMalformed:
  ...
  feature_error = Some(SmtCertificate(NonCanonicalPayload))
    encoded problem command / core-expression / expression / sort count cap、command phase/order/id uniqueness/symbol table/sort validation、
  ...
  feature_error = Some(SmtCertificate(MalformedProofPayload))
    selected `SmtCertificateFormat` の proof payload bytes が canonical proof-node table / opaque payload / solver-result-only schema として
    decode または schema validation できない場合。
    canonical shape の違反。
...
UnsupportedFeature:
  ...
  SMT encoded problem / proof payload が selected `SmtLogic` で許可しない builtin sort / literal / operator を含む場合は
    feature_error = Some(SmtCertificate(UnsupportedFragment))。
  SMT payload が solver の `unsat` 等の result だけを持つ場合は
    feature_error = Some(SmtCertificate(SolverResultOnly))。
...
FeatureRejected:
  ...
  QuotientConstruction:
    ...
    new quotient construction validator は返してはいけない。
  SmtCertificate:
    ...
    UnsupportedFragment, SolverResultOnly, MalformedProofPayload
    ...
    UnsupportedFragment は selected `SmtLogic` / P9H-13 SMT-LIB generator が扱わない theory sort、literal、operator、
    datatype、bitvector、または unsupported native proof fragment を見つけた場合に使う。
    SolverResultOnly は solver result bytes だけがあり proof certificate / reconstruction material がない場合に使う。
    MalformedProofPayload は selected `SmtCertificateFormat` の outer proof payload schema を decode / validate できない場合に使う。
    RuleFingerprintMismatch は、payload が rule descriptor または rule name binding を canonical data として持つ
...
`QuotientConstruction` variant は quotient-capable independent checker profile で quotient success path を有効化した場合の
schema です。`Phase8MvpReference` profile では到達不能ですが、P9H-12 の `QuotientV1Reference` profile では
`/machine/phase9/quotient/check` がこの variant を返せます。
...
現行 MVP で success 前に independent checker を実行する endpoint は、validator が standalone certificate candidate を
この endpoint 内で deterministic に構成するものだけです。
`AdvancedInductive` は single-declaration inductive certificate package、
...
`QuotientConstruction` は `Phase8MvpReference` では success path がないため checker 実行に到達しません。
`QuotientV1Reference` では quotient type `DefDecl` certificate package を構成し、success を返す前に
quotient-capable Phase 8 independent checker へ渡します。
...
`QuotientConstruction.decl_certificate_hash` は quotient-capable profile で success path を有効化した場合に、
validator が再構成した quotient declaration の Phase 2 `decl_certificate_hash` です。
...
MVP では quotient と SMT の endpoint も提供します。default の `Phase8MvpReference` と
`SmtRuleRegistryProfile::MvpEmptyRegistryV1` の範囲では certificate adoption の success path を持ちません。
quotient は P9H-12 の `QuotientV1Reference` opt-in profile だけ success path を持ち、SMT success は
Post-MVP の非空 registry profile に分けます。
...
```text
primitive refs、setoid、proof obligation、operation validation の deterministic 境界を固定し、
default Phase8MvpReference の unsupported surface と P9H-12 QuotientV1Reference success profile を分離する。
```
...
- QuotientV1Reference では quotient certificate adoption success を返す support check
```
...
- Phase8MvpReference では success adoption が UnsupportedFeature として deterministic に拒否される
- QuotientV1Reference では valid quotient candidate が QuotientConstruction success payload を返す
...
```text
- default Phase8MvpReference への quotient success 追加
- SMT / theorem graph / formalization の success profile 追加
```
