...
従来の `develop/phase5-human.md` は、人間が IDE / Web UI / CLI から goal を読み、tactic を試し、
...
`import_closure` にあるが `imports` にない module は、Phase 2 verifier session と Phase 4 kernel env の
transitive dependency reconstruction にだけ使い、Phase 4 の visible `imports`、Machine Surface term の
direct global scope、theorem search premise、AI candidate の direct tactic head にはなりません。
...
MachineDependencyRefWire canonical bytes:
  ...
  - builtin:
      builtin fully-qualified name canonical bytes
```
...
`MachineDependencyRefWire` / `MachineAxiomRefWire` が session context に存在する import/current declaration、
または selected builtin profile の builtin interface へ解決できるかは checked_current_decls validation step 6 で検査します。
...
CurrentDecl axiom dependency derivation:
  0. current core_decl が AxiomDecl の場合は InvalidCheckedCurrentDecl / UncheckedCurrentDecl として拒否する。
     checked_current_decls に入る current declaration は kernel-checkable body を持つ
     DefDecl / TheoremDecl / InductiveDecl だけでなければならない。
  ...
  2. 各 direct dependency entry について:
       imported ref:
       ...
       builtin ref:
         selected builtin profile の builtin interface hash と照合する
         builtin axiom interface の場合は MachineAxiomRefWire::Builtin を追加する
         builtin non-axiom interface の場合は axiom_dependencies には追加しない
  3. MachineAxiomRefWire canonical bytes で sort/dedup する
```
...
checked_current_decls validation:
  ...
  6. 各 package canonical bytes を再 encode し、CurrentDeclDependencyReport の refs が
     session imports / prior current declarations / selected builtin profile の builtin interface の
     許可範囲で解決できることを確認する
...
`allow_axioms` item は semantic validation で実在する axiom declaration または builtin axiom interface だけを指さなければなりません。
...
axiom dependency が `AxiomDecl` / builtin axiom interface 以外、または constructor / recursor などの generated artifact を指す場合は
...
wire encoding では各 item に `kind = "imported" | "current_module" | "builtin"` を必ず入れます。
...
MachineAxiomRefWire item schema:
  ...
  builtin:
    required fields = kind, name, decl_interface_hash
    forbidden fields = module, export_hash, source_index
```
...
AxiomRef to MachineAxiomRefWire:
  precondition:
    ref target must resolve to declaration kind AxiomDecl or a builtin axiom interface.
  ...
  in verified import context C for module M:
    ...
    GlobalRef::Builtin(name, decl_interface_hash):
      builtin profile が提供する builtin axiom interface の場合だけ Builtin を作る
  ...
  in generated certificate current-module context:
    GlobalRef::Imported(import_index, name, decl_interface_hash):
      generated certificate import table[import_index] の module / export_hash と
      name / decl_interface_hash から Imported を作る。
      session direct imports だけでなく、選択された import の公開 interface(type / transparent body) を
      Phase 2 verifier が復元するために必要な transitive import と、参照された direct import export の
      Phase 2 axiom provenance に必要な transitive axiom-origin import も certificate import table に含まれ得る。
...
逆写像できない certificate-local axiom ref、または `AxiomDecl` / builtin axiom interface 以外を指す axiom ref は `VerifyFailed` です。
...
builtin dependency は `GlobalRef::Builtin(name, decl_interface_hash)` の hash を builtin interface tag から再計算して照合し、
`MachineDependencyRefWire::builtin` に変換します。
`DependencyEntry.decl_interface_hash` は解決先の checked signature hash、または builtin の場合は builtin interface hash と一致しなければなりません。
...
selected goal は request top-level の `goal_id` だけで指定し、`candidate` 内に `goal_id` を入れてはいけません。
Phase 5 adapter は snapshot / state / goal lookup 後に、この top-level `goal_id` を Phase 4
`validate_machine_tactic_candidate(goal_id, candidate)` へ渡します。
...
MVP suggested candidate validation:
  ...
  3. selected goal_id を渡した Phase 4 candidate canonicalization による MachineTactic canonical bytes の構築
...
Phase 4 MachineTacticDiagnosticKind -> MachineApiErrorKind:
  ...
  ProofExprScopeError / UnresolvedGoal / AmbiguousKernelEnvDecl / KernelRejected:
    InvalidMachineProofState
  MachineTermElaborationError:
    MachineTermElaborationError
  UnknownName:
  ...
  SimpStepLimitExceeded:
    BudgetExceeded
  UnsupportedMachineTactic / TacticPrimitiveUnavailable:
    UnsupportedTactic
...
Replay step execution:
  ...
  4. step.candidate を current replay state に対して `validate_machine_tactic_candidate(step.goal_id, step.candidate)` する
...
Verify certificate construction:
  ...
  2. certificate import table =
       session direct imports plus transitive imports required by selected import public interfaces
       and any Phase 2 axiom-origin imports required by referenced direct-import exports,
       in canonical order by (module, export_hash, certificate_hash).
       A transitive import is included here when a selected import's public export type or transparent
       export body contains `GlobalRef::Imported`, or when a direct import export referenced by the generated
       root module has `axiom_dependencies` containing `GlobalRef::Imported` through that direct import's
       certificate import table. Hidden transitive dependencies that are not part of any selected import
       public interface and are not axiom origins are not added to the generated certificate import table;
       they remain in `dependency_import_closure` so the generated `import_payload` can be verified by a
       fresh Phase 2 verifier session.
     ...
     projected dependency entries, and projected axiom refs:
       ...
       GlobalRef::LocalGenerated(parent_source_index, generated_name)
         -> GlobalRef::LocalGenerated(SourceIndexToCertificateDeclIndex[parent_source_index],
                                      generated_name)
     Imported refs are resolved against the generated certificate import table; axiom-origin imports
     must be present before Phase 2 certificate construction.
...
生成 certificate は session direct imports と、選択された import の公開 interface 復元に必要な transitive imports、
および参照された direct import export の Phase 2 axiom provenance に必要な transitive axiom-origin imports を
canonical order の import table とし、
