...
- `npa-proof-corpus` は `--build-module`、`--module`、`--changed-only`、`--write-ai-index`、
  `--write-replay`、`--shard`、`--failures-out` を持つ。
- `--build-modules`、`--build-modules-file`、`--verified-cache`、`--promote-plan`、
  `--promote-materialize` は未実装。
- `./scripts/check-corpus.sh` は現時点の full corpus gate であり、split gate scripts は未実装。
...
- Acceptance criteria:
  ...
  - The inventory states that `--build-modules`, `--verified-cache`, and promotion commands are not implemented yet.
...
- Verification:
  ...
  - `./scripts/check-corpus-authoring.sh`
- Notes:
  - If `check-corpus-authoring.sh` is not implemented yet, this milestone must depend on or include PCT-03 completion.
