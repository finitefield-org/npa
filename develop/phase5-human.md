...
決定的・transactional な機械 API 契約は `develop/phase5-ai.md` に分けます。
...
`develop/phase5-ai.md` の `/machine/tactics/run` / `/machine/tactics/batch` です。
...
AI 探索器向けには、同じ検索結果を `develop/phase5-ai.md` の
...
探索ノード identity、batch 実行、replay、verify は `develop/phase5-ai.md` の
...
AI 証明探索器向けの deterministic payload は `develop/phase5-ai.md` の `/machine/prompt_payload` を使います。
...
13. optional assistant payload
   ...
   AI 探索器向けの deterministic payload は `develop/phase5-ai.md` 側で実装する。
...
この Phase 5 Human と `develop/phase5-ai.md` の Machine API がそろうと、次の Phase 6 以降で
