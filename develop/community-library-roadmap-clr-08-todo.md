...
Source: `develop/community-library-roadmap-todo.md` CLR-08
...
- `crates/npa-cli` package high-trust command, requiring release policy,
  runner policy, challenge runner policy, checker registry, and release audit
  bundle evidence
- `crates/npa-package` `npa.package.verified_high_trust.v0.1` schema,
  generator, validator, and reference-only rejection
- `ci-templates/github-actions/npa-package-high-trust.yml` opt-in high-trust
  CI template, kept separate from the base PR/release templates
- deterministic external checker benchmark summary and release audit bundle
  validation links back to checker result hashes
- `scripts/phase8-release-audit.sh` fixture gate
```

Still not generated by the current base release templates or repository-local
active gates:
...
- checked `generated/verified-high-trust.json` in an external seed repository,
  unless that repository opts into the high-trust template and supplies all
...
CLR-08 is complete as a repository contract: the external checker command,
OCaml clean-room source project, high-trust package command,
`verified_high_trust` artifact generator, opt-in high-trust template, and
benchmark/audit summary contract exist. What remains outside this repository is
the consuming repository's pinned built `npa-checker-ext` binary and the
corresponding external / high-trust-reference release audit evidence. CLR-09
may dogfood a reference-checker-only seed library while those high-trust
evidence files are absent. Docs must clearly say that `verified_high_trust` is
generated only from external and high-trust-reference release audit evidence,
not from reference-only evidence, and that base release CI runs without the
external checker gate.
...
- Depends on: CLR-03
- Inputs:
  - `develop/community-library-roadmap-todo.md` CLR-08
  - `develop/phase8-human.md` sections 10, 11, 14, 18, 22
  - `develop/phase8-ai.md` RunnerPolicy, ReleasePolicy, CheckerBinaryRegistry sections
  - `crates/npa-api/src/independent_checker.rs`
...
- Verification:
  - `rg -n "external|high-trust-reference|verified_high_trust|PR mode|not proof input" develop/community-library-roadmap-clr-08-todo.md docs/external-theorem-library-ci.md develop/community-library-roadmap-todo.md`
  - `git diff --check`
- Notes:
...
- Verification:
  - `rg -n "verify-certs --root \\. --checker external|package high-trust|verified-high-trust|checker-registry|runner-policy" ci-templates docs/external-theorem-library-ci.md`
...
- Deliverables:
  ...
  - Docs say CLR-08's repository contract is complete and whether the first
    seed library supplies optional high-trust evidence.
...
- Verification:
  ...
  - `git diff --check`
- Notes:
  - CLR-09 can proceed without high-trust evidence from the seed repository
    when the seed library intentionally uses reference-checker-only release
    gates.
  - Completed for the current handoff: docs distinguish implemented
    `--checker external`, `package high-trust`, `verified_high_trust`, opt-in
    high-trust CI, and benchmark/audit surfaces from the consuming repository's
    still-optional high-trust evidence files.
...
Finding: External checker availability is still evidence supplied by the
consuming repository, so a task list that always emits `verified_high_trust`
would overstate current capability.
Fix: The high-trust command and artifact generator require structured failure
or `not_verified` behavior when external / high-trust-reference evidence is
absent, and forbid verified artifacts from reference-only evidence.
