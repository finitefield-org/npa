...
- Public `npa-mathlib` namespace policy, standalone repository activation,
  ...
  evidence, Layer 3D-A kernel/image release evidence, Layer 3D-B kernel
  quotient release evidence, and Layer 3D-C first isomorphism release evidence
  are now fixed. Larger theorem layers, release signing, and high-trust
  evidence remain explicit follow-up work.
...
Layer 3C subgroup containment/order release continuation, the
`npa-mathlib v0.1.7` Layer 3D-A kernel/image release continuation, and the
`npa-mathlib v0.1.8` Layer 3D-B kernel quotient release continuation, and the
`npa-mathlib v0.1.9` Layer 3D-C first isomorphism release continuation.
...
- `finitefield-org/npa-mathlib` release tag `v0.1.8`
  - tag object: `25aa8ef4bf8a68b15807769b0a57a4cf0729ff3d`
  - target commit: `dd5cec062b08023eacb3252864b64653b3199814`
- `finitefield-org/npa-mathlib` release tag `v0.1.9`
  - tag object: `0b5cff064283a0659154ac605f551625f9f6b553`
  - target commit: `d64e8a70e4b6c1b9f8427285a9c6dc46342db618`
...
- `npa-mathlib v0.1.8` release:
  `https://github.com/finitefield-org/npa-mathlib/releases/tag/v0.1.8`
- `npa-mathlib-v0.1.8-release-artifacts.tar.gz`:
  `https://github.com/finitefield-org/npa-mathlib/releases/download/v0.1.8/npa-mathlib-v0.1.8-release-artifacts.tar.gz`
- `npa-mathlib v0.1.8` bundle SHA-256:
  `3d179c92c455628a9ffeb3b0cd607fcabc783cfe990a0e100f7b36fdf2696ed7`
- `npa-mathlib v0.1.9` release:
  `https://github.com/finitefield-org/npa-mathlib/releases/tag/v0.1.9`
- `npa-mathlib-v0.1.9-release-artifacts.tar.gz`:
  `https://github.com/finitefield-org/npa-mathlib/releases/download/v0.1.9/npa-mathlib-v0.1.9-release-artifacts.tar.gz`
- `npa-mathlib v0.1.9` bundle SHA-256:
...
- `npa-mathlib v0.1.8` release gates passed locally in the standalone
  repository: `check`, `build-certs --check`,
  `verify-certs --checker reference`, `check-hashes`,
  `axiom-report --check`, `index --check`, and `publish-plan --check`.
- `npa-mathlib v0.1.8` published-release downstream smoke passed `check`,
  `build-certs --check`, `verify-certs --checker reference`, and
  `check-hashes` after vendoring only release-bundle certificate bytes for
  `Std.Logic.Eq`, `Mathlib.Logic.EqReasoning`,
  `Mathlib.Algebra.Group.Basic`, and
  `Mathlib.Algebra.Group.Kernel.Quotient.*`.
- `npa-mathlib v0.1.9` release gates passed locally in the standalone
  repository: `check`, `build-certs --check`,
  `verify-certs --checker reference`, `check-hashes`,
  `axiom-report --check`, `index --check`, and `publish-plan --check`.
- `npa-mathlib v0.1.9` published-release downstream smoke passed `check`,
  `build-certs --check`, `verify-certs --checker reference`, and
  `check-hashes` after vendoring only release-bundle certificate bytes for
  `Std.Logic.Eq`, `Mathlib.Logic.EqReasoning`,
  `Mathlib.Algebra.Group.Basic`, `Mathlib.Algebra.Group.Image`,
...
- GitHub Actions status for `npa-mathlib v0.1.1`, `v0.1.2`, `v0.1.3`,
  `v0.1.4`, `v0.1.5`, `v0.1.6`, `v0.1.7`, `v0.1.8`, and `v0.1.9` was
...
- Broader theorem layers beyond the released Layer 3D-C first
  isomorphism-to-image set remain future `npa-mathlib` release work.
...
The published GitHub release asset digest for
`npa-mathlib-v0.1.8-release-artifacts.tar.gz` matched the local bundle
SHA-256:
`3d179c92c455628a9ffeb3b0cd607fcabc783cfe990a0e100f7b36fdf2696ed7`.
The downstream package vendored only certificate bytes from the release bundle
for `Std.Logic.Eq`, `Mathlib.Logic.EqReasoning`,
...
`Mathlib.Algebra.Group.Kernel.Quotient.Hom`, then passed `check`,
`build-certs --check`, `verify-certs --checker reference`, and
`check-hashes`.

The `v0.1.8` negative checks rejected a bad
`Mathlib.Algebra.Group.Kernel.Quotient` export hash, a bad
`Mathlib.Algebra.Group.Kernel.Quotient.Hom` certificate hash, corrupted
certificate bytes, and a stale package lock from bad package version before
proof acceptance.
...
The published GitHub release asset digest for
`npa-mathlib-v0.1.9-release-artifacts.tar.gz` matched the local bundle
SHA-256:
`101d1dfc017c2ade6d0e37e0eeafa11245770b16594046a3e7a033e486b7f752`.
The extracted source-free release bundle passed `package check`,
`verify-certs --checker reference`, `verify-certs --checker fast`,
`axiom-report --check`, `index --check`, and `publish-plan --check`. The root
release bundle intentionally excludes source files, so `package check-hashes`
is not the source-free release-bundle gate.
...
`Mathlib.Algebra.Group.FirstIsomorphism.Image`, then passed `check`,
`build-certs --check`, `verify-certs --checker reference`, and
`check-hashes`.

The `v0.1.9` negative checks rejected a bad
`Mathlib.Algebra.Group.FirstIsomorphism` export hash, a bad
`Mathlib.Algebra.Group.FirstIsomorphism.Image` certificate hash, corrupted
certificate bytes, and a stale package lock from bad package version before
proof acceptance.
...
## Public Layer 3D-C Release Facts

The standalone `npa-mathlib v0.1.9` generated publish plan records:

- schema: `npa.package.publish_plan.v0.1`
- package: `npa-mathlib`
- version: `0.1.9`
- release artifact count: 30
- module registry seed entry count: 24
- downstream import bundle module count: 24
- checker summary count: 52
- signature policy: `checksum-only`
- hash algorithm: `sha256`
- signatures: empty
- publish plan hash:
  `sha256:fa7703515c482d53b45d3a82d6fcfbfffd620a138ef8938a084bba7886e14fdb`

The standalone `npa-mathlib v0.1.9` generated artifacts also record:

- package lock entries: 26
- theorem index entries: 253
- theorem index checker summaries: 26
- axiom report modules: 26
- axiom report local modules: 24
- axiom report external modules: 2
- direct axiom count: 1
- transitive axiom count: 11
- policy violation count: 0
- allowed axioms: `Eq.rec`

The standalone `fixtures/downstream-smoke/generated/package-lock.json` records
one local downstream module, `Downstream.GroupFirstIsomorphism`, and these
external imported modules:
...
- Treat `npa-mathlib v0.1.9` as the current public theorem-library baseline
  for the next algebra closure audit.
- Audit the normal-subgroup quotient route before materializing second/third
