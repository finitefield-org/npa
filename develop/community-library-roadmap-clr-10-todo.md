...
`develop/community-library-roadmap.md` section 4.2, and decides one of three next
steps:
...
`proofs/generated/`, or `docs/external-theorem-library-ci.md` paths. CLR-10
...
- `develop/community-library-roadmap.md` section 4.2.

Implementation:

1. Create or update `develop/registry-readiness.md`.
...
```sh
npa package publish-plan --root . --check --json
rg -n "npa.package.publish_plan.v0.1|npa.registry.module.v0.1|downstream_import_bundle|checksum-only|checker_binary_registry" develop/registry-readiness.md generated
```
...
rg -n "npa-mathlib-seed|fresh checkout|downstream_import_bundle|reference-checker-only|theorem-only" develop/registry-readiness.md README.md CONTRIBUTING.md
```
...
```sh
rg -n "not checker input|not proof evidence|reference-checker-only|verified_high_trust|kernel.*network|registry lookup" develop/registry-readiness.md develop/community-library-roadmap-clr-10-todo.md
git diff --check
```
...
```sh
rg -n "Decision|create registry server|continue Git-release-based registry seed|defer registry work|why not" develop/registry-readiness.md
git diff --check
```
...
```sh
rg -n "Follow-up|server MVP|later ecosystem work|non-goal|namespace|authenticated publish|immutable" develop/registry-readiness.md
git diff --check
```
...
1. Update `develop/community-library-roadmap.md` with the registry readiness
   decision summary.
2. Update `develop/community-library-roadmap-todo.md` to point to the detailed
...
1. Re-read the readiness record against `develop/community-library-roadmap.md`
   section 4.2.
