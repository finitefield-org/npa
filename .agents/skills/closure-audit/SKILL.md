...
## Package Audit Acceleration

Promotion candidate discovery remains read-only. The package audit selection
surface may be generated by `--promote-plan`, manual closure review, or the
internal `npa_package::select_package_audit_modules` API while the public
selection CLI is pending. Selection output is not proof evidence; it only
identifies which modules need a live package audit.

During local iteration, you may use the package audit cache to shorten repeated
source-free checks:

```sh
cargo run -q -p npa-cli -- package verify-certs --root ../npa-mathlib --checker reference --audit-cache read-through --json
cargo run -q -p npa-cli -- package verify-certs --root ../npa-mathlib --checker reference --audit-cache local-hit --json
```

Record these runs as local acceleration only. An audit document that uses them
must include:
...
Do not end a closure audit with only `read-through`, `local-hit`, or selection
output. Final promotion readiness requires the cache-off gates in the Positive
Gates section below.
...
checkout after materialization. These are final evidence commands; keep
`verify-certs` explicitly cache-off even if local acceleration was used during
iteration:
...
cargo run -q -p npa-cli -- package verify-certs --root ../npa-mathlib --checker reference --audit-cache off --json
...
cargo run -q -p npa-cli -- package verify-certs --root ../npa-mathlib/fixtures/downstream-smoke --checker reference --audit-cache off --json
