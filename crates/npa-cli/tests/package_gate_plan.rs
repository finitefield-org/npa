...
fn package_gate_plan_policy_covers_pas16_impact_classes() {
    ...
    assert!(plan_requires(
        &high_trust,
        concat!("./scripts/", "phase", "8", "-release-audit.sh")
    ));
...
#[test]
fn package_gate_plan_scripts_report_by_default_and_select_only_by_opt_in() {
    let root = repo_root();
    let helper = fs::read_to_string(root.join("scripts/package-gate-plan-report.sh")).unwrap();
    assert!(helper.contains("NPA_PACKAGE_GATE_PLAN:-report"));
    assert!(helper.contains("NPA_PACKAGE_GATE_PLAN_SELECT"));
    assert!(helper.contains("changed_path_count"));
    assert!(!helper.contains("proof accepted"));
    assert!(!helper.contains("proof is accepted"));
    ...
    for script in [
        ...
        let content = fs::read_to_string(root.join(script)).unwrap();
        assert!(content.contains("source scripts/package-gate-plan-report.sh"));
        assert!(content.contains("npa_package_gate_plan_report"));
        assert!(content.contains("npa_package_gate_plan_apply_selection"));
        assert!(!content.contains("proof accepted"), "{script}");
        assert!(!content.contains("proof is accepted"), "{script}");
    }
