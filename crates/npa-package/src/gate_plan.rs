...
fn required_commands_for_plan(
    ...
    if impact_class >= PackageGateImpactClass::PackageMetadataProjection {
        push_unique(&mut commands, "./scripts/check-fast.sh".to_owned());
        push_unique(
            &mut commands,
...
fn optional_commands_for_plan(
    ...
    if impact_class >= PackageGateImpactClass::PackageMetadataProjection {
        push_unique(
            &mut commands,
            "cargo test -p npa-cli package_cli_smoke".to_owned(),
        );
        push_unique(
            &mut commands,
        ...
        push_unique(
            &mut commands,
            "cargo run -p npa-cli -- package verify-certs --root proofs --checker fast --audit-cache local-hit --json"
                .to_owned(),
        );
    }
