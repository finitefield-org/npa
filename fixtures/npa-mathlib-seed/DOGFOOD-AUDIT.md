...
A new contributor can treat `fixtures/npa-mathlib-seed/` as the package root and
follow `CONTRIBUTING.md` without knowing internal `npa` workspace paths. The
check-mode command sequence is local to the seed package:
...
certificate-format, package trust, CI trust, or registry behavior change in
`npa`.
...
- theorem index and publish-plan drift are review metadata, not trusted proof
  evidence;
- replay files, meta files, automation output, AI output, CI status, and
  future registry metadata are not trusted proof evidence.

## Downstream Package Review

`fixtures/npa-mathlib-seed-downstream/` consumes the seed as a downstream
...
- `CONTRIBUTING.md` as contributor workflow input;
...
The seed fixture contains GitHub workflow files under
`fixtures/npa-mathlib-seed/.github/workflows/` so the fixture can be copied to a
standalone repository. They are not active inside the parent `npa` repository
because GitHub only activates workflows from the repository root
`.github/workflows/`.

The parent repository currently has no root workflow files for this fixture.
That keeps CLR-09 dogfood CI inert in `npa` while preserving the external
repository template.
...
- Standalone repository activation: copying the fixture to a separate
  `npa-mathlib-seed` repository and enabling its workflows is outside this
  parent-repository milestone.
