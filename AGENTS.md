...
`npa-mathlib` へ promote してよいのは、roadmap / theorem card / audit で
`L2 Derived certificate` と確認できる証明済み定理だけです。`L0` statement /
conjecture、`L1` evidence package / interface、結論そのものを仮定する boundary
theorem、level が不明な候補は promote せず、必要なら先に `L2` 定理として分割・証明して
から再判定してください。
